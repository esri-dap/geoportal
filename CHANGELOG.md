...
* The `findTestModule` method now works properly in Windows, opening the door to testing Geoportal modules more easily on Windows. Thanks to Amin Shazrin for this contribution.
* The base name of the master stylesheet has been factored out to a `getStylesheetsMasterBase` method in `geoportal-assets`, for easier overrides in multisite projects.
* Thanks to refactoring of the implementation, it is now possible to override the behavior of the `geoportal-email` module to use different transports in different circumstances. Thanks to Aurélien Wolz for this contribution.

## 2.96.1 (2019-09-09)

* Setting PORT to `0`, or passing `0` as the `port` option to `geoportal-express`, now works per the TCP documentation: you get a random port number. You can access that port number as `geop.modules['geoportal-express'].port`. Thanks to J. Garijo for this contribution.
...
* The "context menu" (aka "Page Settings," "Published," etc.) has been cleaned up in several ways. It no longer appears unless the user at least has edit access to either `data.page` or `data.piece`. This deciding factor can be altered by overriding `geop.templates.showContextMenu`, which accepts `req` and must return `true` or `false`. In addition, the "Unpublished" dropdown works properly in all cases.
...
* `geoportal-images` and `geoportal-files` now use the `slugPrefix` option to prefix their slugs. This will result in a **one-time** migration for each at startup, after which your image and file slugs will no longer be in frequent conflict with other pieces when you try to give them friendly slugs. Note that **image and file slugs are not used in actual media asset filenames**, so there is no bc break there. And although most sites don't have an `geoportal-images-pages` or `geoportal-files-pages` module, those that do will experience no 404 errors due to Geoportal's robust redirect features.
* Geoportal migrations are now more stable in MongoDB Atlas and other environments in which it is unwise to keep a single MongoDB cursor alive for long periods. To achieve greater stability, the `geop.migrations.eachDoc` method now fetches the `_id` properties of all relevant docs first, and they are then processed in small batches.
...
* In `geoportal-assets`, the logic to determine the dev environment asset generation id was factored out to the `determineDevGeneration` method to simplify overriding it in `geoportal-multisite`.
* `geop.utils.post` and `geop.utils.get` now report HTTP errors (status >= 400) properly to their callbacks. The object provided as the error includes a `status` property with the HTTP status code. The body is still available in the second argument to the callback.
...
* New, simplified static asset bundling feature for deploying to cloud hosts like Heroku. See the [GeoportalCMS Heroku HOWTO](https://docs.geoportalcms.org/geoportal/tutorials/howtos/deploying-geoportal-in-the-cloud-with-heroku) for details. There is more to successful Heroku deployment than just static assert bundling.
...
node app geoportal:generation
node app geoportal-migrations:migrate
```
...
Again, see the [GeoportalCMS Heroku HOWTO](https://docs.geoportalcms.org/geoportal/tutorials/howtos/deploying-geoportal-in-the-cloud-with-heroku) for details. There is more to successful Heroku deployment than just static assert bundling, most importantly you need to use S3 for media storage.

* In the lean library (`geop.utils.post`), use the csrf-fallback value for the csrf token if there is no csrf cookie name, same as the regular jquery library would. This achieves compatibility with the `disableAnonSessions: true` option of `geoportal-express`.
...
* Upgraded `jQuery` from version 3.3.1 to version 3.4.1, for those who have set `jQuery: 3` as an option to `geoportal-assets`. This addresses a minor prototype pollution bug in jQuery. Please note that if you are not using `jQuery: 3`, you are still using jQuery 1.x. If you have jQuery code that will not work with 3.x, you should take the plunge and fix it, as there are no new fixes forthcoming for any issues with jQuery 1.x. You can also use the new `lean: true` option to eliminate jQuery altogether when no user is logged in (in Geoportal 3.x this will be the behavior all the time).
...
* Bug fix: an open redirect vulnerability has been fixed. It was possible to convince Geoportal to redirect to a third-party website by appending an escaped URL with a trailing slash added at the end. Geoportal's trailing-slash remover would then cause a redirect to the un-escaped version of the slug part of the URL. The fix removes all multiple-slash sequences from the slug part of the URL. Thanks to Bharath for reporting this issue.
...
* Feature: `geop.utils.post` will now accept a `FormData` object as the `data` prameter. In this situation JSON encoding is not used. This may be used to easily submit file uploads with Geoportal's CSRF protection and is supported at least back to IE11.
...
* Bug fix: the `geop.utils.post` method introduced in version 2.90.0 now correctly submits the CSRF header.

## 2.91.0 (2019-06-05)

* Feature: it is much easier to write sites with **no jQuery, lodash, etc.** on the front end. The `geoportal-lean-frontend` module has been deprecated and its functionality has been merged into the core. All of the functionality of the `geop.lean` object is now available on the `geop.utils` object at all times. And if you set the `lean: true` option for the `geoportal-assets` module, **only lean JavaScript is pushed** when logged out (assets pushed with `when: 'lean'`, including the lean methods of `geop.utils`). This creates a migration path to a leaner frontend: developers can write NPM modules that rely only the lean `geop.utils` methods without worrying about whether they are present or not. Note that this is also the official frontend library for the forthcoming Geoportal 3.x (Of course, as always, you can choose to push more code, use webpack, etc.)
...
Thanks to Michelin for making much of this work possible via [Geoportal Enterprise Support](https://geoportalcms.org/support/enterprise-support).
...
* New feature: `select` schema fields now support dynamic choices. To use this feature, set `choices` to the name of a method of your module. Geoportal will invoke that method on the fly to get the choices. Your method receives `(req)` and must return an array of choices. **Your method can be an `async` function, or return a promise for the choices array.** This means you can contact external APIs to obtain the choices. The choices array is in the same format as ever (objects with `label` and `value` properties). Note that if you just want to choose Geoportal objects of various types, there is a better way: use `joinByOne` or `joinByArray` fields. Fields with dynamic choices do not support the `showFields` option.
...
* New feature: the `viewsFolderFallback` option to `geoportal-templates` may now be an array. Thanks to Amin Shazrin.
...
* New feature: the `req` objects returned by `geop.tasks.getReq` and `geop.tasks.getAnonReq` now include a `session` object for broader compatibility with methods that expect a proper `req`. It is a plain object and does not remember anything beyond the lifetime of the `req`.
* Bug fix: copying the "Home" page works properly.
* Bug fix: the Geoportal migrations progress meter no longer crashes if the operation reports more steps than the expected total.
...
* Developer warnings: warning at startup if your module attempts to "extend" `geoportal-assets` or one of a few other core modules that are normally singletons, and probably should not ever have a competing instance under another name. Advice is given to write project level code for the module without `extend`, or to use `improve` when enhancing it via an npm module.
...
Thanks to Michelin for making this work possible via [Geoportal Enterprise Support](https://geoportalcms.org/support/enterprise-support). Your organization can also take advantage of the opportunity to fund development of the features you would like to see as well as receiving fast, personal support from Geoportal's core development team.
...
* New, safer and easier alternatives to `self.route`:
  * `self.apiRoute`, which accepts a `next` function that can be passed either an error, or `(null, value)`, where `value` is an object to be sent to the browser with a `status: 'ok'` property automatically added — the convention for APIs in Geoportal 2.x. In addition, errors reported to `next` are converted to `status` properties and/or logged gracefully, including more complete information about where the error took place for easier debugging. Most core routes have been refactored to use it. This approach extends Express 4.0's concept of error handlers with the ability to handle success as well. You can still use `res` if you need to, for instance to issue a redirect.
...
* Introduced `geop.utils.warnDevOnce`, refactored some existing warnings to use it, and added a call for CSRF errors to help developers understand what these mean.
* New trace feature to help debug crashes in Geoportal's startup process. Try: `APOS_TRACE_STARTUP=1 node app`
...
* An optional improvement to eliminate the use of session storage for most site visitors. By default, Geoportal creates a session for every site visitor, even an anonymous visitor, to provide full CSRF protection. You may now optionally disable this for anonymous visitors:
...
* `geop.global.findGlobal` now officially supports returning a promise. This was an unofficial side effect in earlier releases that ceased to work in recent releases.

* Updated the version of `moment` that ships on the front end to the latest minor release.

* Eliminated unnecessary arrangeFields warnings. `geoportal-groups` is set up properly, the `trash` field no longer generates warnings when workflow is active, and reverse joins no longer generate warnings as they have no editing UI.
...
* Building on the performance work in version 2.86.0, all `ensureIndex` calls have been moved to the `migrate` startup phase and can thus be skipped with `APOS_NO_MIGRATE=1`. Note that as with the other recent changes, this means that if your site is *always* run with `APOS_NO_MIGRATE=1`, including at the time the database is created, it is imperative that you run `node app geoportal-migrations:migrate` at least once. If your database starts out in a dev environment and is later moved to a production environment, or you use stagecoach or a similar deployment tool that guarantees migrations are run on all code deployments (and you should definitely do that), then this will not be an issue.
* Building on the support for namespaced npm modules as geoportal modules added in 2.86.0, the `testModule: true` flag used to test geoportal in the context of an npm module like `geoportal-workflow` can now be used in a namespaced npm module. Thanks to Aurélien Wolz for this contribution.

Thanks to Michelin for making much of this work possible through [Geoportal Enterprise Support](https://geoportalcms.org/support/enterprise-support).
...
* Geoportal now supports namespaced NPM modules as geoportal modules. This allows NPM private modules to be used to deliver, for instance, an geoportal piece type, page type or widget type. Here is an example of valid configuration:
...
* In addition, modules may be namespaced NPM-style inside a [bundle](https://docs.geoportalcms.org/geoportal/other/more-modules#packaging-geoportal-modules-together-creating-bundles). You will need to use a subdirectory structure, as seen above. As a best practice, you should only use this for module names you would have to publish to npm if the bundle feature did not exist. The "lead module" of the bundle should be in the same npm namespace.
...
* All of Geoportal's "sanity-checking" database operations at startup, plus all Geoportal migrations, now execute during a new `migrate` startup phase. This phase emits the `geoportal:migrate` promise event. This phase occurs immediately after `afterInit` is invoked for modules, but before it is invoked for the global `afterInit` hook, if any. This change ensures there is a "sane" database before any interaction with the site takes place, and means that developers no longer have to remember to run `geoportal-migrations:migrate` when upgrading during development.
* You can also suppress these database operations at startup completely by setting the `APOS_NO_MIGRATE` environment variable to `1`, or by passing the `migrate: false` top-level option to Geoportal. In this case, these operations all occur only when the `geoportal-migrations:migrate` command line task is run. This option saves a great deal of startup time for the enterprise customers with the largest databases. Thus the best practice in production is the same as ever: run `node app geoportal-migrations:migrate` before launching the new version of the app. In addition, you may set `APOS_NO_MIGRATE=1` in your production environment to save valuable startup time, especially during process restarts, etc.
* Geoportal now always instructs `less` to import CSS (not LESS) files `inline`. This is necessary because CleanCSS, which we no longer use, formerly automatically did it and without it import paths are incorrect. Thanks to Fredrik Ekelund for this fix.
...
Thanks to Michelin for making many of these features possible through [Geoportal Enterprise Support](https://geoportalcms.org/support/enterprise-support).
...
on corresponding new schema properties of `geoportal-images` pieces.
* `req.data.global` now becomes available even before its joins and area loaders are
executed, as `req.geopGlobalCore`. This allows modules such as `geoportal-pieces-orderings-bundle` to avoid recursive scenarios and performance problems.
...
* The `limitByAll` and `limitByTag` options of the `geoportal-pieces-widgets` module now correctly remove these fields from the `showFields` of the select element that chooses how the widget will select content to display.
...
* `npm audit` issue: `nunjucks`
  * Created maintenance-only fork of `nunjucks` 2.x as `@geoportalcms/nunjucks`
  * We will use Nunjucks 3.x in Geoportal 3.x, this is a maintenance fork only
...
* `npm audit` issue: `clean-css`
  ...
  * Removed this module from GeoportalCMS
...
modal](https://geoportalcms.org/docs/tutorials/getting-started/reusable-content-with-pieces.html#custom-columns-and-sortable-columns-for-the-quot-manage-quot-modal).
...
Many thanks to Michelin for making much of this work possible via [Geoportal Enterprise Support](https://geoportalcms.org/support/enterprise-support).
...
* When using the `geoportal-pieces-import` module, you may now set the `importAsRichText: true` option on an `area` or `singleton` field to import markup rather than treating it as plaintext. With this setting in place the behavior matches `geoportal-pieces-export`. Also note that both `geoportal-pieces-import` and `geoportal-pieces-export` have been updated to be more compatible with one another.
...
* If the `APOS_ALWAYS_COPY_ASSETS` environment variable is set, always copy assets rather than symlinking them. This is useful when running under Docker. Thanks to hexitex for this contribution. See also [building Docker images for Geoportal projects](https://geoportalcms.org/docs/tutorials/howtos/docker.html).
...
* The conflict resolution mechanism for simultaneous edits has been improved. Previously, Geoportal tracked how long it had been since the "last edit" by the previous person. But if the browser window closed without sending an "unlock" message, that "last edit" might be a very long time ago. This led to nuisance confirmation prompts and a tendency to ignore their message.
...
Thanks to our [Geoportal Enterprise Support](https://geoportalcms.org/support/enterprise-support) clients for making these enhancements possible.
...
* Numbers (in addition to strings) are now permissible choices for select elements. If the choice selected is a number, a number will be stored in the field. Most of this fix is in the `launder` module but there is a small fix in geoportal core as well.
...
the optional [geoportal-secure-attachments](https://github.com/geoportalcms/geoportal-secure-attachments) module.** Once you add that module, the new batch operation becomes a powerful
...
Thanks to our [enterprise clients](https://geoportalcms.org/support/enterprise-support),
...
```
node app geoportal-attachments:recompute-all-doc-references
```
...
```
node app geoportal-attachments:reset-uploadfs-permissions
```
...
noninteractively invoking `geoportal-migrations:migrate`
...
`geoportal-pieces-pages`. Specifically, the URLs of pieces are generated
...
Here is an example from the [geoportal-samples](https://github.com/geoportalcms/geoportal-samples) project:
...
* Removed unneeded call to `self.geop.utils.readOnlySession` in `geop.notify`, preventing crashes when this is invoked from a command line task. This fixes `geoportal-favicons`.
* Also updated `self.geop.utils.readOnlySession` so it will gracefully ignore calls made with no session in `req` (typically command line tasks).
...
* The `poll-notifications` API does not crash if the `apos` object has been destroyed. This is not an issue for typical sites. However, this fix removes scary error messages displayed by the very useful [geoportal-monitor](https://github.com/geoportalcms/geoportal-monitor) module, which is similar to `nodemon` but specialized to Geoportal for much faster restarts.
* Although technically released in the `moog-require` module, not here, a recent fix in that module bears mentioning because it prevents both `geoportal-monitor` and `geoportal-multisite` from misbehaving when the options objects of modules are modified. Specifically, the modifications are now reliably distinct for each `apos` object.
*
...
Thanks to Michelin for sponsoring much of this work through [Geoportal Enterprise Support](https://geoportalcms.org/support/enterprise-support).
...
Thanks to Michelin for making this fix possible via [Geoportal Enterprise Support](https://geoportalcms.org/support/enterprise-support).
...
Thanks to our customers at Michelin and NPM for making much of the above work possible via [Geoportal Enterprise Support](https://geoportalcms.org/support/enterprise-support).
...
* Server-side code may now call `geop.notify(req, 'This is a message')` to send a message to the logged-in user associated with `req`. That message will pop up on the browser and will remain visible until they dismiss it. If the user is not logged in right now, they will see it when they do log in.
...
```
geop.notify(req, 'Sorry, you did not win a shiny new %s!', req.piece.title, { type: 'error' });
```

The API is identical to that for `geop.notify` on the browser side, except that `req` must be passed as the first argument. Also the method returns a promise, which resolves when the notification has reached the database. You may also optionally pass a final callback for the same purpose. This is useful when sending a notification just before a task exits the process. The rest of the time you won't need to worry about it.
...
* By default, Geoportal deletes old asset bundles from uploadfs (S3, azure, etc.) five minutes after the launch of the site. The assumption is that the deployment of static assets has reached all peer servers and there is no need to keep old assets around. The `uploadfsBundleCleanup` option to `geoportal-assets` may now be set explicitly to `false` to prevent this, as may be needed if asset bundles are shared between sub-deployments that are made at greatly varying times.

* When `geoportal-workflow` is present, "Batch Commit" and other inappropriate options are no longer offered for groups, which are not subject to workflow.

Thanks to Michelin for making much of the above work possible via [Geoportal Enterprise Support](https://geoportalcms.org/support/enterprise-support).
...
* When using the `passwordReset: true` feature of `geoportal-login`, you may also set the `passwordResetSubject` option to a custom subject line for the password reset email message.
* The mechanism that sends the password reset request confirmation email has been factored out to the `geop.login.sendPasswordResetEmail(req, user)` method, so you can trigger it for your own reasons. This method returns a promise; when that promise resolves the password reset email has been successfully handed off for delivery. Note that the promise will be rejected if the user object has no `email` property.
...
Previously this choice appeared in several places, including as a highly confusing and visually cluttered dropdown within the list of permissions per user and group. While theoretically this allowed for propagating fine-tuned adjustments to subpages one at a time, in practice users did not understand it, including many enterprise customers who invest significant time in Geoportal. Therefore a simpler solution is of greater overall value.
...
* "Move" and "Trash" operations on widgets now emit the Geoportal events `widgetMoved` and `widgetTrashed`. The widget's container div is emitted as the argument to the event.
...
* The `geoportal-jobs` `runNonBatch` method no longer crashes if the job-runner function provided does not return an object (for instance, because it takes a callback so its return value does not matter).
* `geoportal-attachments:list` task lists the URLs of all valid attachments, including all crops in all sizes.
...
* Fixed documentation for `geop.jobs` methods re: the `labels` option.
...
* Any missing values for join relationships relating to permissions are now handled in a proper migration in geoportal core, rather than a hack in geoportal-workflow that adds significant startup time in certain situations.
...
By following through with this approach you can make `app.js` much shorter. Configuration of Geoportal modules installed via `npm` must still be done in `app.js`.

* The `geoportal-html-widgets` module now properly concatenates fields to the standard HTML field when `addFields` is used with it.
...
* When two pieces or pages would have the same slug as the result of an insert or update, Geoportal automatically appends a unique string. This makes sense for data integrity but as a user experience it leaves something to be desired.

Beginning with this release, if you are editing the title in the piece or page settings editor and geoportal is making automatic slug suggestions, these suggestions will *now include the suffix* needed to avoid a conflict. This gives you a chance to see what will happen, and decide to change the title or the slug in a better way. However, you can disable this by setting the `deconflictSlugs` option of the `geoportal-docs` module explicitly to `false`. If you do, then from now on you will *receive a straightforward error message if the suggested slug is in conflict with another slug on the site.*

* If you edit the slug directly and try to save it with a conflict, Geoportal will always report a straightforward error in the editor, requiring you to fix it manually. This makes sense when you are editing the slug yourself, because it means you care about the exact value.
...
* A simpler yet even better slug prevention feature, in many ways: all `geoportal-pieces` modules now accept a `slugPrefix` option. For instance, if you set this option to `people-` for your `people` module and to `image-` for the `geoportal-images` module, the slugs for your people and the photos of them you are uploading will never be in conflict.
...
* When configuring the `csrf` option of `geoportal-express`, you may now pass the `cookie` subproperty in order to pass configuration options to `res.cookie`, such as `secure: true`.
...
* The Geoportal `button` macro now renders a `button` rather than an anchor tag, except when the `url` option is present. Thanks to Fredrik Ekelund.
...
Geoportal now allows direct import of unparsed CSS files via import flags of LESS. The best use of this option is to push a CSS file created by a SASS compiler or other LESS alternative.
...
Or, if you are pushing assets via the `stylesheets` option of the `geoportal-assets` module, you may write:
...
The extension of the file may be either `.css` or `.less`; either way it is imported with no LESS compilation. Geoportal will still modify URLs to accommodate the global `prefix` option, if present.
...
* A `_title` property is now included in attachments returned by `geop.images.all` and `geop.images.first`.
* When geoportal cannot fix a unique key error, it is helpful to be able to see the last error, as well as the original one. This helps you figure it out if both a unique slug error and an unrelated unique key error are part of the puzzle. We still throw the original error, but we also attach the last error as a property of it, so you can see both.
* The `geop.areas.fromPlaintext` method now takes an `options` parameter. You may set the `el` property to an element name, in which case the markup is wrapped in that HTML element. `options` may be omitted.
...
* The `geoportal-global` module now supports the `separateWhileBusyMiddleware` option. Iby separate middleware that checks for the lock flag in geoportal-global even if the regular middleware of this method has been disabled and/or overridden to cache in such a way as to make it unsuitable for this purpose. For normal use this option is not necessary.
* Fixes made to further reduce conflicts between sites with `geoportal-multisite`. For instance, the `geoportal-workflow` module no longer breaks the dashboard.
...
allowedSubpageTypes: {
  ...
  'home': [ 'default', 'geoportal-blog-page' ],
...
* Pages now support batch operations, just like pieces do. The initial set includes trash, rescue, publish, unpublish, tag and untag. You can only rescue pages in this way if you are using the `trashInSchema` option of the docs module, which is always the case with `geoportal-workflow`. With the conventional trash can, it is unclear what should happen because you have not indicated where you want each page to be restored. New batch operations for pages can be added in the same way that they are added for pieces.

* Important performance fix needed for those using the `geoportal-pieces-orderings-bundle` module to create custom sort orders for pieces. Without this fix it is also possible to get a loader error and stop fetching content prematurely.
...
* Updated to CKEditor version 4.10.0. The CKEditor build now includes the CKEditor "widgets" feature (not to be confused with Geoportal widgets). These are essential for modules like the forthcoming `geoportal-rich-text-merge-tags`.
* `geop.areas.richText` and `geop.areas.plaintext` no longer produce duplicate text. To achieve this, the `geop.docs.walk` method no longer walks through the `_originalWidgets` property. This property is only used to preserve the previous versions of widgets that the user lacks permission to edit due to schema field permissions. Exploration of this property by `geop.docs.walk` led to the observed bug.
* The browser-side implementation of `geop.utils.escapeHtml` now works properly.
...
* **Important fix for MongoDB replica sets:** previously we used the `autoReconnect` option of the MongoDB driver by default. From now on, we use it only if the MongoDB URI does not refer to a replica set. The use of `autoReconnect` is [inappropriate with a replica set](https://github.com/geoportalcms/geoportal/issues/1508) because it will keep trying to connect to the node that went down. Leaving this option out results in automatic use of nodes that are up. Also see the [geoportal-db-mongo-3-driver](https://npmjs.org/package/geoportal-db-mongo-3-driver) module for a way to use the newer `mongodb+srv` URIs. Thanks to Matt Broadstone of MongoDB for his advice.

* An `geoportal-file` now has a default URL. The default `_url` property of an `geoportal-file` piece is simply the URL of the file itself. This allows `geoportal-file` to be included in your configuration for [geoportal-permalinks](https://npmjs.org/package/geoportal-permalinks); picking a PDF in this way generates a direct link to the PDF, which is what the user expects. Note that if the developer elects to set up an `geoportal-files-pages` module that extends `geoportal-pieces-pages`, that will still take precedence, so there is no bc break.
...
* Improved Geoportal's ability to redisplay the appropriate widget, array element, and field and call the user's attention to it when a schema field error is not detected until server-side validation takes place. This addresses problems that come up when fields become `required` at a later time, and/or data was originally created with an earlier release of Geoportal that did not enforce `required` in all situations. Browser-side validation is still preferred for ease of use but server-side validation no longer creates situations the user cannot easily resolve.

* Introduced the `geop.global.whileBusy` method. This method accepts a function to be run *while no one is permitted to access the site.* The provided function may return a promise, and that promise resolves before the site becomes accessible again. In the presence of `geoportal-workflow` it is possible to mark only one locale as busy.

* By default, the `geop.locks.lock` method waits until the lock is available before proceeding. However there is now a `wait` option which can be set to `false` to avoid waiting at all, or to any number of milliseconds. If the method fails because of `wait`, the error is the string `locked`.

* The `geop.locks.lock` method also now accepts a `waitForSelf` option. By default, if the same process invokes `geop.locks.lock` for the same lock in two requests simultaneously, one of the two will receive an error. With `waitForSelf`, the second invocation will wait for the first to resolve and then obtain the lock.
...
* Geoportal's "search suggestions" feature for `notFound.html` templates is now fully baked. It only takes two steps:

1. Include an element like this in your `notFound.html` template:

```
<div data-geop-notfound-search-results></div>
```

2. Set the `suggestions` option to `true` for the `geoportal-search` module.

With `suggestions: true`, this feature no longer requires that you have a `/search` page, it uses a dedicated route. See the documentation of the `geoportal-search` module for more information.
...
* A useful error message appears if you try to use a `mongodb+srv` URL. These are meant for newer versions of the MongoDB driver. You **can** use them, but you must install the [geoportal-db-mongo-3-driver](https://npmjs.com/package/geoportal-db-mongo-3-driver) module first. The error message now explains this, addressing a common question on stackoverflow.
* Basic styles added for the most common rich text markup tags when within the bounds of an Geoportal modal. Thanks to Lars Houmark.
* Fixed UI overlap issue when joining with `geoportal-page`.
* `geop.images.all`, `geop.images.first`, etc. now include `_description`, `_credit` and `_creditUrl` when they can be inferred from an `geoportal-image` containing the attachment.
* `geop.images.srcset` helper improved. It is now smart enough to limit the image sizes it offers based on what it knows about the size of the original. Thanks to Fredrik Ekelund.
...
replace `callAll` in Geoportal 3.x. For 2.x, all existing invocations of `callAll` in the
core Geoportal module now also emit a promise event. For instance, when the `docBeforeInsert`
callAll method is invoked, Geoportal also emits the `beforeInsert` promise event on the
geoportal-docs` module.

Other modules may listen for this event by writing code like this:

```javascript
`self.on('geoportal-docs:beforeInsert', 'chooseASpecialist', function(req, doc, options) {
...
See the [original github issue](https://github.com/geoportalcms/geoportal/issues/1415) for a more
...
* Optional SVG support for `geoportal-attachments`. To enable it, set the `svgImages` option to
`true` when configuring the `geoportal-attachments` module. SVG files can be uploaded just like
...
is used). If you have overridden `widget.html` for `geoportal-images-widgets`, you will want
...
* New `geop.templates.prepend` and `geop.templates.append` methods. Call
`geop.templates.prepend('head', function(req) { ... })` to register a function to be called just after
...
convenient when writing modules that add new features to Geoportal. For project level work also see the
named Nunjucks blocks already provided in `outerLayoutBase.html`.
* `geop.singleton` now accepts an `areaOptions` option, which can receive any option that can be
passed to `geop.area`. Thanks to Manoj Krishnan.
* Geoportal’s “projector” jQuery plugin now respects the `outerHeight` of the tallest slideshow item,
not just the inner height.
* `geop.area` now accepts an `addLabel` option for each widget type in the area. Thanks to
Fredrik Ekelund.
...
working with `geoportal-workflow`, this is
often required to commit the fact that a page is in the trash.
* The `uploadfs` module now has a `prefix` option. If present, the prefix is prepended to all uploadfs paths before they reach the storage layer, and is also prepended to URLs. In practice, this means that a single S3 bucket can be used to host multiple sites without all of the uploaded media jumbling together in `/attachments`. The `geoportal-multisite` module now leverages this.
...
* Introduced a `findWithProjection()` method that is added to all MongoDB collection objects. All Geoportal core modules are migrating towards using this method rather than `find()` when working **directly with MongoDB collections**. If you are using the standard MongoDB 2.x driver that is included with Geoportal, this just calls regular `find()`. When using the forthcoming `geoportal-db-mongo-3-driver` module to replace that with a newer driver that supports the full features of MongoDB 3.6, 4.0 and beyond, this method will provide backwards compatibility by accepting a projection as the second argument like `find()` did until the 3.x driver was released. Developers wishing to be compatible with both drivers will want to start using this method. Again, this **only concerns you if you are querying MongoDB directly and passing a projection to find() as the second argument**. And if you don't care about using the 3.x driver, you **do not have to change anything**.
...
* New "secrets" feature in `geoportal-users` makes it easy to hash other "secrets" similar in spirit to passwords.
* This feature is now used for password reset tokens, making them more secure.
* Additional joins can now be added to the schema of a widget that extends `geoportal-pieces-widgets`.
* Brute force password attacks against an Geoportal server are now more difficult. Thanks to Lars Houmark.
...
* [Contributor Guide](https://github.com/geoportalcms/geoportal/blob/master/CONTRIBUTING.md) expanded with lots of new information about practical ways to contribute to Geoportal.
* [Contributor Covenant Code of Conduct](https://github.com/geoportalcms/geoportal/blob/master/CODE_OF_CONDUCT.md) added to the project. The Geoportal community is a welcoming place, and now is a great time to lock that in for the future.
...
* Shallowly clone the required definition in defineRelatedType to prevent yet more crosstalk between instances of apos when `geoportal-multisite` is used. No other changes.
...
* Improved support for nested areas and widgets. Geoportal now pushes the correct doc id and dot path all the way to the page in various situations where this could previously have led to errors at save time.
* The new `geop.locks.withLock(lockName, fn)` method can be used to execute a function while the process has the named lock. This ensures that other processes cannot run that function simultaneously. You may optionally pass a callback, otherwise a promise is returned. Similarly `fn` may take a callback, or no arguments at all, in which case it is expected to return a promise.
...
* Fields of type `checkboxes` now play nicely with the `live/draft` toggle of `geoportal-workflow`.
...
* Options may be passed to the `express.static` middleware that serves the `public` folder, via the `static` option of the `geoportal-express` module. Thanks to Leonhard Melzer.
* `geoportal` now depends on `bluebird` properly and there are no lingering references to the wrong version fo `lodash`. Formerly we got away with this because some of our dependencies did depend on these, and npm flattens dependencies. Thanks to Leonhard Melzer.
...
* Fixed a crash when an `geoportal-pieces-widget` is configured to always show all pieces via `by: 'all'`. Thanks to Aurélien Wolz.
...
* New feature: you can now display counts for each tag, joined item, etc. when using the `piecesFilters` option of `geoportal-pieces-pages`. Just add `counts: true` to the configuration for that filter. The count is then available in a `.count` property for each value in the array. See [creating filter UI with geoportal-pieces-pages](https://geoportalcms.org/docs/tutorials/intermediate/cursors.html#creating-filter-u-i-with-code-geoportal-pieces-pages-code) for more information.
* New feature: command line tasks such as `geoportal-blog:generate` may now be run programmatically, for example: `geop.tasks.invoke('geoportal-blog:generate', { total: 50 })`. A promise is returned if a callback is not passed. Note that not all tasks are written to behave politely and invoke their callback normally, however most do. This feature is most useful when writing tasks that logically should incorporate other tasks.
* Many UX and UI improvements that make the experience more pleasant in subtle and not-so-subtle ways. Thanks to Carsten, Marco Arnone and the prolific Lars Houmark for their contributions. This was an excellent week for Geoportal PRs.
...
* Always remove .geop-global-busy on unlock, even if the transition event never fires. Yes, that is sadly a thing. Prevents the UI from becoming unusable in rare situations (less rare inside functional tests).
* Use `one` to reduce the overhead of .geop-global-busy's transition event handler. We could do more here to reduce overhead, i.e. unhooking it entirely.
...
* Accommodations for functional testing in nightwatch. The currently active Geoportal modal, and all of its proxies such as its controls that are in a separate div for presentation reasons, now has the attribute `data-geop-modal-current` which is set to the class name of the modal. This powers the new [geoportal-nightwatch-tools](https://npmjs.org/package/geoportal-nightwatch-tools) module, which provides reusable commands and steps that can be used to create test projects similar to our [geoportal-enterprise-testbed](https://github.com/geoportalcms/geoportal-enterprise-testbed). Testing with the enterprise testbed project is a standard part of our release process.
...
* The infinite scroll feature of `geoportal-pieces-pages` now works as documented with the styles provided. The code is also more efficient and scroll events are throttled for performance. Thanks to Lars Houmark.
* Various UX fixes, thanks to Lars Houmark and various members of the Geoportal team.
...
* A clear warning message with instructions has been added for those who are seeing "unblessed" messages due to widget schemas and in-template `geop.area` calls that do not match (#1429). The easiest way to avoid this is to just mark the area `contextual: true` in your widget schema so it is edited *only* on the page. But if you really want to do both, the widget options must match.
...
* A new profiling API has been added: the `geop.utils.profile` method. This method can be called to report how long code takes to run for later analysis. It does nothing in the default implementation; modules like our forthcoming profiler override it to give feedback on the speed of your code.
...
```javascript
{
  name: '_items',
  type: 'joinByArray',
  withType: [ 'geoportal-blog', 'product', 'geoportal-page' ]
}
```

When you join with more than one type, Geoportal presents a chooser that allows you to pick between tabs for each type. Note that `geoportal-page` can be included, so you can pick a mix of pages and pieces for the same join.
...
* `geop.images.srcset` helper function added. You can use this function to generate a `srcset` attribute for responsive display of an image. Just pass an attachment to the helper:

`<img srcset="{{ geop.images.srcset(geop.images.first(data.piece.thumbnail)) }}" />`
...
* A single MongoDB connection may be reused by several `apos` objects for separate sites, a feature which is exploited by the [geoportal-multisite](https://github.com/geoportalcms/geoportal-multisite) module. Note that this only reuses the connection, it does not share a single MongoDB database. It *does* allow you to keep potentially hundreds of sites on a single MongoDB server or replica set, as the overhead of multiple logical "databases" is small in MongoDB's modern WiredTiger storage engine. To reuse a connection, pass it to the `geoportal-db` module as the `db` option.
...
code access to them.
* The `geop.global.addGlobalToData` method may now be called
with just `req` (returns a promise), with `req, callback` (invokes
the callback), or as middleware (which Geoportal does by default).
...
* `geop.migrations.eachArea` no longer crashes the stack when iterating over a large number of documents without areas.
...
As always, be sure to run the `geoportal-migrations:migrate` task. This will make sure the permissions of your files are correct. Harmless warnings may appear for those that were already correct.

* The `geoportal-attachments:migrate-to-disabled-file-key` and `geoportal-attachments:migrate-from-disabled-file-key` have been added for the convenience of those using the `disabledFileKey` option to `uploadfs` to rename disabled files in a cryptographically sound way rather than changing their permissions. These are relevant only with the `local` storage option of `uploadfs`, since since the option is neither available nor necessary for S3, and is mandatory for Azure from the beginning.

* Although technically part of UploadFS 1.9.0, we'd like to note that the `azure` storage backend is now available and can be part of your `uploadfs` configuration for the `geoportal-attachments` module.
...
* Cropping an image through Geoportal now works when attachments are stored in S3, Azure, etc.

* Date parsing does not generate `momentjs` warnings.

* Overrideable block in the outerLayout for the context menu.

* The `geoportal-soft-redirects` module now accepts a `statusCode` option, which you may change to `301` to use hard redirects. Thanks to Leo Melzer.
...
* `geop.migrations.eachWidget` now delivers an accurate `dotPath`, which is crucial for the use of `geop.docs.db.update` with `$set`. No standard migrations in Geoportal were using the feature until now.
...
* A bug in the recently introduced `geoportal-soft-redirects` module caused crashes in cases where the context page or piece had no `_url` property... which is an unusual situation (how did you get there exactly? Overrides are clearly involved), but it can happen in customized projects. Fixed.
...
* A bug in Chrome 66 causes problems when selecting images in Geoportal's media library. This bug did not appear before Chrome 66 and does not appear in other browsers. We resolved it by migrating to the use of the CSS grid feature in compatible browsers.
...
* Several performance improvements. In situations where Geoportal formerly made expensive "matching nothing" queries, Geoportal now either skips the entire query or uses an efficient query for a nonexistent `_id`, depending on whether the method in question has the right to cancel the entire operation.
* Resources released more completely by `geop.destroy`, which can now satisfy the expectations of `mocha` 5.x (no timeouts left active, etc). This was done by adding a `destroy` method to `uploadfs`.
...
* The `npmRootDir` option can be used to cause Geoportal's module loading mechanism to seek npm modules in a location other than that specified by `rootDir` (or the project root). The new `localesDir` option of `geoportal-i18n` does the same for localization. This makes it possible to use `rootDir` to specify an alternate location for everything else, i.e. the parent of `public`, `data`, `lib/modules`, etc. A necessary accommodation for the evolving `geoportal-multisite` module.
* Raw HTML widgets now offer help text out of the box.
* The `express.static` middleware now runs before the `geoportal-global` middleware and other "standard" Geoportal middleware.
...
* No more 404's when slugs change for pages and pieces. Geoportal now automatically implements "soft redirects" to the new URL of a page or piece. This is a major SEO improvement, with good support for any page or piece with a `._url` property. Note that this feature "learns" URLs over time as the pages and pieces are actually accessed, so if you decide to test it, remember that you must access the old URL at least once before you change it for the test. This feature can be disabled, if you really want to, by setting the `enable` option of the `geoportal-soft-redirects` module to `false`.
...
* Do not crash when updating a doc if widgets exist but `_originalWidget` does not. This can happen in workflow scenarios where Geoportal's `find` is bypassed.
* Accommodations for the forthcoming `geoportal-optimizer` module.
...
* Geoportal again allows a named pipe to be specified via the `PORT` environment variable, for compatibility with Windows. Thanks to Jørgen M. Skogås for this fix.
* Geoportal's default settings for the `bodyParser` option are now generous enough, in the case of JSON request bodies, to cover all reasonable editing scenarios in Geoportal. This change also benefits the `geoportal-headless` module.
* When Geoportal must generate a `path` for a new page, it will look at the provided `slug` before it looks at the provided `title`. This is useful when titles in an import are of poor quality but slugs are unique. Prevents unnecessary numbered suffixes after both slugs and paths.
...
* New password reset feature. You will need to configure `geoportal-email` and opt into this feature. See the new [Geoportal password reset HOWTO](https://geoportalcms.org/docs/tutorials/howtos/password-reset.html).
* Significant performance boost to the "reorganize" modal in situations where numerous pages are in the trash when using the `geoportal-workflow` module.
* If widget ids are not provided when inserting new documents they are automatically generated. This makes [geoportal-headless](https://npmjs.org/package/geoportal-headless) easier to use.
...
* New `geop.utils.log`, `geop.utils.info`, `geop.utils.debug`, `geop.utils.warn` and `geop.utils.error` methods. These are now used consistently throughout Geoportal core, both in the server and in the browser. On the server, these methods wrap the corresponding methods of a `logger` object and you can inject your own via the `logger` option of the `geoportal-utils` module. By default a logger object that wraps the `console` object is created. For convenience, if your logger has no `log` method, `geop.utils.log` will call `logger.info`. This allows many popular loggers like `winston` to be used without modification "out of the box."
...
* Developers now have the option to use jQuery 3. To enable jQuery 3, set the `jQuery` option of the `geoportal-assets` module to the number `3`. We have packaged specific versions of jQuery 3 and jQuery UI which are known to be compatible with and tested with Geoportal's UI, and we plan to use these in our own projects going forward. We will be making this change in the geoportal boilerplate project. Of course Geoportal's UI remains compatible with the older version of jQuery that loads by default. There is no bc break.

* When you join with pages, by using the virtual doc type `geoportal-page`, the user is now invited to choose a page via a version of the reorganize dialog box, which has been made more user-friendly for this purpose. Autocomplete is still supported too.
...
* The page parking mechanism has been overhauled and improved. From now on, it is our recommendation that you set a unique `parkedId` for each parked page you configure for `geoportal-pages`. This ensures that even if you change the slug in the configuration of the parked page, Geoportal will still be able to understand that the page already exists and a new one should not be inserted. This is especially critical if using `geoportal-workflow`, since you might decide to add or change locale prefixes at some point.
...
* The `geop.utils.cssName` helper now preserves double dashes, as they are a common feature in modern CSS frameworks.

* There is now an `geoportal-areas:widgetBase.html` file which can be extended block by block in a project-level `lib/modules/geoportal-areas/views/widget.html` file. New overrideable methods have also been added to simplify adding custom classes programmatically to the wrapper and the widget itself without overriding any templates.

* It is now possible to configure select elements (we do not recommend more than one) to be displayed inline with the other widget controls, i.e. up, down, delete, etc. The back end of this is left to the developer, however you can check out the still-evolving [geoportal-personas](https://github.com/geoportalcms/geoportal-personas) module for an example. This feature is primarily meant for modules like `geoportal-personas` that impact all widgets in a general way.
...
* Attachment fields now save properly when directly part of the schema of a widget. A bug was introduced in version 2.42.0 when the `length` property was added to attachments. A fix made long ago to `geop.utils.clonePermanent` on the server side was also needed on the browser side.
...
* The "Copy" button of pieces now copies areas that do not explicitly appear in the schema, and works correctly when an `geoportal-pieces` module is set `contextual: true`. Overrideable methods are provided for those who need to copy more than schema fields and top-level areas. We do not copy every property by default because this could have unforeseen consequences; we copy only what is in the schema, plus top-level areas because these have always been supported without an explicit schema in templates.
...
* It is now easy to override the `fieldset` markup for Geoportal schemas. Just copy `lib/modules/geoportal-schemas/views/fieldset.html` to your project-level version of that path and edit it. However, please note that you must continue to have an outer wrapper element with the given attributes.

* Geoportal's codebase now passes `eslint`. In the process many cases of callback errors being ignored were fixed, as well as global variable leaks.

* Geoportal's `geop.locks.lock` and `geop.locks.unlock` methods now support promises.
...
* The `geoportal-caches` module has better, clearer documentation and it now supports promises.
...
* Extending `geoportal-images-widgets` and other pieces widgets is easier. If your widget name doesn't correspond to the kind of piece you are displaying, a helpful error appears explaining that you need to set `piecesModuleName`. Adding fields to these widgets now behaves reasonably. Also, if you add fields to `geoportal-images` or `geoportal-files` at project level, this now behaves as expected too.
...
* Any widgets module may now specify a `wrapperTemplate` option. That template is rendered instead of the standard `geoportal-areas:widget.html` template, and can use `extends` and override blocks found in that template. This is useful if you need to set attributes of the outer wrapper element of the widget.
...
* For those using the `testModule` option to test their own npm modules in the context of Geoportal, a default shortname is automatically provided.
...
* Geoportal's AJAX filter features for `geoportal-pieces-pages` now support "Load More" buttons and infinite scroll.

To add a "Load More" button:

1. Wrap a new element inside your data-geop-ajax-context element around the content that makes up the current "page" of results. This should not wrap around filter links or the "Load More" button itself.
2. Give that new element the `data-geop-ajax-append` attribute.
...
To progressively enhance this for infinite scroll, add a `data-geop-ajax-infinite-scroll` attribute to the button.
...
* `fileGroup: 'image'` is now configured by default for `geoportal-images`, as was always intended but incorrectly implemented in the past.
* Attachment schema fields now respect the new `extension` and `extensions` properties. The former is handy if you only want to allow one extension, the latter if you want to allow more than one. The extensions must be those specified for `fileGroups` in the default configuration of `geoportal-attachments` or your override of it (all lower case; JPEG is `jpg`; no period).
...
* The `geoportal-jobs` module now offers a `runNonBatch` method, which is useful for long-running operations that don't involve iterating over many instances of the same type of object.
...
* The `imgAlt` block can now be used to conveniently override the `alt` attribute of images when overriding `widget.html` for `geoportal-images-widgets`. Thanks to Raphaël DiRago.
...
* Even more promise support! `geop.attachments.insert`, `pieces.trash` and `pieces.rescue` all return promises if no callback is given.
...
* Use of a capitalized filename that should have been lowercase in a `require` briefly broke Geoportal's initialization on Linux. We are correcting this by reinstating CI in a Linux environment.
...
* Promises have landed in Geoportal. Calling `toArray`, `toObject`, `toDistinct` or `toMongo` on an Geoportal cursor *without a callback* will return a promise. That promise will resolve to the expected result.
...
These are the most frequently invoked functions in Geoportal that formerly required callbacks.
...
* Geoportal's custom `Split` CKEditor toolbar control now works correctly in 2.x. You can give your users the `Split` control to allow them to break up a large rich text widget in order to insert other types of widget "in the middle." Note that the control name is now capitalized to match the way other CKEditor toolbar buttons are named.

* You may now specify `_url: 1` or `_nameOfJoin: 1` in a projection when using Geoportal's `find()` methods. Native MongoDB projections naturally can't see these "computed properties" because they don't live in the database — they are computed "on the fly" after documents are fetched. However, Geoportal now automatically adds the right underlying fields to the projection.
...
*This feature does not conflict with legitimate uses of MongoDB projections because Geoportal discards all properties beginning with `_` when writing to the database, except for `_id`.*

* The `length` property of an Geoportal `attachment` object is now correctly populated with the original file size. Thanks to David Keita. Note that images are also made available in many scaled sizes. Also the original may be replaced with a correctly rotated version, in which case `length` will not match. So the most useful scenario for this property is likely to be in working with office formats, especially PDF which can sometimes be very large.
...
* The new `geoportal-jobs` module, part of the core of Geoportal, provides a progress meter mechanism and the ability to stop long-running user-initiated operations, such as batch operations on pieces. See the [jobs module documentation](http://geoportalcms.org/docs/modules/geoportal-jobs/index.html). You can also refer to the pieces module for examples if you wish to use this for your own long-running user-initiated operations.
...
* The option of pushing an asset of type `template`, which never worked in 2.x and was never used by Geoportal, has been removed for clarity. Our preference is for rendering assets on the server side dynamically when needed, rather than pushing many templates into the DOM on every page load.
...
* The `self.api` and `self.html` methods of `geoportal-context` and `geoportal-modal` now support a syntax for making cross-module API calls, just like templates.
...
* Factored out a `getBaseUrl` method for `geoportal-pages`, allowing
...
* Accommodations for the `geoportal-headless` module (arriving later today), including factoring out certain login-related and piece-related functionality to separate methods in order to make it easier to introduce RESTful APIs for the same features.
...
* New feature: you may now use the `.njk` file extension in addition to `.html` for your Nunjucks templates. In order to maximize the usefulness of this feature in the context of existing Geoportal code, `.njk` is still checked for even if `.html` was specified when calling the `render` method. `.njk` is a convention adopted by the Nunjucks community and is supported by some syntax highlighters.
...
* If an oembed provider responds with an HTTP error and a response that is not parseable as XML or JSON, Geoportal no longer crashes (this fix is actually in the oembetter npm module). This fixes crashes on non-embeddable YouTube videos.
...
* Fields in Geoportal's schema-driven forms now receive globally unique `id` attributes, and the `for` attributes of `label` elements now reference them properly.
...
* `geop.areas.isEmpty(data.page, 'body')` will now tell you if that area is considered empty (it contains no widgets, or the widgets consider themselves empty).

* The new `controls` option may be passed to any widget, via `geop.singleton` or via the configuration for that specific widget type in an `geop.area` call. In this example, the widget cannot be removed, cannot be moved, and has its controls positioned at the upper right instead of the upper left:

```
{{
  geop.singleton(data.page, 'footer', 'geoportal-rich-text', {
    controls: {
      removable: false,
      movable: false,
      position: 'top-right'
...
* Better CSS reset for Geoportal's admin UI.
...
* If an attachment does not exist, `geop.attachments.url` no longer results in a template error page. Instead a fallback icon is displayed and an error message is logged. Developers should still always check whether attachments and joined objects still exist in their templates. Thanks to Raphaël DiRago.

* Notifications within modals move to lower right corner of modal for readability.

* Cleaned up font paths.

* Accommodations for the latest release of the separately published geoportal-workflow module.
...
* Conflict resolution has been added to Geoportal. When two users attempt to edit the same document, whether "in context" on the page or via a dialog box, Geoportal now makes the latecomer aware of the issue and gives them the option to take control of the document after warning that the first party could lose work.

Since the first user may have simply abandoned their work, Geoportal also indicates how long it has been since the first user last made a change.
...
* In a related change, Geoportal does not begin attempting to save an area on the page until the user interacts with it for the first time. This fixes many commonly reported frustrating situations in which one user is editing and the other is logged in but merely looking at the page, creating a ping-pong exchange of save requests.

* Geoportal's unit tests have been restructured so that a single test file can be run conveniently, via `mocha test/docs.js`, for instance, and there is no longer a need for us to update `test/test.js` every time a test is added. Also, the unit tests use the same `geop.tasks.getReq` and `geop.tasks.getAnonReq` methods that are used by real-life command line tasks, which provide a more faithful simulation of an Express request object and one we anticipate extending as needed.
...
* The login page can be disabled via the new `localLogin` option of the `geoportal-login` module. Set it explicitly to `false` to disable the login URL completely.
* Refactoring: the `geoportal-login` module now has an `afterLogin` method which takes care of invoking the `loginAfterLogin` callAll method on all modules that have one, and then redirecting appropriately. This code was factored out to make it easier to use in the new [geoportal-passport](https://npmjs.org/package/geoportal-passport) module, which allows the use of almost any [Passport](http://passportjs.org)-based strategy, such as Facebook login, Google login, Github login, etc.
* `geop.users.ensureGroup` now delivers the group to its callback as the second argument.
...
* Also added an `geoportal-caches:clear` task to aid in testing various functionality. You must specify the cache name since caches may or may not even be known to Geoportal at task startup time based on whether and when code calls `.get` for each cache name.

## 2.31.0 (2017-08-07)

All tests passing.

* The new `testModule: true` option causes Geoportal to supply much of the boilerplate for a published npm geoportal module that wants to test itself as part of an geoportal instance, i.e. geoportal-workflow, geoportal-caches-redis, etc. See those modules for examples of usage. This is a feature for those writing their own npm modules that wish to unit test by initializing Geoportal and loading the module in question.
...
* `trash` is never offered as a schema field of the `global` doc (mainly a concern with `geoportal-workflow`).
...
* The `createControls` option of the `geoportal-pages` module is now respewcted properly.
...
This is a significant update containing various accommodations required by the shortly forthcoming Geoportal 2.x version of the `geoportal-workflow` module, as well as other recent enhancements in our queue.
...
* "Next" and "previous" links can now be easily added to your `show.html` pages for pieces. Just set the `next` and `previous` options for your `geoportal-pieces-pages` subclass to `true`, or to an object with a `projection` property for best performance. This will populate `data.previous` and `data.next` in your `show.html` template. *For blogs they may seem backwards; they refer to relative position on the index page, and blogs are reverse-chronological. Just switch the terms on the front end in your template in cases where they appear confusing.*
...
* The buttons displayed for each widget in an Geoportal area can be adjusted via the `addWidgetControlGroups` option of the `geoportal-areas` module, which can be used to introduce additional buttons.

* Empty `beforeMove` and `afterMove` methods have been added to the `geoportal-pages` module for the convenience of modules using `improve` to enhance it.

* The `geoportal-doc-type-manager` module now has `getEditPermissionName` and `getAdminPermissionName` methods. These can be overridden by subclasses. For instance, all page subtypes return `edit-geoportal-page` for the former because page types can be changed.

* `geop.destroy(function() { ... })` may be called to shut down a running Geoportal instance. This does **not** delete any data. It simply releases the database connection, HTTP server port, etc. This mechanism is extensible by implementing an `geoportalDestroy` method in your own module.
...
* `geop.instancesOf(name)` returns an array of modules that extend `name` or a subclass of it. `geop.instanceOf(object, name)` returns true if the given `object` is a moog instance of `name` or a subclass of it.

* `geop.permissions.criteria` can now supply MongoDB criteria restricted to the types the user can edit when a general permission name like `edit` or `edit-doc` is asked for. *This was never a security bug because permissions for actual editing were checked when individual edits occurred. The change makes it easier to display lists of editable content of mixed types.*

* Extending the indexes of Geoportal's `aposDocs` collection is easier to achieve in modules that use `improve` to extend `geoportal-docs`.
...
* `insert` method for `geop.attachments`, mirroring the other modules better. Thanks to Arthur Agombart.

## 2.28.0 (2017-07-17)

All tests passing.

* Notifications are available, replacing the use of `alert`. This feature is primarily for Geoportal's own administrative features; you can use it when extending the editing UI. Call `geop.notify('message')` to display a simple message. You can specify several `type` options such as `error` and `info`, and you can also use `%s` wildcards. Everything is localized on the server side. [See the documentation for more information](http://geoportalcms.org/docs/modules/geoportal-notifications/browser-geoportal-notifications.html#trigger). Thanks to Michelin for their support of this work.
* The `geoportal-images` widget now provides a focal point editor. See the new [responsive images HOWTO](http://geoportalcms.org/docs/tutorials/howtos/responsive-images.html). Thanks to Michelin for their support of this work.
...
* `geop.areas.fromRichText` can be used to create an area with a single rich text widget from a trusted string of HTML. Not intended for mixed media, just rich text. Related: both `fromRichText` and `fromPlaintext` now correctly give their widgets an `_id` property.
...
* Complete implementation of the [explicitOrder](http://geoportalcms.org/docs/modules/geoportal-docs/server-geoportal-cursor.html#explicit-order) cursor filter, replacing a nonfunctional implementation.

* If the mongodb connection is lost, the default behavior is now to retry it forever, so when MongoDB does get restarted Geoportal will find it. In addition, a `connect` object may be passed to the `geoportal-db` module to be passed on to the MongoDB connect call.
...
* Geoportal now displays useful warnings and, in some cases, errors at startup when schemas are improperly configured. This is particularly useful if you have found it frustrating to configure joins correctly. We are continuing to deepen the coverage here.
...
cleanCss needs to know that the output CSS files are going to live in geop-minified in order to correctly parse `@import` statements that pull in plain .css files. Also, the mechanism for prefixing URLs in CSS code was not applied at the correct stage of the bundling process (the minify stage), which broke the ability to reference fonts, images, etc. via URLs beginning with /modules when using an S3 asset bundle.

## 2.23.0 (2017-04-24)

All tests passing.

* The "manage" view of `geoportal-pieces` now supports robust filters, in the same way they were already supported on the front end for `geoportal-pieces-pages`. Use the `addFilters` option to configure them. There is bc with existing filters that relied on the old assumption that manage filters have a boolean API. However now you can specify any field with a cursor filter, which includes most schema fields, notably including joins.
...
See [reusable content with pieces](http://geoportalcms.org/docs/tutorials/getting-started/reusable-content-with-pieces.html) for more information and examples.

Thanks to Michelin for their support of this work.

* `geop.utils.isFalse` allows you to check for values that are strictly `=== false` in templates.

* `geop.utils.startCase` converts property names to English, roughly speaking. It is used as a fallback if a filter does not have a `label` property. This is primarily for bc, you should add a `label` property to your fields.
...
* You can now pass a `less` option to `geoportal-assets`, which is merged with the options given to `less.render` both in dev and production. You can use this, for instance, to enable `strictMath`.

* `geoportal.oembed`'s `fetch` method now propagates its `options` object to `oembetter` correctly. Thanks to Fotis Paraskevopoulos.

## 2.22.0 (2017-04-11)

All tests passing.

* Geoportal now supports publishing CSS and JS assets via S3 rather than serving them directly.

Geoportal already had an option to build asset "bundles" and deploy them at startup, as described in our [cloud HOWTO](http://geoportalcms.org/docs/tutorials/howtos/deploying-geoportal-in-the-cloud.html). However this serves the assets from the cloud webserver, such as a Heroku dyno or EC2 instance. It is now possible to serve the assets from Amazon S3.

See the [updated cloud HOWTO](http://geoportalcms.org/docs/tutorials/howtos/deploying-geoportal-in-the-cloud.html) for details.
...
to the `geoportal-images-widgets` module and similar widgets. It should not be applied if you wish
to access the results of the join in asynchronous code, because they are not available until the last
possible moment.

Thanks to Michelin for their support of this work.

* You can also set `deferImageLoading` to `true` for the `geoportal-globals` module if you want the
...
When autocompleting doc titles to add them to a join, Geoportal again utilizes search result quality to display the best results first.
...
* Support for batch uploads. The `geoportal-images` and `geoportal-files` modules now implement batch uploads by default.
...
This feature can be disabled by setting the `insertViaUpload` option to `false` for `geoportal-images` or `geoportal-files`. If you are adding `required` fields to `geoportal-images` or `geoportal-files`, then batch uploading is not the best option for you because it would bypass that.

**If you wish, you can enable the feature for your own `geoportal-pieces` modules that have an `attachment` field in their schema by setting the `insertViaUpload` option to `true`.** However please note that this does not currently do anything for pieces that refer to an image or file indirectly via widget.
...
However, if you use `addFields` to add schema fields to the `geoportal-global` module, this button instead launches an editing modal where you can edit those fields, and also offers a "Versions" button accessible from there.
...
All of these changes correct bugs in intended behavior. Certain checks were present in the code but not completely functional. If you need to update your configuration to add file extensions, [geoportal-attachments](http://geoportalcms.org/docs/modules/geoportal-attachments/).

## 2.19.0 (2017-03-15)

All tests passing.

* As always, Geoportal always populates `req.data.home`; when `req.data.page._ancestors[0]` exists that is used, otherwise Geoportal carries out a separate query. However as a performance enhancement, you may now disable this additional query by passing the `home: false` option to the `geoportal-pages` module. Note that `req.data.home` is not guaranteed to exist if you do this.

As for children of the home page, for performance you may now pass `home: { children: false }` option to the `geoportal-pages` module. This option only comes into play when using `filters: { ancestors: false }`.
...
* For better out-of-the-box SEO, an `alt` attribute with the title of the image is now part of the `img` markup of `geoportal-images` widgets.

## 2.17.1 (2017-02-21)

All tests passing.

* Fixed XSS (cross-site scripting) vulnerability in `req.browserCall` and `geop.push.browserCall`.
...
* The `oembedReady` Geoportal event is now emitted correctly after `geoportal-oembed` renders an oembed-based player, such as a YouTube video player for the `geoportal-video` widget. This event can be listened for via `geop.on('geoportal-oembed', fn)` and receives a jQuery object referring to the relevant element.
...
* Relative `@import` statements now work when you push plain `.css` files as Geoportal assets. There is no change in behavior for LESS files. Thanks to Fredrik Ekelund.
...
* In `geoportal-rich-text-widgets`, initialize CKEditor on `instanceReady`, rather than via a dodgy timeout. Thanks to Frederik Ekelund for finding a better way!
...
* Introduced the new `deleteFromTrash` option to the `geoportal-pages` module. If this option is enabled, a new icon appears in "reorganize" when looking at pages in the trash. This icon allows you to permanently delete a page and its descendants from the site.
...
* The tests for the `geoportal-locks` module are now deterministic and should always pass.
...
* `geop.permissions.addPublic` accepts multiple arguments and array arguments,
...
Fixed [#385](https://github.com/punkave/geoportal/issues/385): if a page is moved to the trash, its slug must always change, even if it has been edited so that it no longer has its parent's slug as a prefix. In addition, if the resulting slug of a descendant of the page moving to the trash conflicts with an existing page in the trash, steps are taken to ensure uniqueness.

## 2.14.1 (2017-01-11)

All tests passing.

* The `geop.utils.clonePermanent` method no longer turns objects into long arrays of nulls if they happen to have a `length` property. `lodash` uses the `length` property as an indicator that the object should be treated as an array, but this would be an unrealistic restriction on Geoportal schema field names. Instead, `clonePermanent` now uses `Array.isArray` to distinguish true arrays. This fixes a nasty bug when importing content from A1.5 and subsequently editing it.

* When a user is logged in there is an `geop.user` object on the browser side. Due to a bug this was an empty object. It now contains `title`, `_id` and `username` properties as intended.

## 2.14.0 (2017-01-06)

All tests passing.

* A version rollback dialog box for the `global` doc is now opened if an element with the `data-geop-versions-global` attribute is clicked. There is currently no such element in the standard UI but you may introduce one in your own layout if you have mission-critical content in the `global` doc that is awkward to recreate after an accidental deletion, such as a custom sitewide nav.
...
The `data-geop-ajax-context` feature had a bug which prevented ordinary anchor links from performing AJAX refreshes correctly.

## 2.13.1 (2016-12-22)

All tests passing.

The `geoportal-attachments` module now calls `geop.ui.busy` correctly on the fieldset so that the busy and completed indicators are correctly shown and hidden. Previously the string `0` was passed, which is not falsy.
...
You can now add middleware to your Geoportal site via any module in your project. Just add an `self.expressMiddleware` method to your module, which takes the usual `req, res, next` arguments. Or, if it's more convenient, set `self.expressMiddleware` to an array of such functions. "Module middleware" is added immediately after the minimum required Geoportal middleware (bodyParser, `req.data`, etc), and before any routes.

## 2.10.3 (2016-12-08)

All tests passing.

Fixed bug in `autoPreserveText` feature of our `data-geop-ajax-context` mechanism; also, restricted it to text inputs and textareas that actually have the focus so that you can replace their values normally at other times
...
* Geoportal now automatically recompiles modified Nunjucks templates. This means you can hit refresh in your browser after hitting save in your editor when working on `.html` files. Also note that this has always worked for `.less` files.
...
* Added a `beforeAjax` Geoportal event to facilitate progress display and animations when using the new `data-geop-ajax-content` feature.

## 2.9.1 (2016-12-02)

All tests passing.

* Fixed an omission that prevented the use of the back button to undo the very first click when using the new `data-geop-ajax-context`. Later clicks worked just fine, but for the first one to work we need a call to `replaceState` to make it possible to restore the original query.

## 2.9.0 (2016-12-01)

All tests passing.

* Two major new features in this release: built-in filters for most schema fields, and built-in AJAX support for `geoportal-pieces-pages`. These combine to eliminate the need for custom code in a wide array of situations where you wish to allow users to browse and filter blog posts, events, etc. In most cases there is no longer any need to write your own `cursor.js` or your own AJAX implementation. The provided AJAX implementation handles browser history operations, bookmarking and sharing properly and is SEO-friendly.

[See the official summary of the pull request for details and examples of usage.](https://github.com/punkave/geoportal/pull/766)
...
In addition, we merged a contribution from Fotis Paraskevopoulos that allows a `bodyParser` option with `json` and `urlencoded` properties to be passed to the `geoportal-express` module. Those properties are passed on to configure those two body parser middleware functions.

## 2.8.0 (2016-11-28)

All tests passing.

* `APOS_MONGODB_URI` environment variable is used to connect to MongoDB if present. Helpful for cloud hosting. See the new [deploying Geoportal in the cloud HOWTO](http://geoportalcms.org/docs/tutorials/howtos/deploying-geoportal-in-the-cloud.html).
* `APOS_S3_BUCKET`, `APOS_S3_ENDPOINT` (optional), `APOS_S3_SECRET`, `APOS_S3_KEY`, and `APOS_S3_REGION` environment variables can be used to configure Geoportal to use S3 for uploaded media storage. This behavior kicks in if `APOS_S3_BUCKET` is set. See the new [deploying Geoportal in the cloud HOWTO](http://geoportalcms.org/docs/tutorials/howtos/deploying-geoportal-in-the-cloud.html).
* New advisory locking API accessible via `geop.locks.lock` and `geop.locks.unlock`. `geoportal-migrations:migrate` is now wrapped in a lock. More locks are coming, although Geoportal was carefully designed for benign worst case outcomes during race conditions.
* Better asset deployment for Heroku and other cloud services. `node app geoportal:generation --create-bundle=NAME` now creates a new folder, `NAME`, containing assets that would otherwise have been written to `public`. Launching a server with the `APOS_BUNDLE` environment variable set to `NAME` will then copy that bundle's contents into `public` before listening for connections. See the new [deploying Geoportal in the cloud HOWTO](http://geoportalcms.org/docs/tutorials/howtos/deploying-geoportal-in-the-cloud.html).
* `geoportal-pieces-pages` index pages are about 2x faster; discovered we were inefficiently deep-cloning `req` when cloning a cursor.
* Helpful error message if you forget to set the `name` property of one of your `types` when configuring `geoportal-pages`.
...
* Geoportal's dropdown menus, etc. will more robustly maintain their font size in the presence of project-level CSS. There is an explicit default font size for `.geop-ui`.
...
* The `minSize` option to `geoportal-images` widgets now works properly when cropping.
...
* Implemented `geop.areas.fromPlaintext`, which accepts a string of plaintext (not markup) and returns an area with a single `geoportal-rich-text` widget in it, containing that text. Useful in implementing importers.
* The so-called `csv` import mode of `geop.schemas.convert` works properly for areas, using the above. Although it is called csv this mode is really suitable for any situation in which you have plaintext representations of each property in an object and would like those sanitized and converted to populate a doc.
* Bug fix: emit the `enhance` Geoportal event only once on page load. This event is emitted only when there is new content that has been added to the page, e.g. once at page load, and also when a new widget is added or updated, etc. The first argument to your event handler will be a jQuery element which will contain only new elements.
...
Workarounds for two limitations in MongoDB that impact the use of Geoportal cursors:
...
* All of the autocomplete suggestions fit on the screen on reasonably sized displays. With the recent addition of the "flip" feature to push the suggestions up rather than down if the bottom of the screen would otherwise be reached, this is critical to show the first and best suggestion. Further discussion for future UX improvement in [issue 704](https://github.com/punkave/geoportal/issues/704).
...
* The `login` module now has an alias, `geop.login`, like other core modules.
...
Fix for [#668](https://github.com/punkave/geoportal/issues/668), crash occurring when admin bar group leader starts out too close to the end of the admin bar items array.

## 2.1.1 (2016-09-18)

Full Windows compatibility restored. The "recursively copy asset folders if on Windows" behavior from 0.5 was reimplemented. This is necessary to allow Geoportal to run as a non-administrator on Windows. Running as administrator is the moral equivalent of running as root on Linux, which we would never recommend.

Since Geoportal did not function previously on Windows and there is no behavior change on Mac/Linux this is effectively a bug fix rather than a new feature, thus 2.1.1.

## 2.1.0 (2016-09-16)

* Introduced the new `geop.areas.richText` and `geop.areas.plaintext` methods, which are also available in templates by the same names.

* Added and documented the `addImageSizes` option of the `geoportal-attachments` module.

## 2.0.4 (2016-09-14)

* The `geoportal-login` module now invokes `loginAfterLogin(req, callback)` on all modules that have such a method, via `geop.callAll`. Modules that do not need a callback can supply this method with only one argument. Afterwards, `geoportal-login` redirects to `req.redirect`, as is supported elsewhere in Geoportal. So you can assign to `req.redirect` in your callback to change the user's destination after a successful login. If `req.redirect` is not set, the user is redirected to the home page.

## 2.0.3 (2016-09-13)

The `ancestors` and `children` filters defaulted to `areas(false)`, but `joins(false)` was omitted, contrary to documentation which has always indicated the information returned is limited for performance. This was fixed. You can still override freely with the `filters` option to `geoportal-pages`.
...
Inaugural npm release of Geoportal 2.x, which was used prior to that in many projects via git dependencies.
