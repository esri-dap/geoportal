geop.define('geoportal-doc-type-manager-chooser', {
  ...
  construct: function(self, options) {
    ...
    self.refresh = function(options) {
      ...
      return self.api('chooser-choices', { choices: self.choices, field: self.field, validate: true }, function(result) {
        ...
        return async.eachSeries(self.choices || [], function(choice, callback) {
          ...
          return geop.schemas.populate(self.$choices.find('[data-relationship-inline="' + choice.value + '"]'), self.inlineSchema, choice, callback);
        }, function(err) {
          if (err) {
            // Ouch
            geop.notify('Unable to populate form correctly, try again later', { type: 'error', dismiss: true });
            self.decrementRefreshing();
            return;
          }
    ...
    self.convertInlineRelationships = function(callback) {
      return async.eachSeries(self.choices, function(choice, callback) {
        ...
        return geop.schemas.convert($relationship, self.inlineSchema, choice, callback);
      }, callback);
    };
    ...
    self.enableLinks = function() {
      self.$el.on('mouseover', '[data-geop-tooltip]', geop.ui.createTooltip);
      self.$el.on('mouseleave click', '[data-geop-tooltip]', geop.ui.removeTooltip);
      self.link('geop-delete', 'item', function($button, _id) {
        var index = _.findIndex(self.choices, { value: _id });
        if (index === -1) {
          return;
        }
        self.remove(_id, false);
        var $choices = self.$el.find('[data-choices]>.geop-chooser-choice');
      ...
      self.link('geop-raise', 'item', function($button, _id) {
        var index = _.findIndex(self.choices, { value: _id });
        if (index === -1) {
          return;
        }
      ...
      self.link('geop-lower', 'item', function($button, _id) {
        var index = _.findIndex(self.choices, { value: _id });
        if (index === -1) {
          return;
        }
      ...
      self.link('geop-relate', 'item', function($button, _id) {
      ...
      self.link('geop-edit', 'item', function($button, _id) {
        self.manager.edit(_id, function() {});
      });
      self.link('geop-browse', function() {
        self.launchBrowser();
      });
    };
    ...
    self.enableSortable = function () {
      var sortableOptions = {
        cancel: false,
        handle: '[data-geop-sort-item]',
        update: function(event, ui) {
          var $choices = self.$el.find('[data-chooser-choice]');

          if ($choices.length !== self.choices.length) {
            geop.notify('Something went wrong, reverting changes', { type: 'error' });
    ...
    self.clone = function(options, callback) {
      ...
      return geop.create(self.__meta.name, _options, function(err, chooser) {
        if (err) {
          return callback(err);
        }
    ...
    self.onChange = function() {
      if ((self.limit > 1) && self.choices.length >= self.limit) {
        self.$el.addClass('geop-chooser-full');
        if (self.options.managerModal) {
          self.options.managerModal.$el.addClass('geop-chooser-full');
        }
        self.$autocomplete.prop('disabled', true);
        self.full = true;
      } else {
        self.$el.removeClass('geop-chooser-full');
        self.$autocomplete.prop('disabled', false);
        if (self.options.managerModal) {
          self.options.managerModal.$el.removeClass('geop-chooser-full');
        }
        self.full = false;
      }
    ...
    // Adds and overrides methods of the geoportal-pieces-manager-modal to
      ...
      manager.enableChooser = function(callback) {
        ...
        return manager.parentChooser.clone(
          ...
          function(err, chooser) {
            ...
            geop.on('pieceInserted', chooser.pieceInsertedListener);
            manager.afterEnableChooser();
            return callback(null);
          }
        );
    ...
    self.afterManagerSave = function() {
      geop.off('pieceInserted', self.pieceInsertedListener);
    };
