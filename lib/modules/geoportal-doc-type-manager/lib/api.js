...
module.exports = function(self, options) {
  ...
  //
  // For suffixes, geoportal-pages will take care of adding and removing
  ...
  self.deduplicateTrash = function(req, doc, callback) {
    ...
    return self.geop.docs.db.update({ _id: doc._id }, { $set: $set }, callback);
  };
  ...
  self.deduplicateRescue = function(req, doc, callback) {
    ...
    function update(callback) {
      ...
      return self.geop.docs.db.update({ _id: doc._id }, action, callback);
    }
    function checkOne(name, callback) {
      ...
      return self.geop.docs.db.findOne(query, { _id: 1 }, function(err, found) {
        if (err) {
          return callback(err);
        }
        if (found) {
  ...
  self.newInstance = function() {
    var doc = self.geop.schemas.newInstance(self.schema);
    doc.type = self.name;
    return doc;
  };
  ...
  // Used by `geoportal-versions` to label changes that
  ...
  self.allowedSchema = function(req) {
    ...
    if (typeIndex !== -1) {
      ...
      // geoportal-option-overrides and geoportal-workflow modules,
      // if present, can be used together to disable various types based
      // on locale settings
      disabled = self.geop.pages.getOption(req, 'disabledTypes');
      if (disabled) {
  ...
  self.composeSchema = function() {
    ...
    if (self.isAdminOnly()) {
      options.removeFields = (options.removeFields || []).concat(self.geop.schemas.getPermissionsFieldNames());
    }

    self.schema = self.geop.schemas.compose(self.options, self);
    ...
    if (slug) {
      ...
      if (self.geop.docs.options.deconflictSlugs === false) {
  ...
  self.autocomplete = function(req, query, callback) {
    ...
    if (query.term && query.term.toString && query.term.toString().length) {
      var term = self.geop.launder.string(query.term);
    ...
    cursor.toArray(function(err, docs) {
      ...
      if (query.values) {
        docs = self.geop.utils.orderById(query.values, docs);
      }
  ...
  // Adds a listener for the `docSearchIndex` Geoportal event, pointing to the
  // `searchIndexListener` method.

  self.addSearchIndexListener = function() {
    self.geop.on('docSearchIndex', self.searchIndexListener);
  };
  ...
  self.searchIndexListener = function(doc, texts) {
    ...
    self.geop.schemas.indexFields(self.schema, doc, texts);
  };
  ...
  // `geop.utils.sortify` so that it sorts and compares in a more
  ...
  self.addSortifyMigration = function(field) {
    ...
    return self.geop.migrations.addSortify(self.__meta.name, { type: self.name }, field);
  };
  ...
  self.addSlugPrefixMigration = function() {
    ...
    const seen = {};
    self.geop.migrations.add(self.__meta.name + 'SlugPrefix', function() {
      return self.geop.migrations.eachDoc({
        type: self.name,
        slug: {
          $not: new RegExp('^' + self.geop.utils.regExpQuote(self.options.slugPrefix))
        }
      }, 5, function(doc) {
  ...
  self.getBatchPermissionsSchema = function(req) {
    return self.geop.schemas.subset(self.allowedSchema(req), self.geop.schemas.getPermissionsFieldNames());
  };
