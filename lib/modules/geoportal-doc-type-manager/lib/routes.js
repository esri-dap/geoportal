...
module.exports = function(self, options) {
  ...
  self.route('post', 'chooser-choices', function(req, res) {
    ...
    return async.series({
      convert: function(callback) {
        ...
        return self.geop.schemas.convert(req, [ _.omit(field, 'readOnly') ], 'form', input, receptacle, callback);
      },
      join: function(callback) {
        return self.geop.schemas.join(req, [ field ], receptacle, true, callback);
      }
    }, function(err) {
      if (err) {
  ...
  self.renderRoute('post', 'relationship-editor', function(req, res, next) {
    var field = req.body.field;
    if (!self.geop.utils.isBlessed(req, field, 'join')) {
  ...
  self.apiRoute('post', 'autocomplete', function(req, res, next) {
    ...
    var term = self.geop.launder.string(req.body.term);
