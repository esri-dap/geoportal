// The `geoportal-users` module provides user accounts. It is **not** intended to
// be extended with new subclass modules. The `geoportal-login` module only
// looks for instances of `geoportal-user`. Of course you may implicitly subclass
...
// join with `geoportal-group` in the schema.
//
// Groups are managed by the `geoportal-groups` module.
//
// There is also a simplified permissions model in which you just specify
// an array of `groups` as an option to `geoportal-users`, and a single-select
...
// `geoportal-users`, the admin interface for `geoportal-groups` will hide itself.
//
...
// `geoportal-users`. You can add it back in, but then you have to deal with
...
// or similar, extending `geoportal-pieces`, unless it's true that basically
...
//
// You may also call `geop.users.addSecret('name')` to add a new
// secret property. This is convenient when implementing a module
// such as `geoportal-signup`.
    ...
    options.arrangeFields = [
      {
        name: 'basics',
        label: 'Basics',
        fields: [ 'firstName', 'lastName', 'title', 'username', 'email', 'password', 'slug', 'group', '_groups', 'disabled', 'slug' ].concat(options.geop.docs.trashInSchema ? [ 'trash' ] : [])
      }
  ...
  construct: function(self, options) {
    ...
    self.ensureSafe = function(callback) {
      return self.ensureSafeCollection(function(err) {
        ...
        self.on('geoportal:migrate', 'ensureSafeIndexesPromisified', function() {
    ...
    self.ensureSafeCollection = function(callback) {
      return self.geop.db.collection('aposUsersSafe', function(err, collection) {
    ...
    // an `geoportal-user` and, if so, hash the password, remove it from the doc
    ...
    // First checks to be sure this is an `geoportal-user` and invokes
    ...
    // hash in the safe. `user` is an `geoportal-user` doc.
    ...
    self.ensureGroup = function(group, callback) {
      ...
      return self.geop.groups.find(req, criteria).toObject(function(err, _group) {
        if (err) {
          return callback(err);
        }
        if (_group) {
          ...
          return self.geop.groups.update(req, _group, function(err) {
            if (err) {
              return callback(err);
            }
        ...
        return self.geop.groups.insert(req, group, function(err) {
          if (err) {
            return callback(err);
          }
    ...
    self.geop.tasks.add('geoportal-users', 'add',
      'Usage: node app geoportal-users:add username groupname\n\n' +
    ...
    // Implement the `geoportal-users:add` command line task.
    ...
    self.addFromTask = function(callback) {
      ...
      var req = self.geop.tasks.getReq();
      // find the group
      self.geop.groups.find(req, {title: groupname}).toObject(function(err, group) {
        if (err) {
          return callback(err);
        }
        if (!group) {
    ...
    self.geop.tasks.add('geoportal-users', 'change-password',
      'Usage: node app geoportal-users:change-password username\n\n' +
    ...
    // Implement the `geoportal-users:change-password` task.
    ...
    self.changePasswordFromTask = function(callback) {
      ...
      function findUser(callback) {
        return self.geop.users.find(req, { username: username }).toObject(function(err, _user) {
          if (err) {
            return callback(err);
          }
          if (!_user) {
    ...
    self.apiRoute('post', 'unique-username', function(req, res, next) {
      var username = self.geop.launder.string(req.body.username);
    ...
    self.addNonNullJoinMigration = function() {
      self.geop.migrations.add(self.__meta.name + ':non-null-joins', function() {
        var fields = {
          'viewUsersIds': [],
          'viewGroupsIds': [],
          'editUsersIds': [],
        ...
        return Promise.mapSeries(_.keys(fields), function(name) {
          ...
          return self.geop.docs.db.update(criteria, {
            $set: $set
          }, {
            multi: true
          });
