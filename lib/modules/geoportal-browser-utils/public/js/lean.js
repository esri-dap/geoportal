...
(function() {
  ...
  window.geop = window.geop || {};
  var apos = window.geop;

  geop.utils = geop.utils || {};

  // Make a POST JSON call to the given URI, which is expected to be on the
  // Geoportal site. CSRF headers are correctly sent and the URL is modified
  ...
  // that respect Geoportal's CSRF token and prefix.
  //
  ...
  geop.utils.post = function(uri, data, callback) {
    if (geop.prefix) {
      uri = geop.prefix + uri;
    }
    var formData = window.FormData && (data instanceof window.FormData);
    var xmlhttp = new XMLHttpRequest();
    var csrfToken = geop.csrfCookieName ? geop.utils.getCookie(geop.csrfCookieName) : 'csrf-fallback';
  ...
  // Like `geop.utils.post` but uses a GET request, with the properties of `data`
  ...
  geop.utils.get = function(uri, data, callback) {
    if (geop.prefix) {
      uri = geop.prefix + uri;
    }
    uri += '?';
  ...
  geop.utils.getCookie = function(name) {
    var match = document.cookie.match(new RegExp('(^| )' + name + '=([^;]+)'));
    return match && match[2];
  };

  // Implementation detail of `geop.utils.post` and `geop.utils.get`
  ...
  geop.utils.removeClass = function(el, className) {
    if (el.classList) {
  ...
  geop.utils.addClass = function(el, className) {
    if (el.classList) {
  ...
  geop.utils.closest = function(el, selector) {
    if (el.closest) {
  ...
  geop.utils.assign = function(obj1, obj2 /*,  obj3... */) {
    if (Object.assign) {
  ...
  // window.geop.utils.widgetPlayers['widget-name'] = function(el, data, options) {}
  //
  // Use the widget's name, like "geoportal-images", NOT the name of its module.
  //
  ...
  //
  // To register a widget player for the `geoportal-images` widget, write:
  //
  // `geop.utils.widgetPlayers['geoportal-images'] = function(el, data, options) { ... }`
  //
  // `el` is a DOM element, not a jQuery object. Otherwise identical to
  // traditional Geoportal widget players. `data` contains the properties
  ...
  geop.utils.runPlayers = function(el) {
    var widgets = (el || document).querySelectorAll('[data-geop-widget]');
    var i;
    if (el && el.getAttribute('data-geop-widget')) {
    ...
    function play(widget) {
      if (widget.getAttribute('data-geop-played')) {
        return;
      }
      var data = JSON.parse(widget.getAttribute('data'));
      var options = JSON.parse(widget.getAttribute('data-options'));
      widget.setAttribute('data-geop-played', '1');
      // bc with the old lean module
      var player = geop.utils.widgetPlayers[data.type] || (geop.lean && geop.lean.widgetPlayers && geop.lean.widgetPlayers[data.type]);
      if (!player) {
        return;
      }
  ...
  // You can run it again with geop.utils.runPlayers() if you AJAX-load some widgets.

  geop.utils.onReady(function() {
    // Indirection so you can override `geop.utils.runPlayers` first if you want to for some reason
    geop.utils.runPlayers();
  });

  // In the event (cough) that we're in the full-blown Geoportal editing world,
  // we also need to run widget players when content is edited
  if (geop.on) {
    geop.on('enhance', function($el) {
      geop.utils.runPlayers($el[0]);
    });
  }
