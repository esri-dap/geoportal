geop.define('geoportal-attachments', {
  ...
  construct: function(self, options) {
    ...
    self.crop = function(attachment, options, callback) {
      ...
      geop.create('geoportal-attachments-crop-editor', args);
    };
    ...
    self.focalPoint = function(attachment, options, callback) {
      ...
      _.assign(args, options);
      geop.create('geoportal-attachments-focal-point-editor', args);
    };

    self.addFieldType = function() {
      geop.schemas.addFieldType({
        name: self.name,
    ...
    self.populate = function(object, name, $field, $el, field, callback) {
      ...
      $input.fileupload({
        ...
        start: function (e) {
          busy(true);
          geop.emit('attachmentUploadStarted', {
            $fieldset: $fieldset,
            field: field
          });
        },
        ...
        always: function (e, data) {
          ...
          if (data.result.status !== 'ok') {
            geop.notify(data.result.status, { type: 'error' });
            geop.emit('attachmentUploadError', {
              $fieldset: $fieldset,
              field: field,
          ...
          if (groups) {
            if (!_.contains(groups, file.group)) {
              ...
              geop.notify("The file must be of one of the following types: %s", extensions.join(', '), { type: 'error' });
              return;
            }
          }
          ...
          geop.emit('attachmentUploadSuccess', {
            $fieldset: $fieldset,
            field: field
          });
    ...
    self.addHandlers = function() {
      geop.ui.link('geop-crop', 'attachment', function($el, _id) {
      ...
      geop.ui.link('geop-focal-point', 'attachment', function($el, _id) {
        var $existing = $el.closest('[data-existing]');
    ...
    self.updateExisting = function($fieldset, info, field) {
      ...
      $existing.alterClass('geop-extension-*', 'geop-extension-' + info.extension);
      var $preview = $existing.find('[data-preview]');
      if (info.group === 'images') {
        $preview.attr('src', self.url(info, { size: 'one-half', crop: info.crop }));
        $fieldset.find('[data-geop-crop-attachment]').show();
        $fieldset.find('[data-geop-focal-point-attachment]').show();
      } else {
        $preview.hide();
        $fieldset.find('[data-geop-crop-attachment]').hide();
        $fieldset.find('[data-geop-focal-point-attachment]').hide();
      }
      $existing.show();
      geop.emit('attachmentUpdated', {
        $fieldset: $fieldset,
        attachment: info,
        field: field
      });
