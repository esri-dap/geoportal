...
// [attachment](https://docs.geoportalcms.org/geoportal/tutorials/getting-started/schema-guide#attachment) schema field type,
// which makes it straightforward to allow users to attach uploaded files to docs. See
// the [schema guide](https://docs.geoportalcms.org/geoportal/tutorials/getting-started/schema-guide#attachment) for
// more information.
//
// ## Options
//
// ### `addImageSizes`
//
// Add an array of image sizes, in addition to Geoportal's standard sizes. For example:
//
// ```javascript
// [
//   {
...
// aspect ratio.
//
// These extra sizes are then available as the `size` option to `geoportal-images` widgets
// and when calling `geop.attachments.url`.
//
// ### `imageSizes`
//
// Like `addImageSizes`, but Geoportal's standard sizes are completely replaced. Bear in mind
// that certain sizes are used by Geoportal's editing interface unless overridden. We recommend
// using `addImageSizes`.
//
// ### `fileGroups`
//
// Geoportal will reject files that do not have extensions configured via `fileGroups`.
...
// ## `svgImages`
//
// If set to `true`, SVGs are permitted to be uploaded as "images" in Geoportal. This
// means they may appear in any widget that uses images, such as the `geoportal-images`
...
// To help you account for this, the CSS class `geop-slideshow-item--svg` is added
...
//
// If you have overridden `widget.html` for `geoportal-images-widgets`, view recent commits
...
module.exports = {
  ...
  singletonWarningIfNot: 'geoportal-attachments',
  ...
  afterConstruct: function(self, callback) {
    return async.series({
      ...
      self.on('geoportal:migrate', 'ensureIndexesPromisified', function() {
      ...
      self.addPermissions();
      self.geop.tasks.add('geoportal-attachments', 'rescale',
        'Usage: node app geoportal-attachments:rescale\n\n' +
      ...
      self.geop.tasks.add('geoportal-attachments', 'migrate-to-disabled-file-key',
        'Usage: node app geoportal-attachments:migrate-to-disabled-file-key\n\n' +
      ...
      self.geop.tasks.add('geoportal-attachments', 'migrate-from-disabled-file-key',
        'Usage: node app geoportal-attachments:migrate-from-disabled-file-key\n\n' +
      ...
      self.geop.tasks.add('geoportal-attachments', 'urls',
        'Usage: node app geoportal_attachments:urls --uploadfs-path\n\n' +
  ...
  construct: function(self, options) {
    ...
    var uploadfsDefaultSettings = {
      backend: 'local',
      uploadsPath: self.geop.rootDir + '/public/uploads',
      uploadsUrl: (self.geop.baseUrl || '') + self.geop.prefix + '/uploads',
      tempPath: self.geop.rootDir + '/data/temp/uploadfs',
      // Register Geoportal's standard image sizes. Notice you could
    ...
    self.geoportalDestroy = function(callback) {
      if (!self.uploadfs.destroy) {
        self.geop.utils.warn('uploadfs dependency is old, cannot clean up resources i.e. timers that will prevent exit');
        return callback(null);
      }
    ...
    self.enableCollection = function(callback) {
      return self.geop.db.collection('aposAttachments', function(err, collection) {
        self.db = collection;
        return callback(err);
      });
    };
