...
module.exports = function(self, options) {
  ...
  // Insert a file as an Geoportal attachment. The `file` object
  ...
  self.insert = function(req, file, options, callback) {
    ...
    function body(callback) {
      ...
      var info = {
        _id: self.geop.utils.generateId(),
        group: group.name,
        createdAt: new Date(),
        name: self.geop.utils.slugify(path.basename(file.name, path.extname(file.name))),
        title: self.geop.utils.sortify(path.basename(file.name, path.extname(file.name))),
        extension: extension,
        type: 'attachment',
        docIds: [],
        trashDocIds: []
      ...
      function permissions(callback) {
        ...
        return callback(self.geop.permissions.can(req, 'edit-attachment') ? null : 'forbidden');
      }

      function length(callback) {
        return self.geop.utils.fileLength(file.path, function(err, size) {
          if (err) {
            return callback(err);
          }
          info.length = size;
      ...
      function md5(callback) {
        return self.geop.utils.md5File(file.path, function(err, md5) {
          if (err) {
            return callback(err);
          }
          info.md5 = md5;
          ...
          return self.uploadfs.copyImageIn(file.path, targetPath, function(err, result) {
            ...
            if (info.extension === 'jpg') {
              return exif.parse(file.path, (err, exifData) => {
                if (err) {
                  self.geop.utils.error('jpeg-exif error:', err);
                  return callback(null);
                }
      ...
      function remember(callback) {
        if ((!options) || (options.permissions !== false)) {
          info.ownerId = self.geop.permissions.getEffectiveUserId(req);
        }
  ...
  self.crop = function(req, _id, crop, callback) {
    ...
    return async.series([
      ...
      var tempFile = self.uploadfs.getTempPath() + '/' + self.geop.utils.generateId() + '.' + info.extension;
  ...
  self.sanitizeCrop = function(crop) {
    ...
    crop.top = self.geop.launder.integer(crop.top, 0, 0, 10000);
    crop.left = self.geop.launder.integer(crop.left, 0, 0, 10000);
    crop.width = self.geop.launder.integer(crop.width, 1, 1, 10000);
    crop.height = self.geop.launder.integer(crop.height, 1, 1, 10000);
  ...
  self.clone = function(req, source, callback) {
    ...
    var tempFile = self.uploadfs.getTempPath() + '/' + self.geop.utils.generateId() + '.' + source.extension;
  ...
  self.getMissingAttachmentUrl = function() {
    var defaultIconUrl = '/modules/geoportal-attachments/img/missing-icon.svg';
    self.geop.utils.warn('Template warning: Impossible to retrieve the attachment url since it is missing, a default icon has been set. Please fix this ASAP!');
    return defaultIconUrl;
  };

  // This method is available as a template helper: geop.attachments.url
  //
  ...
  // This method is available as a template helper: geop.attachments.first
  //
  ...
  //
  // geop.attachments.first(page.body)
  //
  // 2. Must be a PDF
  //
  // geop.attachments.first(page.body, { extension: 'pdf' })
  //
  // 3. May be any office-oriented file type
  //
  // geop.attachments.first(page.body, { group: 'office' })
  //
  // geop.images.first is a convenience wrapper for fetching only images.
  //
  // OPTIONS:
  //
  ...
  // This method is available as a template helper: geop.attachments.all
  //
  ...
  // 1. In the body please
  //
  // geop.attachments.all(page.body)
  //
  // 2. Must be a PDF
  //
  // geop.attachments.all(page.body, { extension: 'pdf' })
  //
  // 3. May be any office-oriented file type
  //
  // geop.attachments.all(page.body, { group: 'office' })
  //
  // geop.images.all is a convenience wrapper for fetching only images.
  //
  // OPTIONS:
  //
  // You may specify `extension`, `extensions` (an array of extensions)
  ...
  self.all = function(within, options) {
    ...
    self.geop.docs.walk(within, function(o, key, value, dotPath, ancestors) {
      if (test(value)) {
        ...
        // that cropping coordinates stored in an geoportal-images widget
        // are passed through when we make a simple call to
        // geop.attachments.url with the returned object
        var i;
  ...
  self.each = function(criteria, limit, each, callback) {
    ...
    // tasks like geoportal-attachments:rescale. We need a robust solution that
  ...
  self.middleware = {
    canUpload: function(req, res, next) {
      if (!self.geop.permissions.can(req, 'edit-attachment')) {
        res.statusCode = 403;
  ...
  self.addTypeMigration = function() {
    ...
    self.geop.migrations.add(self.__meta.name + '.addType', function(callback) {
      ...
      function docs(callback) {
        ...
        return self.geop.migrations.eachDoc({}, function(doc, callback) {
          var changed = false;
          self.geop.docs.walk(doc, function(o, key, value) {
          ...
          self.geop.docs.db.update({ _id: doc._id }, doc, callback);
        }, callback);
      }
    }, {
      safe: true
  ...
  self.addDocReferencesMigration = function() {
    ...
    self.geop.migrations.add(self.__meta.name + '.docReferencesContained', function(callback) {
  ...
  // `geoportal-attachments:recompute-doc-references`
  ...
  self.recomputeAllDocReferences = function(callback) {
    ...
    function docs(callback) {
      return self.geop.migrations.eachDoc({}, 5, addAttachmentUpdates, callback);
    }
  ...
  self.addFixPermissionsMigration = function() {
    self.geop.migrations.add(self.__meta.name + '.fixPermissions', function(callback) {
      return self.geop.migrations.each(self.db, { group: 'images', trash: true }, 5, function(attachment, callback) {
        return self.applyPermissions(attachment, true, callback);
      }, function(err) {
        if (err) {
          return callback(err);
        }
        self.geop.utils.warn('^^^ WARNINGS ABOVE ARE OK. This migration fixes a scenario in which SOME permissions were not set.');
        return callback(null);
      });
    }, {
      safe: true
  ...
  self.resetUploadfsPermissions = function(callback) {
    return self.geop.migrations.each(self.db, {}, 5, function(attachment, callback) {
      return self.applyPermissions(attachment, attachment.trash, callback);
    }, function(err) {
      if (err) {
        return callback(err);
      }
      self.geop.utils.warn('^^^ WARNINGS ABOVE ARE OK. You may see some in cases where the permissions are\nalready correct and cannot be set the same way twice in a row.');
      return callback(null);
    });
  };
  ...
  self.updateDocReferences = function(doc, callback) {
    ...
    doc = self.geop.utils.clonePermanent(doc);

    var attachments = self.all(self.geop.utils.clonePermanent(doc));
      ...
      return async.eachSeries(sizes, function(size, callback) {
        ...
        return method(path, function(err) {
          if (err) {
            ...
            self.geop.utils.warn('Unable to set permissions on ' + path + ', most likely already done');
          }
          return callback(null);
        });
      }, callback);
    ...
    function cropOne(crop, callback) {
      return async.eachSeries(self.imageSizes.concat([ { name: 'original' } ]), function(size, callback) {
        ...
        return method(path, function(err) {
          if (err) {
            // afterSave is not a good place for fatal errors
            self.geop.utils.warn('Unable to set permissions on ' + path + ', possibly it does not exist');
          }
          return callback(null);
        });
      }, callback);
  ...
  self.urlsTask = function(callback) {
    var uploadfsPath = !!self.geop.argv['uploadfs-path'] || !!self.geop.argv['uploads-url'];
    var seen = {};
    return self.geop.migrations.each(self.db, {}, 1, function(attachment, callback) {
      url(attachment);
      _.each(self.imageSizes, function(size) {
    ...
    function url(attachment, options) {
      ...
      if (self.geop.argv['uploads-url']) {
        url = self.geop.argv['uploads-url'] + url;
      }
      if (seen[url]) {
        return;
      }
