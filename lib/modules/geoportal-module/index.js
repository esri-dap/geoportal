...
// an implementation provided in another module, such as `geoportal-assets`,
...
module.exports = {
  ...
  beforeConstruct: function(self, options) {
    ...
    self.geop = options.geop;
    // all geoportal modules are properties of self.geop.modules.
    // Those with an alias are also properties of self.geop
    self.geop.modules[self.__meta.name] = self;
    if (self.options.alias) {
      if (_.has(self.geop, self.options.alias)) {
        throw new Error('The module ' + self.__meta.name + ' has an alias, ' + self.options.alias + ', that conflicts with a module registered earlier or a core Geoportal feature.');
      }
      self.geop[self.options.alias] = self;
    }
  },
  ...
  construct: function(self, options) {
    ...
    if (self.geop.assets) {
      if (!self.geop.assets.chains) {
        self.geop.assets.chains = {};
      }
      _.each(self.__meta.chain, function(meta, i) {
        self.geop.assets.chains[meta.name] = self.__meta.chain.slice(0, i + 1);
      });
    }
    ...
    // Add an Express route to geop.app. The `path` argument is
    ...
    //
    // `self.geop.app.post(self.action + '/edit-monkey', function(req, res) { ... })`
    //
    ...
    // `self.geop.middleware.files` for file uploads.
    //
    ...
    self.route = function(method, path, responder, fn) {
      var key = method + '.' + path;
      if (!routes[key]) {
        self.geop.app[method](self.action + '/' + path, function(req, res) {
          var args = routes[key];
    ...
    self.addHelpers = function(object /* or name, value */) {
      if (typeof (object) === 'string') {
        self.geop.templates.addHelpersForModule(self, arguments[0], arguments[1]);
      } else {
        self.geop.templates.addHelpersForModule(self, object);
      }
    };
    ...
    // only exceptions in geoportal core are `geop.area` and `geop.singleton`.
    //
    // The helper must be added first with `addHelpers`.

    self.addHelperShortcut = function(name) {
      self.geop.templates.addHelperShortcutForModule(self, name);
    };
    ...
    self.pushAsset = function(type, name, options) {
      ...
      _.each(self.__meta.chain, function(typeMeta) {
        if (self.geop.assets.push(type, name, options, typeMeta)) {
          exists = true;
        }
      });
      if (!exists) {
        self.geop.utils.error('WARNING: no versions of the ' + type + ' ' + name + ' exist, but you are pushing that asset in the ' + self.__meta.name + ' module.');
      }
    };
    ...
    self.render = function(req, name, data) {
      ...
      return self.geop.templates.renderForModule(req, name, data, self);
    };
    ...
    self.partial = function(name, data) {
      ...
      return self.geop.templates.partialForModule(name, data, self);
    };
    ...
    self.renderString = function(req, s, data) {
      ...
      return self.geop.templates.renderStringForModule(req, s, data, self);
    };
    ...
    self.partialString = function(req, s, data) {
      ...
      return self.geop.templates.partialStringForModule(s, data, self);
    };
    ...
    //
    // `geoportal-templates:outerLayout.html`
    //
    // Or:
    //
    // `geoportal-templates:refreshLayout.html`
    //
    ...
    // `req.query.geop_refresh` is true
    //
    ...
    self.renderPage = function(req, template, data) {
      return self.geop.templates.renderPageForModule(req, template, data, self);
    };
    ...
    self.sendPage = function(req, template, data) {
      return async.series([
        ...
        return req.res.send(
          self.geop.templates.renderPageForModule(req, template, data, self)
        );
      });
      ...
      function loadDeferredWidgets(callback) {
        return self.geop.areas.loadDeferredWidgets(req, callback);
      }
    ...
    // otherwise it is still available via `geop.modules[full-module-name]`.
    //
    ...
    // even if an `geop.change` event would otherwise cause it to be created again.
    //
    ...
    self.pushCreateSingleton = function(req, when) {
      ...
      if (req) {
        req.browserCall('geop.createModule(?, ?, ?)', self.__meta.name, options, self.options.alias);
      } else {
        self.geop.push.browserCall(when, 'geop.createModule(?, ?, ?)', self.__meta.name, options, self.options.alias);
      }
    };
    ...
    // Geoportal uses this method to define database cursor types related to modules.
    ...
    // point. For instance, if `geoportal-images` has already called this
    // as a subclass of `geoportal-pieces`, then `geoportal-files` will
    // just define its own cursor, extending `geoportal-pieces-cursor`, and stop.
    // This prevents duplicate definitions when many types extend `geoportal-pieces`.

    // If `options.stop` is set to the name of an ancestor module,
    // the chain stops **after** defining the related type for that module.

    // For instance, the module `geoportal-files` extends
    // `geoportal-pieces`, which extends `geoportal-doc-type-manager`.

    // So when that module calls:

    // ```
    // self.defineRelatedType('cursor', {
    //   stop: 'geoportal-doc-type-manager'
    // });
    // ```

    // We get:

    // geoportal-files-cursor extends...
    // geoportal-pieces-cursor which extends...
    // geoportal-doc-type-manager-cursor which extends...
    // geoportal-cursor (because `cursor.js` for doc-type-manager says so)
    ...
    self.defineRelatedType = function(tool, options) {
      ...
      // if (self.__meta.name === 'geoportal-tags') {
      //   process.exit(1);
      // }
      function defineRelatedTypeBody(i, tool, options) {
        ...
        if (_.has(self.geop.synth.definitions, self.geop.synth.myToOriginal(name))) {
          // Already defined
          return;
        }
        ...
        if (inferred && ((extend === inferred) || (self.geop.synth.originalToMy(extend) === inferred))) {
          defineRelatedTypeBody(i - 1, tool, options);
        }
        if (self.geop.synth.isMy(name)) {
          // Restore the implicit subclassing
          name = self.geop.synth.myToOriginal(name);
        ...
        self.geop.synth.define(name, definition);

        function inferParentName() {
          if ((i > 0) && (chain[i].name !== options.stop)) {
            return self.geop.synth.myToOriginal(chain[i - 1].name + '-' + tool);
          } else {
            // A hard stop, so we don't implicitly extend
            // `geoportal-module`. Related types are not modules
            return false;
          }
        }
    ...
    // See `defineRelatedType`. A convenient wrapper for calling `geop.create`.
    //
    // For instance, if this module is `geoportal-images`, then
    // `self.createRelatedType('cursor', { options... })` will
    // create an instance of `geoportal-images-cursor`.
    //
    ...
    self.createRelatedType = function(tool, options, callback) {
      return self.geop.synth.create(self.__meta.name + '-' + tool, options, callback);
    };

    // A convenience method to fetch properties of `self.options`.
    //
    // `req` is required to provide extensibility; modules such as
    // `geoportal-workflow` and `geoportal-option-overrides`
    ...
    self.logError = function(req, err) {
      if (!(req && req.res)) {
        self.geop.utils.error('Looks like you did not pass req to self.logError, you should not have to call this method yourself,\nit is usually called for you by self.apiRoute, self.htmlRoute or self.renderRoute', (new Error()).stack);
        return;
      }
      // err.stack also includes the description of the error
      self.geop.utils.error(req.method + ' ' + req.url + ': ' + '\n\n' + err.stack);
    };
    ...
    //
    // **The `nodemailer` option of the `geoportal-email` module
    ...
    self.email = function(req, templateName, data, options, callback) {
      ...
      function body(callback) {
        return self.geop.modules['geoportal-email'].emailForModule(req, templateName, data, options, self, callback);
      }
    };

    // Add an Geoportal command line task to your module. The command line
    // syntax will be:
    //
    ...
    // `self.geop.tasks.getReq` to get a `req` with unlimited admin permissions.

    self.addTask = function(name, usage, fn) {
      return self.geop.tasks.add(self.__meta.name, name, usage, fn);
    };

    // Only safe or meaningful to add a helper in modules initialized after the
    // templates module
    if (self.geop.templates && self.geop.templates.addHelpersForModule) {
      self.addHelpers({
        ...
        'getOption': function(dotPath, def) {
          return self.getOption(self.geop.templates.contextReq, dotPath, def);
        }
      });
    }
