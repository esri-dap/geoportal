...
module.exports = function(self, options) {
  ...
  self.emit = function(name /* , arg1, arg2... */) {
    ...
    var eh = self.geop.eventHandlers[self.__meta.name] || {};
    return Promise.mapSeries(eh[name] || [], function(h) {
      var module = self.geop.modules[h.moduleName];
      var method = module[h.methodName];
      return method.apply(module, args);
    });
  };

  // For safe use inside the geoportal core, which has a legacy emit
  ...
  self.on = function(name, methodName, fn) {
    ...
    if ((typeof methodName) !== 'string') {
      throw new Error('When registering an Geoportal module event handler, the second argument must be a method name, not a function. This ensures that event handlers can be overwritten and/or extended with the "super" pattern.');
    }
    if (index === -1) {
    ...
    self.geop.eventHandlers[moduleName] = self.geop.eventHandlers[moduleName] || {};
    var eh = self.geop.eventHandlers[moduleName];
    if (fn) {
      self[methodName] = fn;
    }
  ...
  self.callAllAndEmit = function(callAllName, eventName, /* argument, ... */ callback) {
    ...
    return self.geop.callAll.apply(self.geop, args);
  };
};
