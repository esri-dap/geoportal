// `geoportal-modal` is a base class for modal dialog boxes. Define a new
// type that extends `geoportal-modal`, set the `source` option to the
...
// *Examples*
//
// The `geoportal-tags`, `geoportal-widgets` and `geoportal-pieces` modules
...
// on the stack has the `geop-modal-slideable` CSS class, in which case
// the new modal "slides in" creating a breadcrumb trail.
//
// Subclasses of `geoportal-modal` can also provide a `$view` jQuery
...
geop.define('geoportal-modal', {
  ...
  construct: function(self, options) {
    ...
    self.enableGlobalEventsOnce = function() {
      if (!geop.modalSupport.initialized) {
        geop.modalSupport.initialized = true;
        ...
        $(document).on({
          'keydown.geopModal': function(e) {
            if (e.keyCode === 27) {
              geop.modalSupport.cancelTopModal();
              return false;
            }
          },
          'click.geopModal': function(e) {
            if (e.target.className === 'geop-modal-blackout') {
              geop.modalSupport.cancelTopModal();
              return false;
            }
          }
    ...
    self.disableGlobalEvents = function() {
      ...
      geop.modalSupport.initialized = false;
      $(document).off('keydown.geopModal');
      $(document).off('click.geopModal');
    };

    // Add event handlers for the cancel and save buttons,
    // [data-geop-cancel] and [data-geop-save]. If the save
    ...
    self.enableButtonEvents = function() {
      ...
      self.$controls.on('click', '[data-geop-cancel]', function() {
        return self.cancel();
      });

      self.$controls.on('click', '[data-geop-save]', function() {
        var $button = $(this);
    ...
    self.enableBreadcrumbEvents = function() {
      self.$el.on('click', '[data-modal-breadcrumb]:first .geop-modal-breadcrumb-item', function() {
        var $item = $(this);
    ...
    // or there is no modal already open with the `geop-modal-slideable`
    ...
    // the `geop-modal-slideable` CSS class and, if so, returns
    // a jQuery reference to that modal.

    self.getSlideableAncestorEl = function() {
      var stack = geop.modalSupport.stack;
      if (!stack.length) {
        return false;
      }
      var $slideableAncestorEl = stack[stack.length - 1];

      if (!$slideableAncestorEl.hasClass('geop-modal-slideable')) {
        return false;
      }
    ...
    // Returns the `geoportal-modal` object, not a jQuery element. Use findSafe
    ...
    // Returns the `geoportal-modal` objects corresponding to each
    // slide nested in this modal, which presumably is a slide parent
    // modal (one with the `geop-modal-slideable` CSS class). The
    ...
    // Displays the modal. The `enhance` Geoportal event is triggered,
    ...
    // the top modal already on the stack has the `geop-modal-slideable`
    ...
    // modal on the stack has the `geop-modal-slideable` CSS class.
    ...
    self.slideIn = function() {
      ...
      setImmediate(function() {
        $wrapper.find('[data-modal-content]').removeClass('geop-modal-slide-current');
        var fired = false;
        ...
        $content.addClass('geop-modal-slide-current');
      });

    };

    // Make the current modal's depth available as an attribute on various
    // elements such as `data-geop-modal-instructions`. This is needed for
    // reliable Nightwatch testing
    self.setDepthAttribute = function($el) {
      $el.attr('data-geop-modal-depth', geop.modalSupport.depth);
    };
    ...
    // the stack or the parent modal does not have the `geop-modal-slideable`
    // CSS class.
    ...
    self.stackPush = function() {
      ...
      geop.modalSupport.stack.push(self.$el);

      geop.emit('modalStackPush');

      self.$el.addClass('geop-modal-stack-push');
      $('body').append(self.$el);

      self.$el.show();
      self.$el.removeClass('geop-modal-stack-push');
    ...
    // all have the data-geop-modal-current attribute, and
    // that nothing else does.

    self.indicateCurrentModal = function(adding) {
      if (adding) {
        geop.modalSupport.all.push(self);
      } else {
        var top = geop.modalSupport.getLatestModal();
        if (top !== self) {
          geop.utils.error('Types do not match in indicateCurrentModal');
        }
        geop.modalSupport.all.pop();
      }
      $('body [data-geop-modal-current]').removeAttr('data-geop-modal-current');
      var latest = geop.modalSupport.getLatestModal();
      if (latest) {
        _.each([ latest.$el, latest.$controls, latest.$modalFilters, latest.$instructions ], function($el) {
          $el.attr('data-geop-modal-current', latest.__meta.name);
        });
      }
    };
    ...
    self.resizeContentHeight = function() {
      ...
      var headerHeight = $container.find('.geop-modal-header').outerHeight() || 0;
      var breadcrumbHeight = $container.find('.geop-modal-breadcrumb').outerHeight() || 0;
      var footerHeight = $container.find('.geop-modal-footer').outerHeight() || 0;

      var contentHeight = 'calc(100vh - 50px - ' + (headerHeight + breadcrumbHeight) + 'px)';

      // Set height on .geop-modal-content.
      // Check if self.$el is a sliding .geop-modal-content or modal wrapper

      var $content;
      if (self.$el.is('.geop-modal-content')) {
        $content = self.$el;
        ...
        $content = self.$el.find('.geop-modal-content').last();

        // dont resize the height of modals within modals
        if ($content.parents('[data-tab]').length === 0) {
          $content.css('height', contentHeight);
          self.$el.find('.geop-modal-content').find('[data-schema-tabs]').css('height', contentHeight);
        }
      }

      // geop-modal-footer is now *inside* geop-modal-content, we need to
      // adjust the height of geop-modal-body relative to it. -Tom
      var $body = $content.find('.geop-modal-body');

      if ($body.parents('[data-tab]').length === 0) {
        $body.css('height', 'calc(100% - ' + footerHeight + 'px)');
      }

      // geop-chooser needs the same treatment, although the CSS seems to imply it should not.
      var $chooser = $content.find('.geop-chooser');
      if ($chooser.length) {
    ...
    // schema field (see `geoportal-schemas`) or a modal related to editing one
    ...
    self.refreshBreadcrumb = function() {
      ...
      if (!$ancestor) {
        if (self.$el.hasClass('geop-modal-slideable')) {
          $ancestor = self.$el;
        }
        if (!$ancestor) {
          return;
      ...
      _.each(slides, function(slide, i) {
        var $li = $('<li></li>');
        $li.addClass('geop-modal-breadcrumb-item');
    ...
    // such as `geoportal-pieces-editor-modal` that provides its own version,
    ...
    self.stackPop = function() {
      ...
      var $current = geop.modalSupport.getTopModalOrBody();
      if ($current.data('aposSavedScrollTop') !== undefined) {
        $(window).scrollTop($current.data('aposSavedScrollTop'));
      }

      geop.modalSupport.stack.pop();

      geop.emit('modalStackPop');

      var $blackoutContext = geop.modalSupport.getTopModalOrBody();
      var $blackout = $blackoutContext.find('.geop-modal-blackout');
      if ($blackout.data('interval')) {
        clearInterval($blackout.data('interval'));
      }

      $blackout.addClass('geop-modal-blackout-fade');
      self.$el.addClass('geop-modal-stack-push');
      ...
      self.$el.one('webkitTransitionEnd otransitionend oTransitionEnd msTransitionEnd transitionend', function() {
        ...
        self.$body.removeClass('geop-active-blackout');

        if (!geop.modalSupport.stack.length) {
    ...
    // Applies a blackout div with the `geop-modal-blackout` CSS class to
    ...
    self.applyBlackout = function() {
      ...
      var $blackoutContext = geop.modalSupport.getTopModalOrBody();
      var $blackout = $('<div class="geop-modal-blackout"></div>');
      ...
      self.$body.addClass('geop-active-blackout');
    };
    ...
    self.focusFirstFormElement = function() {
      ...
      var $firstElm = self.$el.find('.geop-modal-body form fieldset:first input');
...
// `geop.modalSupport` provides a handful of global methods used to manipulate
...
geop.modalSupport = {
  ...
  // The objects are actual modal objects (`geoportal-modal`),
  // NOT jQuery objects
  all: [],
  // Returns jQuery object
  getTopModalOrBody: function() {
    return $(geop.modalSupport.stack.length ? geop.modalSupport.stack[geop.modalSupport.stack.length - 1] : 'body');
  },
  // Returns actual geoportal-modal object, including latest slide (not views)
  getLatestModal: function() {
    if (!geop.modalSupport.all.length) {
      return null;
    }
    return geop.modalSupport.all[geop.modalSupport.all.length - 1];
  },
  closeTopModal: function() {
    var topModal = geop.modalSupport.getTopModalOrBody();
