geop.define('geoportal-widgets', {
  ...
  construct: function(self, options) {
    ...
    self.edit = function(data, options, save) {
      ...
      if (!data._id) {
        _.assign(data, geop.schemas.newInstance(self.schema));
      }
      ...
      function checkOrGenerateId (data) {
        data = data || {};
        if (!data._id) {
          data._id = geop.utils.generateId();
        }
        return data;
      }
    ...
    self.getData = function($widget) {
      ...
      _.each(self.schema, function(field) {
        var type = geop.schemas.fieldTypes[field.type];
        if (
          ...
          geop.schemas.fieldTypes[field.type].contextualConvert(data, field.name, $widget, field);
        }
      });
      return data;
    };
    ...
    // like geoportal-rich-text, as opposed to widgets edited
    // via a modal.
