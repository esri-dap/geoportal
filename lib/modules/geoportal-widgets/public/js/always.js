// `geoportal-widgets` is a parent class for the browser-side managers of
// widget types. Each manager object is responsible for *all* widgets of that type.
//
// Extends `geoportal-context` in order to gain access to conveniences like
...
geop.define('geoportal-widgets', {
  ...
  afterConstruct: function(self) {
    ...
    geop.areas.setWidgetManager(self.name, self);
  ...
  construct: function(self, options) {
    ...
    // JSON data, but some widgets, like geoportal-rich-text,
    ...
    self.canEdit = function($widget) {
      ...
      var result = data._edit && (!$widget.closest('[data-geop-area-disabled-editing]').length);
      return result;
    };
  }
});
