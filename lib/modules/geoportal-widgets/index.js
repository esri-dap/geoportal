...
// [geoportal-rich-text-widgets](../geoportal-rich-text-widgets/index.html),
// [geoportal-pieces-widgets](../geoportal-pieces-widgets/index.html) and
// [geoportal-video-widgets](../geoportal-video-widgets/index.html).
//
...
// It will be singular if it displays one thing, like `geoportal-video`,
// and plural if it displays more than one thing, like `geoportal-pieces`.
// **By default, Geoportal automatically removes `-widgets` from the name
...
// (see `always.js` in `geoportal-images-widgets` for an example).
//
...
// ### `scene`
//
// If your widget wishes to use Geoportal features like schemas
...
// subclass of `geoportal-custom-pages` or `geoportal-pieces-pages`, as well.
//
...
// ### `defer`
//
// If you set `defer: true` for a widget module, like geoportal-images-widgets, the join to
...
// you can also set `deferWidgetLoading: true` for the `geoportal-global` module. To avoid chicken
...
// [geoportal-video-widgets](../geoportal-video-widgets/index.html) module:
//
// ```javascript
...
//       return geop.oembed.queryAndPlay($widget.find('[data-geop-video-player]'), data.video);
//     };
//   }
// });
// ```
...
module.exports = {
  ...
  afterConstruct: function(self) {
    ...
    if (!self.options.label) {
      throw new Error('You must specify the label option when subclassing geoportal-widgets');
    }
    ...
    self.geop.areas.setWidgetManager(self.name, self);

    self.geop.push.browserMirrorCall('user', self);
    self.geop.push.browserMirrorCall('user', self, { tool: 'editor' });

    self.geop.tasks.add(self.__meta.name, 'list',
  ...
  construct: function(self, options) {
    ...
    self.composeSchema = function() {
      self.schema = self.geop.schemas.compose(options, self);
    ...
    // `geoportal-areas` module as it iterates over widgets in
    ...
    self.load = function(req, widgets, callback) {
      ...
      function join(callback) {
        return self.geop.schemas.join(req, self.schema, widgets, undefined, callback);
      }
      function area(callback) {
        ...
        var cursor = self.geop.docs.find(req).joins(false);
    ...
    // `convert` method of `geoportal-schemas`, creating a new `output` object
    ...
    self.sanitize = function(req, input, callback) {
      ...
      // `by` doesn't go missing for `geoportal-images-widgets`
      var output = self.geop.schemas.newInstance(self.schema);
      var schema = self.allowedSchema(req);
      output._id = self.geop.launder.id(input._id) || self.geop.utils.generateId();
      return self.geop.schemas.convert(req, schema, 'form', input, output, function(err) {
        if (err) {
          return callback(err);
        }
    ...
    self.allowedSchema = function(req) {
      return _.filter(self.schema, function(field) {
        return (!field.permission) || self.geop.permissions.can(req, field.permission);
      });
    };
    ...
    self.filterForDataAttribute = function(widget) {
      var data = self.geop.utils.clonePermanent(widget, true);
    ...
    self.filterOptionsForDataAttribute = function(options) {
      return self.geop.utils.clonePermanent(options, true);
    };
    ...
    self.pushDefineSingleton = function() {
      self.geop.push.browserMirrorCall('always', self, { stop: 'geoportal-widgets' });
    };
    ...
    self.list = function(apos, argv, callback) {
      ...
      return self.geop.migrations.eachWidget({}, iterator, callback);
      ...
      function iterator(doc, widget, dotPath, callback) {
        ...
        if (widget.type === self.name) {
          ...
          // not become an geop.utils.log call. -Tom
    ...
    self.addSearchTexts = function(widget, texts) {
      self.geop.schemas.indexFields(self.schema, widget, texts);
    };
