...
// the `geoportal-passport` module, which adds support for login via
...
// The default is `48`.
//
// ## Notable properties of geop.modules['geoportal-login']
//
// `passport`
//
// Geoportal's instance of the [passport](https://npmjs.org/package/passport) npm module.
...
module.exports = {
  ...
  singletonWarningIfNot: 'geoportal-login',
  ...
  construct: function(self, options) {
    ...
    // user via the [geoportal-users](https://docs.geoportalcms.org/geoportal/modules/geoportal-users)
    ...
    self.deserializeUser = function(id, callback) {
      ...
      return self.geop.users.find(req, { _id: id }).toObject(function(err, user) {
        if (err) {
          return callback(err);
        }
        if (!user) {
    ...
    // [geoportal-users](https://docs.geoportalcms.org/geoportal/modules/geoportal-users) module.
    // Users with the `disabled` property set to true may not log in.
    // Passwords are verified via the `verifyPassword` method of
    // [geoportal-users](https://docs.geoportalcms.org/geoportal/modules/geoportal-users), which is
    ...
    self.verifyLogin = function(username, password, callback) {
      var req = self.geop.tasks.getReq();
      return self.geop.users.find(req, {
        $or: [
          { username: username },
          { email: username }
        ],
        ...
        return self.geop.users.verifyPassword(user, password, function(err) {
          if (err) {
            // Slow down and keep 'em hanging to make brute force attacks less easy
            return setTimeout(function () {
              return callback(null, false);
    ...
    self.addRoutes = function() {
      if (self.options.localLogin !== false) {
        self.geop.app.get('/login', function(req, res) {
          if (req.user) {
      ...
      if ((self.options.localLogin !== false) && self.options.passwordReset) {
        ...
        self.geop.app.get('/password-reset-request', function(req, res) {
          if (req.user) {
            // User is already logged in, redirect to home page
            return res.redirect('/');
          }
        ...
        self.geop.app.post('/password-reset-request', function(req, res) {
        ...
        self.geop.app.get('/password-reset', function(req, res) {
          var reset = self.geop.launder.string(req.query.reset);
          var email = self.geop.launder.string(req.query.email);
          if (!reset.length) {
            return res.redirect('/password-reset-request?error=missing');
          }
          req.scene = 'user';
          var adminReq = self.geop.tasks.getReq();
          return self.geop.users.find(adminReq, {
            email: email,
        ...
        self.geop.app.post('/password-reset', function(req, res) {
          var reset = self.geop.launder.string(req.body.reset);
          var email = self.geop.launder.string(req.body.email);
          var password = self.geop.launder.string(req.body.password);
          ...
          return Promise.try(function() {
            ...
            return self.geop.users.verifySecret(user, 'passwordReset', reset).then(function() {
              return user;
            });
          }).then(function(user) {
            user.password = password;
            delete user.passwordResetAt;
            return self.geop.users.update(adminReq, user);
          }).then(function(user) {
            return self.geop.users.forgetSecret(user, 'passwordReset');
          }).then(function(user) {
      ...
      self.geop.app.get('/logout', function(req, res) {
    ...
    self.sendPasswordResetEmail = function(req, user) {
      ...
      var reset;
      return Promise.try(function() {
        reset = self.geop.utils.generateId();
        user.passwordReset = reset;
        user.passwordResetAt = new Date();
        return self.geop.users.update(req, user, { permissions: false }).then(function() {
          return user;
        });
      }).then(function(user) {
        if (!user.email) {
    ...
    self.addAdminBarItems = function() {
      self.geop.adminBar.add(self.__meta.name + '-logout', 'Log Out', null, { last: true, href: '/logout' });
    };
    ...
    self.modulesReady = function() {
      ...
      self.geop.users.addSecret('passwordReset');
    };
