geop.define('geoportal-docs', {
  ...
  construct: function(self, options) {
    ...
    self.lock = function(_id, callback) {
      ...
      function fail() {
        geop.notify('You were unable to take control of the document.', { type: 'error' });
    ...
    // Watch for clicks on links with a [data-geop-fix-id], and open
    // the relevant document for editing, displaying the
    // [data-geop-fix-hint] first to explain why the document is being
    ...
    // at any time.
    //
    // The data-geop-fix-id, data-geop-fix-type and data-geop-fix-url
    // attributes should be populated on your link or button.
    // If you know the doc is a piece, not a page, you can skip
    // the url attribute.
    //
    // If data-geop-fix-error is present, the error with the same name
    // is removed from the enclosing fieldset when the link is clicked.

    self.enableFix = function() {
      $('body').on('click', '[data-geop-fix-id]', function() {
        var $el = $(this);
        var hint = $el.attr('data-geop-fix-hint');
        var type = $el.attr('data-geop-fix-type');
        var id = $el.attr('data-geop-fix-id');
        var manager = geop.docs.getManager(type);
        if (!confirm(hint)) {
          return false;
        }
        geop.schemas.removeError($el.closest('[data-name]'), $el.attr('data-geop-fix-error'));
        if (manager && manager.edit) {
          manager.edit(id, { field: 'slug' });
        } else {
          geop.pages.pageSettings({
            page: {
              type: type,
              _id: id
            },
