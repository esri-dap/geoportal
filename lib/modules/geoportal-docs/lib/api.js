...
module.exports = function(self, options) {
  ...
  self.enableCollection = function(callback) {
    self.geop.db.collection('aposDocs', function(err, collection) {
      self.db = collection;
      return callback(err);
    });
  };
  ...
  self.ensureIndexes = function(callback) {
    ...
    function indexText(callback) {
      return self.ensureTextIndex(function(err) {
        if (err) {
          self.geop.utils.error('WARNING: unable to ensure text index, geoportal:migrate can fix that');
        }
  ...
  // you get every document in Geoportal, which is
  // too many.
  //
  ...
  // cursor methods:
  //
  // geop.docs.find(req, { type: 'foobar' }).toArray(
  //   function(err, docs) { ... }
  // );
  ...
  self.find = function(req, criteria, projection) {
    ...
    return self.geop.create('geoportal-cursor', { apos: self.geop, req: req, criteria: criteria, projection: projection });
  };
  ...
  // geop.*.docFixUniqueError is called with the
  ...
  // Geoportal edits doc editing and viewing permissions via joins,
  ...
  self.deleteFromTrash = function(req, idOrCriteria, callback) {
    ...
    criteria.trash = true;
    criteria = {
      $and: [ criteria, self.geop.permissions.criteria(req, 'publish-doc') ]
    };
      ...
      function(callback) {
        return actor(function(err) {
          ...
          if (attempt === maxAttempts) {
            ...
            firstError.geopAddendum = 'retryUntilUnique failed, most likely you need another docFixUniqueError method to handle another property that has a unique index, reporting original error, see also aposLatestError property';
            firstError.geopLatestError = err;
  ...
  self.docBeforeInsert = function(req, doc, options) {
    ...
    if (!doc._id) {
      doc._id = self.geop.utils.generateId();
    }
  ...
  // and, in some extensions of Geoportal, the new doc's
  // content.
  ...
  self.testInsertPermissions = function(req, doc, options) {
    ...
    if (!(options.permissions === false)) {
      if (!self.geop.permissions.can(req, 'edit-' + doc.type, null, doc)) {
  ...
  self.docBeforeSave = function(req, doc, options) {
    ...
    _.each(manager.schema, function(field) {
      if (field.sortify) {
        doc[field.name + 'Sortified'] = self.geop.utils.sortify(doc[field.name] ? doc[field.name] : '');
      }
    });
  ...
  self.ensureSlug = function(doc) {
    ...
    if ((!doc.slug) || (doc.slug === 'none')) {
      if (doc.title) {
        doc.slug = self.geop.utils.slugify(doc.title);
  ...
  // Geoportal then retries the operation.
  //
  // Note that methods of this name are invoked
  // on ALL modules that have them, starting with
  // this one. Although this method takes no
  ...
  self.docBeforeUpdate = function(req, doc, options) {
    ...
    if (options.permissions !== false) {
      if (!self.geop.permissions.can(req, 'edit-' + doc.type, doc)) {
        throw new Error('forbidden');
      }
    }
  };
  ...
  //
  // self.geop.docs.getManager(doc.type).update(...)
  //
  ...
  self.updateBody = function(req, doc, options, callback) {
    ...
    return self.retryUntilUnique(req, doc, function(callback) {
      return self.db.update({ _id: doc._id }, self.geop.utils.clonePermanent(doc), callback);
    }, callback);
  };
  ...
  self.insertBody = function(req, doc, options, callback) {
    ...
    return self.retryUntilUnique(req, doc, function(callback) {
      return self.db.insert(self.geop.utils.clonePermanent(doc), callback);
    }, callback);
  };
  ...
  // is a module that subclasses `geoportal-doc-type-manager`
  // (or its subclasses `geoportal-pieces` and `geoportal-custom-pages`).
  ...
  // with `geoportal-pages` always have one).
  ...
  self.lock = function(req, id, contextId, options, callback) {
    ...
    criteria = {
      $and: [
        criteria,
        self.geop.permissions.criteria(req, 'edit')
      ]
    };
      ...
      if (!result.result.nModified) {
        return self.db.findOne({ _id: id }, { advisoryLock: 1 }, function(err, info) {
          ...
          if (info.advisoryLock.username === req.user.username) {
            return callback('locked-by-me', self.geop.i18n.__('You may be editing that document in another tab or window.\nIf you take control, you could lose work in that tab or window.\nDo you want to take control?'));
          }
          return callback('locked', self.geop.i18n.__(
        ...
        function(err, info) {
          ...
          if ((!info.advisoryLock) || (info.advisoryLock._id !== contextId)) {
            if (info.advisoryLock && info.advisoryLock.username === req.user.username) {
              message = self.geop.i18n.__('You are now editing this document in another tab or window.');
              return callback('locked-by-me');
            }
            message = self.geop.i18n.__('Control of this document has been taken by another user.');
