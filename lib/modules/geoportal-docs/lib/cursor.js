...
// An `geoportal-cursor` allows you to conveniently fetch docs from
// the `aposDocs` mongodb collection using chainable filters, much
// like a MongoDB or Doctrine cursor.
//
// Usually you'll be working with a subclass of `geoportal-cursor`
...
// main `geoportal-cursor` class that are useful with all types
// of docs, like the `areas` filter that calls loaders for
// the widgets in Geoportal areas.
//
...
// `find()` method of the `geoportal-docs` module (`geop.docs.find`).
// If you are interested in all pages (docs that are part of the
// page tree), call the `find()` method of the `geoportal-pages`
// module (`geop.pages.find`).
//
...
// `geoportal-blog` module is in use)
//
// ```
// return geop.docs.getManager('geoportal-blog').
//   find().
//   limit(10).
...
// on an `geoportal-pieces-pages` page, such as a blog. In many
...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    // this functionality and Geoportal builds the
    ...
    // in any way (hint: use `self.geop.launder.string`, etc).
    // If you also set `safeFor: 'public'`, the filter then becomes
    // available as a query string parameter in `geoportal-pieces-pages`
    ...
    self.addFilter('projection', {
      finalize: function() {
        ...
        // in MongoDB projections. However Geoportal
    ...
    self.projectUrlField = function(add) {
      ...
      var manager = type && self.geop.docs.getManager(type);
      var fields;
      if (manager) {
        fields = manager.getUrlFields();
      } else {
        fields = self.geop.docs.getDefaultUrlFields();
      }
    ...
    // generic cursor obtained from `geop.docs.find`
    ...
    // a default.
    //
    // This filter is called by geoportal-pieces based on its
    ...
    self.addFilter('skip', {
      launder: function(s) {
        return self.geop.launder.integer(s, 0, 0);
      },
      safeFor: 'public'
    });
    ...
    //
    // Used by `geoportal-pieces` and `geoportal-pieces-pages`.

    self.addFilter('perPage', {
      def: undefined,
      launder: function(i) {
        return self.geop.launder.integer(i, 1, 1);
      },
      safeFor: 'manage'
    });
    ...
    self.addFilter('permission', {
      finalize: function() {
        ...
        if (permission !== false) {
          ...
          var criteria = self.geop.permissions.criteria(self.get('req'), permission);
          self.and(criteria);
        }
      },
      after: function(results) {
        ...
        self.geop.permissions.annotate(self.get('req'), 'edit-doc', results);
        self.geop.permissions.annotate(self.get('req'), 'publish-doc', results);
      }
    });
    ...
    self.addFilter('tags', {
      ...
      launder: function(s) {
        ...
        return _.map(s, function(value) {
          return self.geop.launder.string(value);
        });
      }
    });
    ...
    self.addFilter('trash', {
      finalize: function() {
        ...
        if (!trash) {
          ...
          // an index: https://github.com/geoportalcms/geoportal/issues/1601
          self.and({
            trash: {
              $ne: true
            }
      ...
      safeFor: 'manage',
      launder: function(s) {
        return self.geop.launder.booleanOrNull(s);
      },
    ...
    self.addFilter('explicitOrder', {
      ...
      after: function(results) {
        ...
        var property = self.get('explicitOrderProperty');
        var temp = self.geop.utils.orderById(values, results, property);
        var i;
    ...
    self.addFilter('joins', {
      ...
      after: function(results, callback) {
        ...
        return async.eachSeries(_.keys(resultsByType), function(type, callback) {
          ...
          return self.geop.schemas.join(self.get('req'), manager.schema, resultsByType[type], self.get('joins'), callback);
        }, callback);
      }
    });
    ...
    self.addFilter('addUrls', {
      ...
      after: function(results, callback) {
        ...
        return async.eachSeries(interesting, function(type, callback) {
          return self.geop.docs.getManager(type).addUrls(req, byType[type], callback);
        }, callback);
      }
    });
    ...
    // geoportal-pieces-pages automatically does this when a filter is
        ...
        return async.waterfall([
          ...
          function(callback) {
            ...
            if (self.get('log') || process.env.APOS_LOG_ALL_QUERIES) {
              self.geop.utils.log(require('util').inspect(criteria, { depth: 20 }));
            }
    ...
    // It is a low level implementation detail overridden by `geoportal-optimizer` as needed.
    // Seemingly we don't need req at all here, but overrides like geoportal-optimizer need it,
    ...
    self.clone = function() {
      var cursor = self.geop.create(self.__meta.name, options);
    ...
    self.finalizeSort = function() {
      ...
      if (sort) {
        ...
        // There's an geoportal-page "type" that is never really in the database but
        // has a manager thanks to geoportal-any-page-manager. Use that as a default
        // so that we always get a manager object

        var manager = self.geop.docs.getManager(self.get('type') || 'geoportal-page');
        if (!(manager && manager.schema)) {
          return;
        }
