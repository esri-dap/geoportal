...
module.exports = function(self) {
  ...
  return function(callback) {
    ...
    autocomplete = self.geop.utils.sortify(autocomplete);
    ...
    clauses = clauses.concat(
      _.map(words, function(word) {
        return {
          highSearchWords: self.geop.utils.searchify(word, true)
        };
      })
    );

    clauses.push(
      { highSearchText: self.geop.utils.searchify(autocomplete) }
    );
