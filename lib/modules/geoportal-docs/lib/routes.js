...
module.exports = function(self, options) {
  ...
  self.apiRoute('post', 'lock', function(req, res, next) {
    const result = self.lock(req,
      self.geop.launder.id(req.body._id),
      req.htmlPageId,
      {
  ...
  self.apiRoute('post', 'slug-taken', function(req, res, next) {
    ...
    var slug = self.geop.launder.string(req.body.slug);
    ...
    return self.find(req, criteria).permission(false).trash(null).published(null).projection({ _url: 1, title: 1 }).toObject().then(function(doc) {
      if (doc) {
        var pageType = _.find(self.geop.pages.options.types, { name: doc.type });
        var label = (pageType && pageType.label) || self.geop.docs.getManager(doc.type).options.label || doc.type;
