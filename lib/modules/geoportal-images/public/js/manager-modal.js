...
geop.define('geoportal-images-manager-modal', {
  extend: 'geoportal-pieces-manager-modal',

  construct: function(self, options) {

    // save manager so we can call edit() later
    self.manager = geop.docs.getManager(options.name);
    ...
    self.enableButtonEvents = function() {
      ...
      self.$el.on('click', '[data-focus-' + options.name + ']', function(e) {
        ...
        if ($(e.target).attr('data-geop-edit-geoportal-image')) {
    ...
    self.focusElement = function($el) {
      ...
      if ($checkbox.prop('checked') || (!(self.chooser && self.chooser.full))) {
        $el.toggleClass('geop-focus');
        $checkbox.prop('checked', function(i, currentState) { return !currentState; });
        $checkbox.trigger('change');
        $el.toggleClass('geop-focus', $checkbox.prop('checked'));
      }
    };

    var superAfterRefresh = self.afterRefresh;
    self.afterRefresh = function() {
      self.$gridView = self.$el.find('.geop-manage-view');
      self.$gridItems = self.$el.find('.geop-manage-grid-piece');
      superAfterRefresh();
    };
    ...
    self.displayChoiceInCheckbox = function(id, state) {
      ...
      $checkbox.parent('label').toggleClass('geop-focus', state);
      $checkbox.closest('[data-piece]').toggleClass('geop-focus', state);
      return $checkbox;
    };
