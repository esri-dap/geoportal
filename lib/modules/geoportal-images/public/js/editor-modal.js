geop.define('geoportal-images-editor-modal', {
  ...
  construct: function(self, options) {
    ...
    self.enableTitleFromAttachment = function() {
      geop.on('attachmentUpdated', self.titleFromAttachmentListener);
    };
    ...
    self.titleFromAttachmentListener = function(info) {
      if (!self.$el.is(':visible')) {
        ...
        geop.off('attachmentUpdated', self.titleFromAttachmentListener);
        return;
      }
      ...
      var $title = geop.schemas.findField(self.$el, 'title');
      if (!$title.val().length) {
