...
geop.define('geoportal-images-chooser', {
  extend: 'geoportal-pieces-chooser',
  autocomplete: false,
  construct: function(self, options) {
    self.finalize = function(callback) {
      ...
      var manager = geop.docs.getManager(self.field.withType);
        ...
        return async.eachSeries(choices, function(choice, callback) {
          ...
          function retrieve(callback) {
            return manager.api('retrieve', { _id: choice.value }, function(result) {
              if (result.status !== 'ok') {
                geop.notify('An error occurred while retrieving the image to be cropped. Perhaps it has been removed.', { type: 'error' });
                return;
              }
          ...
          function crop(callback) {
            if (!geop.attachments.options.croppable[attachment.extension]) {
              return callback(null);
            }
            ...
            return geop.attachments.api(
              ...
              function(data) {
                if (data.status !== 'ok') {
                  geop.notify('An error occurred while cropping.', { type: 'error', dismiss: true });
    ...
    self.link('geop-focal-point', 'item', function($button, _id) {
