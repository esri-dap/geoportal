...
module.exports = function(self, options) {
  ...
  // This method is available as a template helper: geop.images.first
  //
  ...
  // an `geoportal-images` piece or not. It simply checks whether the `group`
  ...
  //
  // geop.images.first(page.body)
  //
  // 2. Must be a GIF
  //
  // geop.images.first(page.body, { extension: 'gif' })
  //
  // (Note Geoportal always uses .jpg for JPEGs.)
  //
  // OPTIONS:
  //
  ...
  self.first = function(within, options) {
    ...
    var result = self.geop.attachments.first(within, options);
    return result;
  };

  // This method is available as a template helper: geop.images.all
  //
  ...
  //
  // geop.images.all(page.body)
  //
  // 2. Must be GIFs
  //
  // geop.images.all(page.body, { extension: 'gif' })
  //
  // (Note Geoportal always uses .jpg for JPEGs.)
  //
  // OPTIONS:
  ...
  self.all = function(within, options) {
    options = options ? _.clone(options) : {};
    options.group = 'images';
    return self.geop.attachments.all(within, options);
  };

  // This method is available as a template helper: geop.images.srcset
  //
  ...
  self.srcset = function(attachment, cropRelationship) {
    if (!self.geop.attachments.isSized(attachment)) {
      return '';
    }
    ...
    var sources = self.geop.attachments.imageSizes.filter(function(imageSize) {
      ...
      var src = self.geop.attachments.url(attachment, { size: imageSize.name, crop: cropRelationship });
  ...
  self.isCroppable = function(image) {
    return image && self.geop.attachments.isCroppable(image.attachment);
  };
  ...
  self.afterList = function(req, results, callback) {
    ...
    if (minSize) {
      results.minSize = [
        self.geop.launder.integer(minSize[0]),
        self.geop.launder.integer(minSize[1])
      ];
    }
