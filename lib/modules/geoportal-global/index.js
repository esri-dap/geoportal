// Provides req.data.global, an Geoportal doc
...
// even in requests that are not for Geoportal pages. In a command line task, you can use
// the provided `findGlobal` method.
//
// `separateWhileBusyMiddleware`: if true, the `whileBusy` method is powered
// by separate middleware that checks for the lock flag in `geoportal-global`
...
module.exports = {
  ...
  singletonWarningIfNot: 'geoportal-global',

  name: 'geoportal-global',
  ...
  construct: function(self, options) {
    ...
    // geoportal:migrate event. But to maximize bc, we still register the event
    ...
    self.modulesReady = function(callback) {
      self.on('geoportal:migrate', 'initGlobalPromisified', function(options) {
    ...
    self.initGlobal = function(callback) {
      ...
      return async.series({
        ...
        fetch: function(callback) {
          ...
          return self.geop.docs.db.find({ slug: self.slug }).toArray(function(err, result) {
            if (err) {
              return callback(err);
            }
            existing = result;
        ...
        insert: function(callback) {
          ...
          return self.geop.docs.insert(req, insert, callback);
        },
        update: function(callback) {
          ...
          return async.eachSeries(existing, function(doc, callback) {
            ...
            if (modified) {
              ...
              return self.geop.docs.update(eReq, doc, callback);
            } else {
              return setImmediate(callback);
            }
          }, callback);
    ...
    self.checkWhileBusy = function(req, _global, callback) {
      var lockName = 'geoportal-global-busy';
      var localeLockName;
      ...
      function considerLock(propName, lockName, callback) {
        if (_global[propName]) {
          return self.geop.locks.lock(lockName, {
        ...
        function unlock(callback) {
          return self.geop.locks.unlock(lockName, callback);
        }
    ...
    self.addGlobalToData = function(req, res, next) {
      if (arguments.length >= 3) {
        ...
        return body(function(err) {
          if (err) {
            self.geop.utils.error('ERROR loading global doc: ', err);
    ...
    // 'geoportal-workflow' (the global docs must have `workflowLocale`
    // properties).
    ...
    self.whileBusy = function(fn, options) {
      ...
      return Promise.try(function() {
        ...
        return self.geop.docs.db.update(criteria, args, {
        ...
        return Promise.try(function() {
          ...
          if (locked) {
            return self.geop.locks.unlock(lockName);
          }
        });
      });
    };
    ...
    // to set the permission requirement to admin-geoportal-global.
    // This is called for you.

    self.addToAdminBar = function() {
      self.geop.adminBar.add(self.__meta.name, self.pluralLabel, 'admin-' + self.name);
    };
