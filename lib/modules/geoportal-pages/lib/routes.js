...
module.exports = function(self, options) {
  ...
  self.renderRoute('post', 'editor', function(req, res, next) {
    var type = self.geop.launder.string(req.body.type);
    var verb = self.geop.launder.string(req.body.verb);
    var id = self.geop.launder.id(req.body.id);
    var schema;
    var parentPage;
    if (verb === 'update') {
      var trash = self.geop.docs.trashInSchema ? null : false;
      return self.find(req, { _id: id }).ancestors(true).permission('publish-doc').trash(trash).toObject(function(err, page) {
        ...
        self.geop.schemas.bless(req, schema);
        return succeed();
      });
    } else if (verb === 'copy') {
    ...
    function succeed() {
      return next(null, {
        ...
        data: {
          schema: schema,
          verb: verb,
          trashInSchema: self.geop.docs.trashInSchema
        }
      });
    }
  });
  ...
  self.apiRoute('post', 'fetch-to-insert', function(req, res, next) {
    ...
    var type = self.geop.launder.string(req.body.type);
    return self.find(req, { _id: _id }).permission('edit-geoportal-page').toObject(function(err, parentPage) {
      if (err) {
        return next(err);
      }
      if (!parentPage) {
  ...
  self.apiRoute('post', 'insert', function(req, res, next) {
    var parentId = self.geop.launder.id(req.body.currentPageId);
    ...
    return async.series({
      findParent: function(callback) {
        return self.find(req, { _id: parentId }).permission('publish-geoportal-page').toObject(function(err, _parentPage) {
          if (err) {
            return callback(err);
          }
          if (!_parentPage) {
      ...
      convert: function(callback) {
        ...
        return self.geop.schemas.convert(req, schema, 'form', page, safePage, callback);
      },
  ...
  self.apiRoute('post', 'fetch-to-update', function(req, res, next) {
    var _id = self.geop.launder.id(req.body._id);
    var type = self.geop.launder.string(req.body.type);
    if (!self.geop.docs.getManager(type)) {
      return next('notfound');
    }
    return self.find(req, { _id: _id }).ancestors(true).permission('edit-geoportal-page').trash(self.geop.docs.trashInSchema ? null : false).toObject(function(err, page) {
      if (err) {
        return next('error');
      }
      if (!page) {
  ...
  self.apiRoute('post', 'update', function(req, res, next) {
    var id = self.geop.launder.id(req.body.currentPageId);
    var page = req.body.page || {};
    if (typeof (page) !== 'object') {
      // cheeky
      return next('notfound');
    ...
    return async.series({
      find: function(callback) {
        return self.find(req, { _id: id }).ancestors(true).permission('publish-geoportal-page').trash(self.geop.docs.trashInSchema ? null : false).toObject(function(err, _page) {
          if (err) {
            return callback(err);
          }
          if (!_page) {
      ...
      convert: function(callback) {
        ...
        var manager = self.geop.docs.getManager(self.geop.launder.string(page.type || existingPage.type));
        if (!manager) {
          // sneaky
          return callback('notfound');
        }
        ...
        return self.geop.schemas.convert(req, schema, 'form', page, existingPage, callback);
      },
      ...
      findAgain: function(callback) {
        // Fetch the page. Yes, we already have it, but this way all the cursor
        // filters run and we have access to ._url
        return self.find(req, { _id: existingPage._id }).published(null).trash(self.geop.docs.trashInSchema ? null : false).toObject(function(err, _page) {
          if (err) {
            return callback(err);
          }
          if (!_page) {
  ...
  self.apiRoute('post', 'fetch-to-copy', function(req, res, next) {
    ...
    return self.find(req, { _id: _id }).ancestors(true).permission('edit-geoportal-page').toObject(function(err, page) {
      if (err) {
      ...
      if (currentSlugTitle !== self.geop.utils.slugify(page.title)) {
        slugComponents[slugComponents.length - 1] = self.geop.utils.slugify(page.title);
  ...
  self.apiRoute('post', 'copy', function(req, res, next) {
    var currentPageId = self.geop.launder.id(req.body.currentPageId);
    var page = req.body.page || {};
    if ((typeof page) !== 'object') {
      // cheeky
      return next('invalid');
    ...
    return async.series({
      findParentAndSibling: function(callback) {
        return self.find(req, { _id: currentPageId }).permission('publish-geoportal-page').ancestors({ permission: false }).toObject(function(err, currentPage) {
          if (err) {
            return callback(err);
          }
          if (!currentPage) {
      ...
      convert: function(callback) {
        var manager = self.geop.docs.getManager(self.geop.launder.string(page.type));
        if (!manager) {
          // sneaky
          return callback('notfound');
        }
  ...
  self.apiRoute('post', 'move', function(req, res, next) {
    return self.move(req, self.geop.launder.id(req.body.movedId), self.geop.launder.id(req.body.targetId), self.geop.launder.string(req.body.position), function(err, changed) {
      if (err) {
        return next(err);
      }
  ...
  self.apiRoute('post', 'move-to-trash', function(req, res, next) {
    return self.moveToTrash(req, self.geop.launder.id(req.body._id), function(err, parentSlug, changed) {
      if (err) {
        return next(err);
      }
  ...
  self.apiRoute('post', 'rescue-from-trash', function(req, res, next) {
    return self.rescueInTree(req, self.geop.launder.id(req.body._id), function(err, parentSlug, changed) {
      if (err) {
        return next(err);
      }
      return next(null, { changed: self.mapMongoIdToJqtreeId(changed), parentSlug: parentSlug });
  ...
  self.apiRoute('post', 'delete-from-trash', function(req, res, next) {
    return self.deleteFromTrash(req, self.geop.launder.id(req.body._id), function(err, parentSlug) {
      if (err) {
        return next(err);
      }
    ...
    return self.find(req, { level: 0 }).children({ depth: 1000, published: null, trash: null, orphan: null, reorganize: true, areas: false, permission: false }).published(null).trash(null).reorganize(true).toObject(function(err, page) {
      ...
      function pageToJqtree(page) {
        var info = {
          ...
          publish: (page.path === '/trash') || self.geop.permissions.can(req, 'publish-geoportal-page', page),
          edit: self.geop.permissions.can(req, 'edit-geoportal-page', page)
        };
          ...
          _.each(page._children, function(child) {
            if (child.trash && (child.type === 'trash')) {
              ...
              if (self.geop.docs.trashInSchema) {
                forJqtree.label = self.geop.i18n.__('Legacy Trash');
              }
  ...
  self.apiRoute('post', 'info', function(req, res, next) {
    return self.find(req, { _id: self.geop.launder.id(req.body._id) }).published(null).areas(false).toObject(function(err, page) {
      if (err) {
        return next(err);
      }
