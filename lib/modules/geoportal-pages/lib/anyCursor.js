...
// subclassing geoportal-cursor. This way all pages
    ...
    self.addFilter('pageUrl', {
      ...
      after: function(results) {
        _.each(results, function(result) {
          if (result.slug && self.geop.pages.isPage(result)) {
            var url = self.geop.pages.getBaseUrl(self.get('req'));
            result._url = url + self.geop.prefix + result.slug;
          }
        });
      }
    });
