...
module.exports = function(self, options) {
  ...
  self.find = function(req, criteria, projection) {
    return self.geop.create(self.__meta.name + '-cursor', {
      apos: self.geop,
      req: req,
      criteria: criteria,
    ...
    function body(callback) {
      ...
      return async.series({
        ...
        determineNextRank: function(callback) {
          return self.geop.docs.db.findWithProjection({
        ...
        insert: function(callback) {
          var slugBasename = require('path').basename(self.geop.utils.slugify(page.slug, { allow: '/' }));
          var pathFrom;
          if (slugBasename.length && (slugBasename !== 'none')) {
            pathFrom = slugBasename;
          } else {
            pathFrom = self.geop.utils.slugify(page.title);
          }
          page.path = addSlashIfNeeded(parent.path) + pathFrom;
          page.level = parent.level + 1;
          return self.geop.docs.insert(req, page, options, callback);
        }
      }, function(err) {
        if (err) {
          return callback(err);
  ...
  self.lock = function(req, callback) {
    if (req.geopPageTreeLockDepth) {
      req.geopPageTreeLockDepth++;
      return callback(null);
    }
    return self.geop.locks.lock('geoportal-pages:tree', function(err) {
      if (!err) {
        req.geopPageTreeLockDepth = 1;
      }
      return callback(err);
    });
  };
  ...
  self.unlock = function(req, callback) {
    if (!req.geopPageTreeLockDepth) {
      return callback(new Error('Looks like you called geop.pages.unlock without ever calling geop.pages.lock, or you have more unlock calls than lock calls'));
    }
    req.geopPageTreeLockDepth--;
    if (req.geopPageTreeLockDepth) {
      return callback(null);
    }
    return self.geop.locks.unlock('geoportal-pages:tree', callback);
  };
  ...
  //
  // This method is called for us by the geoportal-docs module on update
  ...
  self.docAfterDenormalizePermissions = function(req, page, options, callback) {
    ...
    var criteria = {
      $and: [
        ...
        self.geop.permissions.criteria(req, 'edit-' + page.type)
      ]
    };
    ...
    return self.geop.docs.db.update(criteria, changeSet, { multi: true }, callback);
  ...
  self.newChild = function(parentPage) {
    ...
    if (!pageType) {
      self.geop.utils.warn('No allowed Page types are specified.');
      return null;
    }
    var page = self.geop.docs.getManager(pageType).newInstance();
    _.extend(page, {
      title: 'New Page',
      slug: self.geop.utils.addSlashIfNeeded(parentPage.slug) + 'new-page',
      type: pageType,
  ...
  //
  // These are mainly for use by modules that extend Geoportal's model layer,
  // such as `geoportal-workflow`.
  //
  ...
  // on any Geoportal cursor find() calls used to read and write the database in `self.move`.
  //
  ...
  self.move = function(req, movedId, targetId, position, options, callback) {
    ...
    function body(callback) {
      ...
      function nudgeNewPeers(callback) {
        ...
        self.geop.docs.db.update(mergeCriteria({ path: self.matchDescendants(parent), level: parent.level + 1, rank: { $gte: rank } }), { $inc: { rank: 1 } }, { multi: true }, function(err, count) {
          return callback(err);
        });
      }
      function moveSelf(callback) {
        ...
        if (parent._id !== oldParent._id) {
          var matchOldParentSlugPrefix = new RegExp('^' + self.geop.utils.regExpQuote(addSlashIfNeeded(oldParent.slug)));
      ...
      function trashDescendants(callback) {
        ...
        return self.geop.docs.db.update(mergeCriteria({ path: matchParentPathPrefix }), action, { multi: true }, callback);
      }
    ...
    return async.series(
      [
        ...
        function trashOrUntrashPages(cb) {
          ...
          return self.geop.docs.db.update({ _id: { $in: ids } }, action, {
            multi: true
          }, (err, res) => {
            cb(err);
          });
  ...
  self.deduplicatePages = function(req, pages, toTrash, callback) {
    return async.eachSeries(pages, (page, cb) => {
      ...
      function deduplicate(cb) {
        if (toTrash) {
          return self.geop.docs.getManager(page.type).deduplicateTrash(req, page, cb);
        } else {
          return self.geop.docs.getManager(page.type).deduplicateRescue(req, page, cb);
        }
      }
      function propagate(cb) {
        ...
        return async.eachSeries(descendants, function(descendant, callback) {
          descendant.path = descendant.path.replace(new RegExp('^' + self.geop.utils.regExpQuote(oldPath)), page.path);
          descendant.slug = descendant.slug.replace(new RegExp('^' + self.geop.utils.regExpQuote(oldSlug)), page.slug);
          return self.geop.docs.db.update({ _id: descendant._id }, {
            $set: {
              path: descendant.path,
              slug: descendant.slug
            }
          }, function(err) {
            if (self.geop.docs.isUniqueError(err)) {
  ...
  self.deleteFromTrash = function(req, _id, callback) {
    ...
    function deleteFromTrash(callback) {
      return self.geop.docs.deleteFromTrash(req,
        {
          $or: [
            {
              path: self.matchDescendants(page)
  ...
  self.update = function(req, page, options, callback) {
    ...
    return async.series({
      ...
      update: function(callback) {
        return self.geop.docs.update(req, page, options, callback);
      }
    }, callback);
  };
  ...
  self.serveNotFound = function(req, callback) {
    ...
    return self.callAllAndEmit('pageNotFound', 'notFound', req, function(err) {
      ...
      req.data.suggestedSearch = self.geop.utils.slugify(req.url, { separator: ' ' });
      req.notFound = true;
      ...
      req.browserCall("geop.emit('notfound', ?)", {
  ...
  self.serveDeliver = function(req, err) {
    ...
    if (err) {
      ...
      req.template = self.geop.templates.renderer('templateError');
      req.statusCode = 500;
      providePage = false;
    }
    ...
    if (args.contextMenu) {
      ...
      args.contextMenu = _.filter(args.contextMenu, function(item) {
        ...
        if (self.geop.permissions.can(req, item.permission)) {
          return true;
        }
      });
    }
  ...
  // This method invokes `pushCreateSingleton` to create the `geoportal-pages`
  ...
  self.pageBeforeSend = function(req, callback) {
    ...
    if (typeof _.get(req, 'data.page.level') === 'number') {
      self.geop.templates.addBodyDataAttribute(req,
        {
          'geop-level': req.data.page.level
        }
      );
    }
  ...
  self.ensurePathIndex = function(callback) {
    ...
    return self.geop.docs.db.ensureIndex(params, { unique: true, sparse: true }, callback);
  };
  ...
  self.ensureLevelRankIndex = function(callback) {
    ...
    return self.geop.docs.db.ensureIndex(params, {}, callback);
  };
  ...
  self.updateDescendantsAfterMove = function(req, page, originalPath, originalSlug, options, callback) {
    ...
    var matchParentPathPrefix = new RegExp('^' + self.geop.utils.regExpQuote(originalPath + '/'));
    var matchParentSlugPrefix = new RegExp('^' + self.geop.utils.regExpQuote(originalSlug + '/'));
    var done = false;
    var cursor = self.geop.docs.db.findWithProjection(mergeCriteria({ path: matchParentPathPrefix }), { slug: 1, path: 1, level: 1 });
    return async.whilst(function() { return !done; }, function(callback) {
      return cursor.nextObject(function(err, desc) {
        ...
        return self.geop.docs.retryUntilUnique(req, desc, updateDescendant, callback);

        function updateDescendant(callback) {
          return self.geop.docs.db.update(mergeCriteria({ _id: desc._id }), {
  ...
  self.manageOrphans = function(callback) {
    var managed = self.geop.docs.getManaged();
    var distinct;
    ...
    function findUndefined(callback) {
      return self.geop.docs.db.findOne({ type: { $exists: 0 } }, function(err, _undefinedItem) {
        if (err) {
          return callback(err);
        }
    ...
    function patchUndefined(callback) {
      ...
      return self.geop.docs.db.update({
        type: { $exists: 0 }
      }, {
        $set: {
          type: 'WASUNDEFINED'
    ...
    function find(callback) {
      return self.geop.docs.db.distinct('type', function(err, types) {
        if (err) {
          return callback(err);
        }
        distinct = types;
    ...
    function manage(callback) {
      ...
      self.geop.utils.error('* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *');
      self.geop.utils.error('HEY DEVELOPER: the following types exist in your aposDocs collection,');
      self.geop.utils.error('but are not managed by any module. A generic manager is being stubbed in');
      self.geop.utils.error('for each one to keep your site from crashing, but you should remove these');
      self.geop.utils.error('permanently from the database OR manage them properly by:\n');
      self.geop.utils.error('* Listing them as "types" for "geoportal-pages", OR');
      self.geop.utils.error('* Providing an appropriate subclass of geoportal-pieces.');
      self.geop.utils.error();
      self.geop.utils.error(orphans.join(', '));
      self.geop.utils.error();
      if (_.contains(orphans, 'WASUNDEFINED')) {
        self.geop.utils.error('Some of your docs had no type at all. The type WASUNDEFINED');
        self.geop.utils.error('has been set for them to allow a generic manager to work.');
      }
      self.geop.utils.error('* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *');
  ...
  self.implementParkOne = function(req, item, callback) {
    ...
    return async.series({
      ...
      findExisting: function(callback) {
        var criteria = {};
        var slugWithOrWithoutSlash = new RegExp('^' + self.geop.utils.regExpQuote(item.slug.replace(/\/$/, '')) + '/?$');
        if (item.parkedId) {
      ...
      updateExisting: function(callback) {
        ...
        return self.geop.docs.db.update({
          ...
          $set: self.geop.utils.clonePermanent(item)
        }, callback);
      },
      ...
      insert: function(callback) {
        ...
        if (!parent) {
          return self.geop.docs.insert(req, item, callback);
        } else {
  ...
  self.geop.tasks.add('geoportal-pages', 'unpark',
    'Usage: node app geoportal-pages:unpark /page/slug\n\n' +
  ...
  self.unparkTask = function(callback) {
    if (self.geop.argv._.length !== 2) {
      return callback('Wrong number of arguments');
    }
    var slug = self.geop.argv._[1];
    return self.geop.docs.db.update({
      slug: slug
    }, {
      $unset: {
  ...
  function addSlashIfNeeded(s) {
    return self.geop.utils.addSlashIfNeeded(s);
  }
  ...
  // Invoked by the geoportal-versions module.
  //
  ...
  self.matchDescendants = function(page) {
    ...
    return new RegExp('^' + self.geop.utils.regExpQuote(addSlashIfNeeded(page.path)) + '.');
  };
  ...
  self.isAncestorOf = function(possibleAncestorPage, ofPage) {
    ...
    var regex = new RegExp('^' + self.geop.utils.regExpQuote(possibleAncestorPage.path + '/'));
  ...
  // Registers a manager for every page type that doesn't already have one via `geoportal-custom-pages`,
  // `geoportal-pieces-pages`, etc. Invoked by `modulesReady`
  ...
  // Registers a manager for a specific page type that doesn't already have one via `geoportal-custom-pages`,
  // `geoportal-pieces-pages`, etc. Invoked by `modulesReady` via `registerGenericPageTypes` and
  // `manageOrphans`

  self.registerGenericPageType = function(type, callback) {
    var manager = self.geop.docs.getManager(type);
    if (manager) {
      return setImmediate(callback);
    }
    var typeName = type + '-auto-pages';
    self.geop.define(typeName, {
      extend: 'geoportal-custom-pages',
      name: type
    });
    return self.geop.create(typeName, {
      apos: self.geop
    }, function(err, manager) {
      if (err) {
        return callback(err);
      }
      self.geop.docs.setManager(type, manager);
      return callback(null);
    });
  };
  ...
  // bc wrapper for `geop.templates.append('contextMenu', helper)`.

  self.addAfterContextMenu = function(helper) {
    return self.geop.templates.append('contextMenu', helper);
  };
  ...
  self.addPermissions = function() {
    self.geop.permissions.add({
      value: 'admin-geoportal-page',
      label: 'Admin: Pages'
    });
    self.geop.permissions.add({
      value: 'edit-geoportal-page',
      label: 'Edit: Pages'
    });
  };
  ...
  self.addToAdminBar = function() {
    self.geop.adminBar.add(self.__meta.name, 'Pages', 'edit-geoportal-page');
  };

  // Returns the effective base URL for the given request.
  // If Geoportal's top-level `baseUrl` option is set, it is returned,
  ...
  // Geoportal cursors used to fetch Geoportal pages
  // consult this method, and it is extended by the optional
  // `geoportal-workflow` module to create correct absolute URLs
  // for specific locales.

  self.getBaseUrl = function(req) {
    return self.geop.baseUrl || '';
  };
  ...
  // be passed to `geop.modules['geoportal-jobs'].start()` on the rowser side to
  // monitor progress.
  //
  ...
  self.batchSimpleRoute = function(req, name, change) {
    ...
    var data = self.geop.schemas.newInstance(schema);
    return self.geop.schemas.convert(req, schema, 'form', req.body, data, function(err) {
      if (err) {
    ...
    function runJob() {
      return self.geop.modules['geoportal-jobs'].run(req, one, {
        labels: {
  ...
  self.allowedSchema = function(req, page, parentPage) {
    var schema = self.geop.docs.getManager(page.type).allowedSchema(req);
    ...
    function getLabel(name) {
      ...
      if (!label) {
        var manager = self.geop.docs.getManager(name);
        if (!manager) {
