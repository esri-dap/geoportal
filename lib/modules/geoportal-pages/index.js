...
// ```
//
// The `action` becomes a `data-geop-ACTIONGOESHERE` attribute on the
...
// check out the [geoportal-custom-pages](https://docs.geoportalcms.org/geoportal/modules/geoportal-custom-pages)
...
// ### `filters`
//
// Geoportal cursor filters applied when fetching the current page.**
...
// ```
//
// See the [geoportal-pages-cursor](https://docs.geoportalcms.org/geoportal/modules/geoportal-pages/server-geoportal-pages-cursor) type for additional
// cursor filters and options you might wish to configure, such as adding
// a `depth` option to `children`.
//
// ### `home`
//
// Geoportal populates the home page from `req.page._ancestors[0]` if possible.
// If not, Geoportal fetches the home page separately, using the same filters configured for
...
// ```
//
// In addition, if ancestors are not configured, Geoportal assumes you want the children
...
// ### `deleteFromTrash`
//
// If set to `true`, Geoportal offers a button in the
...
module.exports = {
  ...
  beforeConstruct: function(self, options) {
    ...
    ].concat(options.geop.docs.trashInSchema ? [
      {
        name: 'rescue',
        label: 'Rescue'
      }
  ...
  construct: function(self, options) {
    ...
    self.geop.define('geoportal-cursor', require('./lib/anyCursor.js'));

    // Add methods to cursors used specifically to fetch pages only
    self.geop.define('geoportal-pages-cursor', require('./lib/pagesCursor.js'));

    // When all modules are ready, invoke `registerGenericPageTypes` to register a manager
    // for any page type that doesn't already have one via `geoportal-custom-pages`,
    // `geoportal-pieces-pages`, etc.
    ...
    self.on('geoportal:migrate', 'parkAllPromisified', function() {
