...
geop.define('geoportal-pages-reorganize', {
  ...
  construct: function(self, options) {
    ...
    self.enhanceNode = function(node, $li) {
      ...
      if (!node.virtualTrashcan) {
        ...
        $li.find('.jqtree-element .geop-reorganize-title').before($checkbox);

        var $link;
        $link = $('<a class="geop-edit" target="_blank"></a>');
        $link.attr('data-node-id', node.id);

        $link.attr('data-edit', '1');
        $link.append('Settings');
        // add published indicator
        $li.find('.jqtree-element .geop-reorganize-controls--published').append('<span class="geop-reorganize-published geop-reorganize-published--' + node.published + '"></span>');

        $link.attr('href', '#');
        $li.find('.jqtree-element .geop-reorganize-controls--edit').append($link);
        ...
        if (node.publish) {
          ...
          $link = $('<a class="geop-delete"></a>');
          $link.attr('data-node-id', node.id);
          $link.attr('data-delete', '1');
          $link.attr('href', '#');
          $link.append('Move to Trash');
          $li.find('.jqtree-element .geop-reorganize-controls--trash').append($link);

          if (self.options.deleteFromTrash) {
            // "Destroy forever" button. CSS reveals when
            // already in the trash
            $link = $('<a class="geop-delete-from-trash"></a>');
            $link.attr('data-node-id', node.id);
    ...
    self.addControlGroupToNode = function(node, $li) {
      $li.find('.jqtree-element').append($('<span class="geop-reorganize-controls geop-reorganize-controls--published"></span>'));
      $li.find('.jqtree-element').append($('<span class="geop-reorganize-controls geop-reorganize-controls--edit"></span>'));
      $li.find('.jqtree-element').append($('<span class="geop-reorganize-controls geop-reorganize-controls--link"></span>'));
      $li.find('.jqtree-element').append($('<span class="geop-reorganize-controls geop-reorganize-controls--trash"></span>'));
    };

    self.indentLevel = function(node, $li) {
      // add page depth as data attr
      $li.attr('data-geop-reorganize-depth', node.getLevel());
      if (node.getLevel() !== 1) {
        $li.find('.geop-reorganize-title').css('padding-left', 15 * node.getLevel() + 'px');
      }
    };

    self.configureTitleContainer = function(node, $li) {
      var $titleContainer = $('<span class="geop-reorganize-title"></span>');
      $titleContainer.append($li.find('.jqtree-title'));
      if ($li.find('.jqtree-toggler').length === 0) {
        $titleContainer.addClass('geop-reorganize-title--no-toggle');
      } else {
    ...
    self.addVisitLink = function(node, $li) {
      ...
      var $link = $('<a class="geop-visit" target="_blank"></a>');
      $link.attr('data-node-id', node.id);
      ...
      $link.append('Link');
      $li.find('.jqtree-element .geop-reorganize-controls--link').append($link);
    };
    ...
    self.visit = function($node) {
      ...
      var tab = window.open(geop.prefix + node.slug, '_blank');
      tab.focus();
    };
    ...
    self.edit = function($node) {
      ...
      geop.create('geoportal-pages-editor-update', {
    ...
    self.delete = function($node) {
      ...
      if (!trash) {
        geop.notify('No trashcan.', { type: 'error', dismiss: true });
        return false;
      }

      geop.pages.trash(node.id, function(err, parentSlug, changed) {
        if (err) {
    ...
    self.deleteFromTrash = function($node) {
      ...
      geop.pages.deleteFromTrash(node.id, function(err, parentSlug) {
        if (err) {
          geop.notify('An error occurred.', { type: 'error', dismiss: true });
          return;
        }
    ...
    self.move = function(e) {
      var trashing;
      e.preventDefault();
      self.$el.find('.geop-reorganize-progress').fadeIn();
      var data = {
      ...
      return async.series([
        ...
        e.move_info.do_move();
        self.updateVirtualTrashcans();
        self.$el.find('.geop-reorganize-progress').fadeOut();
      });
      ...
      function trash(callback) {
        ...
        if (trashing) {
          return geop.pages.trash(node.id, callback);
        } else {
          return geop.pages.rescue(node.id, callback);
        }
      }
    ...
    self.reload = function(callback) {
      self.api('get-jqtree', {}, function(data) {
        if (data.status === 'ok') {
          if (geop.docs.trashInSchema) {
            // Remove the standard Geoportal trash can from the displayed tree,
    ...
    self.errorOnReload = function() {
      geop.notify('The server did not respond or you do not have the appropriate privileges.', { type: 'error' });
      self.hide();
    };

    self.errorOnMove = function() {
      geop.notify('You may only move pages you are allowed to publish. If you move a page to a new parent, you must be allowed to edit the new parent.', { type: 'error' });
    ...
    self.enableBatchOperations = function(callback) {
      ...
      self.link('geop-batch-operation', function($el, action) {
    ...
    self.reflectBatchOperation = function() {
      ...
      self.$batch.find('[data-geop-batch-operation-form]').removeClass('geop-active');
      self.$batch.find('[data-geop-batch-operation-form="' + val + '"]').addClass('geop-active');
      self.$batch.find('[data-geop-batch-operation]').addClass('geop-hidden');
      self.$batch.find('[data-geop-batch-operation="' + val + '"]').removeClass('geop-hidden');
      ...
      var $buttons = self.$batch.find('.geop-button');
      if (count) {
        $buttons.removeClass('geop-button--disabled');
      } else {
        $buttons.addClass('geop-button--disabled');
      }
    ...
    self.enableBatchOperation = function(batchOperation, callback) {
      ...
      batchOperation.handler = self['batch' + geop.utils.capitalizeFirst(geop.utils.camelName(batchOperation.name))];
      if (!batchOperation.schema) {
        return setImmediate(callback);
      }
      var data = geop.schemas.newInstance(batchOperation.schema);
      return geop.schemas.populate(
        self.$batch.find('[data-geop-batch-operation-form="' + batchOperation.name + '"]'),
    ...
    self.batchSimple = function(operationName, confirmationPrompt, options) {
      ...
      function convert(callback) {
        ...
        return geop.schemas.convert(
          self.$batch.find('[data-geop-batch-operation-form="' + operationName + '"]'),
          operation.schema,
          data,
          {},
          callback
      ...
      function save(callback) {
        ...
        return self.api(operation.route || operationName, data, function(result) {
          ...
          if (result.jobId) {
            var jobs = geop.modules['geoportal-jobs'];
