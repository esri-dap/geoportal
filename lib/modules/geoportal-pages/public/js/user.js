...
geop.define('geoportal-pages', {
  ...
  construct: function(self) {
    self.addLinks = function() {
      geop.ui.link('apos-insert', 'page', function() {
        geop.create('geoportal-pages-editor', { action: self.options.action });
      });
      geop.ui.link('apos-copy', 'page', function() {
        geop.create('geoportal-pages-editor-copy', { action: self.options.action });
      });
      geop.ui.link('apos-update', 'page', function() {
        self.pageSettings();
      });
      geop.ui.link('apos-reorganize', 'page', function() {
        self.reorganize();
      });
      geop.adminBar.link(self.__meta.name, function() {
        self.reorganize();
      });
      geop.ui.link('apos-trash', 'page', function() {
        self.trash(self.page._id, function(err, parentSlug, changed) {
          if (err) {
            geop.notify('A server error occurred.', { type: 'error', dismiss: true });
          } else {
            geop.ui.redirect(parentSlug);
          }
        });
      });
      geop.ui.link('apos-publish', 'page', function() {
        // fetch the page
        return self.api('fetch-to-update', { _id: self.page._id, type: self.page.type }, function(result) {
          if (result.status !== 'ok') {
            return geop.notify('An error occurred while publishing the page: ' + result.status, { type: 'error' });
          }
          // set to published
    ...
    self.pageSettings = function(options) {
      geop.create('geoportal-pages-editor-update', _.merge({}, options || {}, { action: self.options.action }));
    };

    // Display UI permitting the user to reorganize the page tree

    self.reorganize = function() {
      geop.create('geoportal-pages-reorganize',
    ...
    self.chooserModal = function(options) {
      return geop.create('geoportal-pages-reorganize',
        _.assign(
          {
