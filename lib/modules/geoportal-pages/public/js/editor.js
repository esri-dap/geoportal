...
geop.define('geoportal-pages-editor', {
  extend: 'geoportal-modal',
  source: 'editor',
  verb: 'insert',
  label: 'New Page',
  construct: function(self, options) {
    self.typeChoices = geop.pages.options.typeChoices;
    self.page = options.page || geop.pages.page;
    ...
    self.addTypeChangeHandler = function() {
      var $type = geop.schemas.findField(self.$el, 'type');
      ...
      return self.api(
        'fetch-to-' + self.verb,
        {
          ...
          if (result.status === 'notfound') {
            geop.utils.error('That page does not exist.');
            return callback('error');
          } else if (result.status !== 'ok') {
            geop.notify('An error occurred. Please try again.', { type: 'error' });
            return callback('error');
          }
    ...
    self.populate = function(data, callback) {
      return geop.schemas.populate(self.$el, self.schema, data, callback);
    };
    ...
    self.convert = function(page, callback) {
      return geop.schemas.convert(self.$el, self.schema, page, callback);
    };
      ...
      return async.series({
        ...
        save: function(callback) {
          return self.api(self.verb, { currentPageId: self.page._id, page: page }, function(_data) {
            data = _data;
            if (data.status !== 'ok') {
              geop.notify('An error occurred while creating the page: ' + data.status, { type: 'error' });
        ...
        redirect: function(callback) {
          if (self.options.redirectIfSamePage || (self.options.redirectIfSamePage === undefined)) {
            if (self.page._id === geop.pages.page._id) {
      ...
      return async.series({
        ...
        fetch: function(callback) {
          return self.api('fetch-to-' + self.verb, { _id: self.page._id, type: type }, function(result) {
            if (result.status === 'notfound') {
              geop.notify('That page does not exist.', { type: 'error' });
              return callback('error');
            } else if (result.status !== 'ok') {
              geop.notify('An error occurred. Please try again.', { type: 'error' });
              return callback('error');
        ...
        geop.emit('enhance', self.$el.find('[data-page-schema-fields]'));
        self.addTypeChangeHandler();
        self.type = type;
      });
    };
...
geop.define('geoportal-pages-editor-update', {
  extend: 'geoportal-pages-editor',
  verb: 'update',
  label: 'Edit Page Settings'
});

geop.define('geoportal-pages-editor-copy', {
  extend: 'geoportal-pages-editor',
  verb: 'copy',
  label: 'Copy Page'
});
