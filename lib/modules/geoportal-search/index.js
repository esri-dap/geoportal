// Implement sitewide search for Geoportal. Provides the
// `geoportal-search` page type for the `/search` page, which
// you should include in your "parked pages" if you wish
// to have one (see [geoportal-pages](https://docs.geoportalcms.org/geoportal/modules/geoportal-pages)).
//
...
// template of `geoportal-pages` contains this element:
//
// `<div data-geop-notfound-search-results></div>`
//
// Geoportal will attempt to locate relevant pages by feeding the component
...
// will work adequately if you have an Geoportal sitewide search page
...
// take an array argument and push new type names on it. `geoportal-pieces` modules
...
module.exports = {
  ...
  afterConstruct: function(self) {
    ...
    self.enableFilters();
    self.pushAssets();
    self.geop.tasks.add(self.__meta.name, 'index', self.indexTask);
  },
  ...
  construct: function(self, options) {
    ...
    self.determineTypes = function(callback) {
      self.types = self.options.types || _.pluck(self.geop.pages.typeChoices, 'name');
    ...
    self.renderRoute('get', 'suggest', function(req, res, next) {
      return self.suggest(req, self.geop.launder.string(req.query.q)).then(function(docs) {
        return next(null, {
          template: 'suggest',
          data: {
            docs: docs
    ...
    self.suggest = function(req, q) {
      return self.geop.docs.find(req, {}, { _url: 1, title: 1 })
    ...
    self.indexPage = function(req, callback) {
      ...
      var cursor = self.geop.docs.find(req, {})
      ...
      return async.series([ totalDocs, findDocs ], function(err) {
        ...
        if (self.geop.utils.isAjaxRequest(req)) {
      ...
      function findDocs(callback) {
        ...
        function getDocs(callback) {
          ...
          return async.eachSeries(_.keys(byType), getDocsOfType, function(err) {
            ...
            docs = self.geop.utils.orderById(_.pluck(idsAndTypes, '_id'), docs);
            return callback(null);
          });
        }
    ...
    self.indexDoc = function(req, doc) {
      ...
      var titleSortified = self.geop.utils.sortify(doc.title);
    ...
    // Implements the `geoportal-search:index` task, which re-indexes all pages.
    ...
    self.indexTask = function(apos, argv, callback) {
      ...
      return self.geop.migrations.eachDoc({}, _.partial(self.indexTaskOne, req), callback);
    };

    // Indexes just one document as part of the implementation of the
    // `geoportal-search:index` task. This isn't the method you want to
    // override. See `indexDoc` and `getSearchTexts`

    self.indexTaskOne = function(req, doc, callback) {
      self.indexDoc(req, doc);
      return self.geop.docs.db.update({ _id: doc._id }, doc, callback);
    };
    ...
    self.getSearchTexts = function(doc) {
      ...
      self.geop.emit('docSearchIndex', doc, texts);

      // Areas can be schemaless so find them automatically
      self.geop.areas.walk(doc, function(area, dotPath) {
        ...
        _.each(area.items, function(item) {
          var manager = self.geop.areas.getWidgetManager(item.type);
          if (!manager) {
            self.geop.areas.warnMissingWidgetType(item.type);
            return;
          }
    ...
    // through geop.utils.sortify to eliminate unwanted characters and case differences

    self.boilTexts = function(texts) {
      var text = _.reduce(texts, function(memo, text) {
        return memo + ' ' + text.text;
      }, '');
      text = self.geop.utils.sortify(text);
      return text;
    };
