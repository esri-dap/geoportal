geop.define('geoportal-video-fields', {
  ...
  construct: function(self, options) {
    ...
    self.convert = function(data, name, $field, $el, field, callback) {
      ...
      var $player = $fieldset.find('[data-geop-video-player]');
      data[name] = {
        url: $field.val(),
    ...
    self.preview = function($fieldset, value, field) {
      ...
      $fieldset.find('[data-geop-video-error]').hide();
      return geop.oembed.queryAndPlay(
        $fieldset.find('[data-geop-video-player]'),
        {
          url: value && value.url,
          ...
          if (err) {
            $fieldset.find('[data-geop-video-error="' + err + '"]').show();
