module.exports = function(self, options) {
  ...
  self.addFieldType = function() {
    self.geop.schemas.addFieldType({
      name: self.name,
      ...
      converters: {
        form: function(req, data, name, object, field, callback) {
          ...
          object[name] = {
            url: self.geop.launder.url(data[name].url),
            title: self.geop.launder.string(data[name].title),
            thumbnail: self.geop.launder.url(data[name].thumbnail)
          };
        ...
        string: function(req, data, name, object, field, callback) {
          ...
          // and thumbnail here
          object[name] = {
            url: self.geop.launder.url(data[name])
          };
          return setImmediate(callback);
        }
      },
