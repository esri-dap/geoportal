...
//
// Geoportal is fully initialized before your task is run, except that it does
...
module.exports = {
  ...
  singletonWarningIfNot: 'geoportal-tasks',

  afterConstruct: function(self) {
    self.geop.on('runTask', self.run);
  },
  ...
  construct: function(self, options) {
    ...
    // For use when you wish to execute an Geoportal command line task from your code and continue,
    ...
    //
    // Examples (assume `products` extends `geoportal-pieces`):
    //
    // `self.geop.tasks.invoke('geoportal-users:add', [ 'admin', 'admin' ]).then(function() { ... })`
    //
    // `self.geop.tasks.invoke('products:generate', { total: 20 }).then(function() { ... })`
    //
    ...
    //
    // During the execution of the task, `self.geop.argv` will have a new,
    // temporary value to accommodate tasks that inspect this property directly
    // rather than examining their `argv` argument. `self.geop.argv` will be
    ...
    self.invoke = function(name, args, options, callback) {
      ...
      function body(callback) {
        ...
        self.geop.argv = argv;
        var promise = task.callback(self.geop, argv, after);
        ...
        function after(err) {
          self.geop.argv = aposArgv;
          return callback(err);
        }
      }
    };

    // Add a command line task to Geoportal. It is easiest to invoke this
    ...
    // If the callback is invoked with `null`, Geoportal will exit quietly
    ...
    // Call `self.geop.tasks.getReq()` to get a `req` object with
    // unlimited admin permissions. Use `self.geop.tasks.getAnonReq()` to get
    ...
    // want `geop.tasks.invoke` (see above).
    //
    // This method is invoked by Geoportal to execute the task specified
    ...
    self.run = function() {
      ...
      var task;
      var cmd = self.geop.argv._[0];
      if (!cmd) {
        throw new Error('geop.tasks.run invoked but there is no command line argument to serve as a task name, should never happen');
      }

      if (cmd === 'help') {

        // list all tasks
        if (self.geop.argv._.length === 1) {
          self.usage();
        }

        // help with specific task
        if (self.geop.argv._.length === 2) {
          task = self.find(self.geop.argv._[1]);
          if (!task) {
      ...
      var promise = task.callback(self.geop, self.geop.argv, afterTask);
      if (promise && promise.then) {
        return promise.then(function() {
    ...
    // with sufficient functionality to implement Geoportal's
    ...
    self.getReq = function(properties) {
      var req = {
        ...
        browserCall: self.geop.app && self.geop.app.request.browserCall,
        getBrowserCalls: self.geop.app && self.geop.app.request.getBrowserCalls,
        query: {},
        url: '/',
        session: {}
      };
      ...
      self.geop.modules['geoportal-express'].addAbsoluteUrlsToReq(req);
      return req;
    };
