...
module.exports = {
  ...
  construct: function(self, options) {
        ...
        function attempt(callback) {
          ...
          function insert(callback) {
            return self.db.insert({
              ...
              unique: self.geop.utils.generateId()
            }, callback);
          }
        }
    ...
    self.withLock = function(name, fn, callback) {
      ...
      function body(callback) {
        ...
        return async.series([ lock, fnWrapper ], function(err) {
          if (locked) {
            return self.geop.locks.unlock(name, function(_err) {
        ...
        function lock(callback) {
          return self.geop.locks.lock(name, function(err) {
            if (err) {
              return callback(err);
            }
            locked = true;
    ...
    self.ensureCollection = function(callback) {
      self.db = self.geop.db.collection('aposLocks');
