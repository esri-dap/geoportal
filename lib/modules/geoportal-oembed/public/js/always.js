...
geop.define('geoportal-oembed', {
  construct: function(self, options) {
    ...
    // Adds the geop-oembed-busy class to $el during the interim.
    //
    ...
    self.queryAndPlay = function($el, options, callback) {
      $el.removeClass('geop-oembed-invalid');
      $el.addClass('geop-oembed-busy');
      if (!options.url) {
      ...
      return self.query(options, function(err, result) {
        ...
        $el.removeClass('geop-oembed-busy');
        return self.play($el, result, callback);
      });
      function fail(err) {
        $el.removeClass('geop-oembed-busy');
        $el.addClass('geop-oembed-invalid');
    ...
    // through Geoportal's built-in proxy. options.url must be set to the
