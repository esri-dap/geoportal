...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    // Create the browser-side object `geop.oembed` for convenient oembed queries
    // and display of oembed responses. Called by `afterConstruct`.

    self.pushCreateSingleton = function() {
      self.geop.push.browserCall('always', 'geop.create("geoportal-oembed", ?)', { action: self.action });
    };
    ...
    self.createRoutes = function() {
      ...
      self.route('get', 'query', function(req, res) {
        var url = self.geop.launder.string(req.query.url);
        var options = {
          alwaysIframe: self.geop.launder.boolean(req.query.alwaysIframe),
          iframeHeight: self.geop.launder.integer(req.query.iframeHeight),
          neverOpenGraph: self.geop.launder.boolean(req.query.neverOpenGraph)
        };
