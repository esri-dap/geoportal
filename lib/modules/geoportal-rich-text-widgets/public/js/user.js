geop.define('geoportal-rich-text-widgets', {
  construct: function(self, options) {
    ...
    self.edit = function(data, options, save) {
      ...
      return save(data, function(err, $widget) {
        if (err) {
          ...
          geop.notify('An error occurred saving the text.', { type: 'error' });
          return;
        }
    ...
    self.startEditing = function($widget) {
      ...
      self.startAutosavingAreaThen($widget, function() {
        $widget.removeClass('geop-empty');
        if ($widget.data('editor')) {
          $widget.data('editor').start();
          return;
        }
        var options = geop.areas.getWidgetOptions($widget);
        options.$widget = $widget;
        var instance = geop.create('geoportal-rich-text-widgets-editor', options);
