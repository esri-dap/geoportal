// `geoportal-pieces-widgets` provides widgets that display display pieces of a
...
// ### `loadManyById`
//
// If `true` (the default), Geoportal will take all the widgets passed to the `load` method for which pieces
...
module.exports = {
  ...
  beforeConstruct: function(self, options) {
    ...
    self.pieces = options.geop.modules[self.piecesModuleName];
    if (!self.pieces) {
      if (!options.piecesModuleName) {
        throw new Error('The module ' + self.__meta.name + ' extends geoportal-pieces-widgets, but the piecesModuleName option is not set, and we can\'t guess it from the name of your module.');
      } else {
  ...
  construct: function(self, options) {
    ...
    // Load the appropriate pieces for each widget in the array. Geoportal will try to feed
    ...
    self.load = function(req, widgets, callback) {
      // Since we are overriding the default load method of `geoportal-widgets`, make
    ...
    // Geoportal to work efficiently when a page contains many pieces widgets in an
    ...
    self.loadOtherJoins = function(req, widgets, callback) {
      ...
      return self.geop.schemas.join(req, schema, widgets, undefined, callback);
    };
    ...
    self.orderPiecesForWidget = function(widget) {
      ...
      widget._pieces = self.geop.utils.orderById(widget.pieceIds, widget._pieces, key);
    };
