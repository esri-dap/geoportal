// The admin bar module implements Geoportal's admin bar at the top of the screen. Any module
...
// particular permissions. [geoportal-pieces](../geoportal-pieces/README.md) automatically
...
module.exports = {
  ...
  // that provides `geop.adminBar.link`. Most server-side initialization happens
  ...
  construct: function(self, options) {
    ...
    //
    // The name argument becomes the value of the `data-geop-admin-bar-item`
    // attribute of the admin bar item.
    //
    // `permission` should be a permission name such as `admin`
    // (the user must be a full admin) or `edit-geoportal-event`
    ...
    //
    // ```
    // self.geop.adminBar.add(self.__meta.name, self.pluralLabel, 'edit')
    // ```
    //
    // If you have an `events` module that subclasses pieces, then this
    // creates an admin bar item with a data-geop-admin-item="events" attribute.
    //
    // Browser side, you can call `geop.adminBar.link('item-name', function() { ...})`
    ...
    self.addHelpers({
      ...
      output: function() {
        ...
        var user = self.geop.templates.contextReq.user;
        if (!user) {
          return '';
        }
        var items = _.filter(self.items, function(item) {
          return self.itemIsVisible(self.geop.templates.contextReq, item);
        });
        if (!items.length) {
          return '';
        }
    ...
    self.pushAssets = function() {
      ...
      var closeDelay = (self.options.closeDelay || self.options.closeDelay === 0 ? self.geop.launder.integer(self.options.closeDelay) : null);
    ...
    self.itemIsVisible = function(req, item) {
      ...
      return self.geop.permissions.can(req, item.permission);
    };
  }
};
