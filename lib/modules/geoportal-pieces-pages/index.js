// `geoportal-pieces-pages` implements "index pages" that display pieces of a
...
// that extend `geoportal-pieces`.
//
// To learn more and see complete examples, see:
//
// [Reusable content with pieces](https://docs.geoportalcms.org/geoportal/tutorials/getting-started/reusable-content-with-pieces)
//
// ## Options
//
// ### `piecesFilters`
...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.pieces = self.geop.modules[self.piecesModuleName];
    self.piecesCssName = self.geop.utils.cssName(self.pieces.name);
    ...
    // `geoportal-pieces-cursor` in your subclass of
    // `geoportal-pieces`.
    ...
    self.indexPage = function(req, callback) {
      ...
      if (self.pieces.contextual) {
        ...
        }).concat(self.geop.pages.options.contextMenu);
      }
    ...
    // invoking `self.showPage`. See [geoportal-custom-pages](https://docs.geoportalcms.org/geoportal/modules/geoportal-custom-pages)
    ...
    self.buildUrl = function(page, piece) {
      ...
      return self.geop.utils.addSlashIfNeeded(page._url) + piece.slug;
    };

    // Make the browser-side `apos` object aware of the current
    // in-context piece, as `geop.contextPiece`. Just enough to
    ...
    self.pushContextPiece = function(req) {
      if (self.pieces.options.contextual && req.data.piece) {
        req.browserCall('geop.contextPiece = ?', _.pick(req.data.piece, '_id', 'title', 'slug', 'type'));
      }
    };

    var superPageBeforeSend = self.pageBeforeSend;

    // Calls `pushContextPiece` to make `geop.contextPiece` available
    // in the browser
    ...
    // Aliased as the `addUrls` method of [geoportal-pieces](https://docs.geoportalcms.org/geoportal/modules/geoportal-pieces), which
    // is invoked by the `addUrls` filter of [geoportal-cursor](https://docs.geoportalcms.org/geoportal/modules/geoportal-docs/server-geoportal-cursor).

    self.addUrlsToPieces = function(req, results, callback) {
      var pieceName = self.pieces.name;
      return async.series({
        getIndexPages: function(callback) {
          if (req.geopParentPageCache && req.geopParentPageCache[pieceName]) {
            return setImmediate(callback);
          }
          return self.findForAddUrlsToPieces(req)
            .toArray(function(err, pages) {
              ...
              if (!req.geopParentPageCache) {
                req.geopParentPageCache = {};
              }
              req.geopParentPageCache[pieceName] = pages;
              return callback(null);
            }
            );
        }
        ...
        _.each(results, function(piece) {
          var parentPage = self.chooseParentPage(req.geopParentPageCache[pieceName], piece);
          if (parentPage) {
    ...
    // Geoportal will supply a `count` property for each distinct value,
