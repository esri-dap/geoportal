// Provide a way to group [geoportal-users](https://docs.geoportalcms.org/geoportal/modules/geoportal-users) together
// and assign permissions to them. This module is always active "under the hood," even if
// you take advantage of the `groups` option of `geoportal-users` to skip a separate
...
// `geoportal-users` module. That option configures a hardcoded list of groups
...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.setPermissionsChoices = function() {
      ...
      permissions.choices = self.geop.permissions.getChoices();
    };

    self.addToAdminBar = function() {};

    // Adds an admin bar button if and only if the `geoportal-users` module
    // is not using its `groups` option for simplified group administration.

    self.addToAdminBarIfSuitable = function() {
      if (self.geop.users.options.groups) {
        // Using the simplified group choice menu, so
        // there is no managing of groups by the end user
      } else {
        self.geop.adminBar.add(self.__meta.name, self.pluralLabel, self.isAdminOnly() ? 'admin' : ('edit-' + self.name), { after: 'geoportal-users' });
      }
    };

    self.enableAddGroupTask = function() {
      self.geop.tasks.add(self.__meta.name,
        'add',
        'Usage: node app geoportal-groups:add groupname permission1 permission2...\n\nTo list available permissions, run:\n\nnode app geoportal-permissions:list',
        function(apos, argv) {
          ...
          return Promise.try(function() {
            _.each(permissions, function(permission) {
              if (!_.contains(_.pluck(self.geop.permissions.getChoices(), 'value'), permission)) {
                throw 'The permission ' + permission + ' does not exist. Use node app geoportal-permissions:list to list available permissions.';
              }
            });
