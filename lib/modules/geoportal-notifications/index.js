...
// within the Geoportal admin UI. Notifications may be triggered
// either on the browser or the server side, via `geop.notice`.
//
// ## Options
//
...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.apiRoute('post', 'trigger', function(req, res, next) {
      ...
      self.geop.utils.readOnlySession(req);
      var type = self.geop.launder.select(req.body.type, [ 'error', 'warn', 'success', 'info' ], 'info');
      var message = self.geop.launder.string(req.body.message);
      var strings = self.geop.launder.strings(req.body.strings);
      var dismiss = self.geop.launder.integer(req.body.dismiss);
      var pulse = self.geop.launder.boolean(req.body.pulse);
    ...
    // `/modules/geoportal-global/poll-notifications`. It is implemented
    ...
    self.expressMiddleware = function(req, res, next) {
      ...
      var start = Date.now();
      var displayingIds = self.geop.launder.ids(req.body.displayingIds);
      return attempt();
    ...
    self.ensureCollection = function(callback) {
      self.db = self.geop.db.collection('aposNotifications');
      return callback(null);
    };
