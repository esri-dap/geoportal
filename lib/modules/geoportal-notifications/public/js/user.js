geop.define('geoportal-notifications', {

  extend: 'geoportal-context',

  afterConstruct: function(self) {
    geop.notify = self.trigger;

    geop.on('modalStackPop', self.reparentContainer);
    geop.on('modalStackPush', self.reparentContainer);
  ...
  construct: function(self, options) {
    ...
    //
    // This method is aliased as `geop.notify` for convenience.
    //
    ...
    self.enable = function() {
      // wait for geop.user to be testable
      setImmediate(go);
      function go() {
        if (!(geop.user && geop.user._id)) {
        ...
        function poll() {
          self.api('poll-notifications', {
            ...
            _.each(data.dismissed, function(id) {
              var $notification = $('[data-geop-notification-id="' + id + '"]');
    ...
    // You want `geop.notify`, not this method, unless you are
    ...
    self.display = function(notification) {
      ...
      if (notification.type) {
        $notification.addClass('geop-notification--' + notification.type);
      }

      if (notification.pulse) {
        $notification.addClass('geop-notification--pulse');
      }
      ...
      $notification.attr('data-geop-notification-id', notification._id);

      if (notification.dismiss) {
        $notification.attr('data-geop-notification-dismiss', notification.dismiss);
    ...
    self.createContainer = function() {
      self.$container = $('<div class="geop-notification-container" data-geop-notification-container role="alert"></div>');
      self.$container.hide();
    ...
    self.addToContainer = function($notification) {
      ...
      setTimeout(function() {
        $notification.removeClass('geop-notification--hidden');
      }, 100);
    };

    self.dismiss = function($notification, fromServer) {

      var _id = $notification.attr('data-geop-notification-id');
      ...
      $notification.addClass('geop-notification--hidden');

      setTimeout(function() {
        $notification.remove();
        if (!self.$container.find('[data-geop-notification]').length) {
          self.$container.hide();
        }
      }, 300);
    };
