...
geop.define('geoportal-polymorphic-manager-manager-modal', {
  ...
  construct: function(self, options) {
    ...
    self.enableChooserViews = function() {
      _.each(self.options.chooser.field.withType, function(type) {
        var view = geop.docs.getManager(type).getTool('manager-modal', {
      ...
      self.$el.find('[data-tab-button]').removeClass('geop-active');
      self.$el.find('[data-tab-button]:first').addClass('geop-active');
      self.$el.on('click', '[data-tab-button]', function() {
        var tabName = $(this).attr('data-tab-button');
        self.$el.find('[data-tab-button]').removeClass('geop-active');
        $(this).addClass('geop-active');

        self.$el.find('[data-tab]').removeClass('geop-chooser-tab--active');
        self.$el.find('[data-tab="' + tabName + '"]').addClass('geop-chooser-tab--active');
        return false;
      });
      self.hideOthers();
    };
    ...
    self.enableChooser = function(callback) {
      return self.parentChooser.clone(
        ...
        function(err, chooser) {
          ...
          geop.on('pieceInserted', self.chooser.pieceInsertedListener);
          return callback(null);
        }
      );
    };
    ...
    self.afterHide = function() {
      geop.off('pieceInserted', self.chooser.pieceInsertedListener);
    ...
    self.decorateManager = function(manager, options) {
      ...
      manager.beforeShow = function(callback) {
        manager.$el.find('.geop-chooser').remove();
