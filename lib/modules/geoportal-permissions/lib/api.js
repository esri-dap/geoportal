...
module.exports = function(self, options) {
  ...
  // allow file uploads by anonymous users for geoportal-moderator.
  //
  ...
  self._check = function(req, action, event, _true, _false, object, newObject, then) {
    ...
    function filter(response) {
      ...
      self.geop.emit(event, req, action, object, info);
  ...
  // the generic `geoportal-page`, because pages can change type.
  ...
  self.getEffectiveTypeName = function(type) {
    ...
    if (self.geop.instanceOf(manager, 'geoportal-custom-pages')) {
      return 'geoportal-page';
    }
    return type;
  };
  ...
  //
  // Used internally to implement self.geop.permissions.criteria().
