geop.define('geoportal-array-editor-modal', {
  ...
  construct: function(self, options) {
    ...
    self.load = function() {
      self.$arrayItems = self.$el.find('[data-geop-array-items]');
      self.$arrayItem = self.$el.find('[data-geop-array-item]');
    ...
    self.addSaveHandler = function() {
      self.$save = self.$el.parents('[data-modal]').find('[data-geop-save-array-items]');
    ...
    self.addToItems = function() {
      var item = geop.schemas.newInstance(self.field.schema);
      _.assign(item, {
    ...
    self.populateItem = function() {
      var $form = self.$arrayItem.find('[data-geop-form]');
      geop.schemas.populate($form, self.field.schema, self.arrayItems[self.active], function(err) {
        if (err) {
          geop.utils.error(err);
        }
        self.refresh();
        if (self.errorPath && (self.errorPath.length > 1)) {
          geop.schemas.returnToError($form, self.field.schema, self.errorPath.slice(1), self.error, function(err) {
            if (err) {
              geop.utils.error(err);
            }
          });
    ...
    self.saveItemState = function(callback) {
      ...
      geop.schemas.convert(self.$arrayItem.find('[data-geop-form]'), self.field.schema, self.arrayItems[self.active], function(err) {
        if (err) {
          return callback(err);
        }
        self.setItemTitles();
    ...
    self.bindClickHandlers = function() {
      self.$el.on('click', '[data-geop-array-items-trigger]', function(e) {
        e.stopPropagation();
        e.preventDefault();
        var $trigger = $(this);

        var index = $trigger.data('geop-array-items-trigger');
      ...
      if (options.field.readOnly) {
        ...
        self.$el.children().find('[data-geop-add-array-item]').remove();
        self.$el.parents('[data-modal]').find('[data-geop-save-array-items]').remove();
      } else {
        self.$el.on('click', '[data-geop-add-array-item]', function(e) {
          e.stopPropagation();
          e.preventDefault();
          self.saveItemState(function(err) {
            if (err) {
        ...
        self.$el.on('click', '[data-geop-move-array-item]', function(e) {
          e.stopPropagation();
          e.preventDefault();
          var $el = $(this);
          ...
          self.saveItemState(function(err) {
            ...
            var direction = $el.data('geop-move-array-item');
        ...
        self.$el.on('click', '[data-geop-delete-array-item]', function(e) {
          e.stopPropagation();
          e.preventDefault();
          ...
          // before this one.
          if (self.active === parseInt($self.closest('[data-geop-array-items-trigger]').attr('data-geop-array-items-trigger'))) {
    ...
    self.limit = function() {
      ...
      if (self.arrayItems.length >= self.field.limit) {
        self.$el.find('[data-geop-add-array-item]').hide();
        self.$el.find('[data-geop-array-limit-reached]').show();
      } else {
        self.$el.find('[data-geop-add-array-item]').show();
        self.$el.find('[data-geop-array-limit-reached]').hide();
      }
    };
    ...
    self.required = function() {
      ...
      if (self.arrayItems.length) {
        ...
        self.$save.removeClass('geop-button--disabled');
      } else {
        self.$save.attr('disabled', '');
        self.$save.addClass('geop-button--disabled');
      }
    };
