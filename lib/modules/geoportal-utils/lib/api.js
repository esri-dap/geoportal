...
module.exports = function(self, options) {
  ...
  // An id for this particular Geoportal instance that should be
  // unique even in a multiple server environment.
  self.geop.pid = self.generateId();
  ...
  // "search" option to geop.get and everything layered on top of it).
  ...
  // an unrealistic restriction on geoportal doc schemas.
  ...
  // `req.query.xhr` is set to emulate it, or `req.query.geop_refresh` has
  // been set by Geoportal's content refresh mechanism).

  self.isAjaxRequest = function(req) {
    return (req.xhr || req.query.xhr) && (!req.query.geop_refresh);
  };
  ...
  //
  // `geop.utils.bless(req, options, 'widget', widget.type)`
  //
  // ON A LATER AJAX REQUEST TO THE render-widget ROUTE
  //
  // `if (geop.utils.isBlessed(req, options, 'widget', widget.type)) { /* It's safe! */ }`
  //
  ...
  self.bless = function(req, options /* , arg2, arg3... */) {
    ...
    req.session.geopBlessings = req.session.geopBlessings || {};
    req.session.geopBlessings[hash] = true;
  };

  // See geop.utils.bless. Checks whether the given set of arguments
  // (everything after `req`) has been blessed in the current session.

  self.isBlessed = function(req, options /* , arg2, arg3... */) {
    var hash = self.hashBlessing(Array.prototype.slice.call(arguments, 1));
    return req.session.geopBlessings && _.has(req.session.geopBlessings, hash);
  };
  ...
  self.enableLogger = function() {
    self.logger = self.options.logger ? self.options.logger(self.geop) : require('./logger.js')(self.geop);
  };
  ...
  // `console.warn` documentation.
  //
  // The intention is that `geop.utils.warn` should be
  ...
  // Identical to `geop.utils.warn`, except that the warning is
  ...
  // Identical to `geop.utils.warnDev`, except that the warning is
  ...
  self.warnDevOnce = function(name, msg) {
    var always = self.geop.argv['all-' + name];
  ...
  //
  // Profiler modules such as `geoportal-profiler` override this
  ...
  // However, the `geoportal-profiler` module extends them to invoke it,
  ...
  // with high-resolution accuracy. Used by geop.utils.profile.
