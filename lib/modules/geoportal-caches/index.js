...
module.exports = {
  ...
  singletonWarningIfNot: 'geoportal-caches',
  ...
  construct: function(self, options) {
    ...
    // var myCache = self.geop.caches.get('weather-data');
    //
    ...
    self.getCollection = function(callback) {
      return self.geop.db.collection('aposCache', function(err, collection) {
        if (err) {
          return callback(err);
        }
    ...
    self.addClearCacheTask = function() {
      self.geop.tasks.add('geoportal-caches', 'clear',
        'Usage: node app geoportal-caches:clear cachename cachename2...\n\n' +
        'Clears caches by name. If you are using geop.caches in your own code you will\n' +
        'know the cache name. Standard caches include "geoportal-migrations",\n' +
