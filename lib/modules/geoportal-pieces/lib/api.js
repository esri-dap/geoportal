...
module.exports = function(self, options) {
  ...
  // of geoportal-pieces often extend this to remove more default filters
  ...
  self.requirePiece = function(req, res, next) {
    var id = self.geop.launder.id(req.body._id);
  ...
  self.requirePieceEditorView = function(req, res, next) {
    ...
    if (!self.geop.permissions.can(req, 'edit-' + self.name)) {
  ...
  self.list = function(req, options, callback) {
    ...
    if (options.filters && options.filters.sortColumn) {
      ...
      var direction = self.geop.launder.select(options.filters.sortColumn.direction, [ '1', '-1', '1' ]);
    ...
    return async.series({
      ...
      populateFilters: function(callback) {
        ...
        // for the `piecesFilters` option of `geoportal-pieces-pages`
  ...
  self.insert = function(req, piece, options, callback) {
    ...
    function body(callback) {
      ...
      if ((options.permissions !== false) &&
        (!self.geop.permissions.can(req, 'admin-' + self.name))) {
      ...
      function insert(callback) {
        self.geop.docs.insert(req, piece, options, callback);
      }
  ...
  self.update = function(req, piece, options, callback) {
    ...
    function body(callback) {
      ...
      function update(callback) {
        self.geop.docs.update(req, piece, options, callback);
      }
  ...
  self.trash = function(req, id, callback) {
    ...
    function body(callback) {
      if (self.geop.permissions.can(req, 'admin-' + self.name)) {
        // This user has blanket permission for this piece type
        permission = false;
      }
      return self.geop.docs.trash(req, id, {
  ...
  self.rescue = function(req, id, callback) {
    ...
    function body(callback) {
      ...
      return self.geop.docs.rescue(req, id, {
        permission: permission
      }, function(err, piece) {
        if (err) {
          return callback(err);
  ...
  self.convert = function(req, piece, callback) {
    return self.geop.schemas.convert(req, self.allowedSchema(req), 'form', req.body, piece, callback);
  };

  // Invoked after geop.schemas.convert by the `insert` and
  // `update` routes
  ...
  self.addToAdminBar = function() {
    self.geop.adminBar.add(self.__meta.name, self.pluralLabel, self.getEditPermissionName());
  };

  // Add `._url` properties to the given pieces, if possible.
  // The default implementation does nothing, however
  // [geoportal-pieces-pages](https://docs.geoportalcms.org/geoportal/modules/geoportal-pieces-pages) will
  // call `setAddUrls` to point to [its own `addUrlsToPieces` method](https://docs.geoportalcms.org/geoportal/modules/geoportal-pieces-pages#addurlstopieces-req-results-callback).
  ...
  // Called by [geoportal-pieces-pages](https://docs.geoportalcms.org/geoportal/modules/geoportal-pieces-pages) to
  ...
  // See [the `addUrlsToPieces` method of `geoportal-pieces-pages`](https://docs.geoportalcms.org/geoportal/modules/geoportal-pieces-pages#addurlstopieces-req-results-callback).
  ...
  // by the optional `geoportal-pieces-rest-api` module.
  //
