...
module.exports = function(self, options) {
  ...
  self.createRoutes = function() {
    ...
    self.route('post', 'insert-via-upload', self.geop.attachments.middleware.canUpload, self.geop.middleware.files, self.routes.insertViaUpload);
  };
  ...
  self.routes.list = function(req, res) {
    ...
    return async.series({
      ...
      if (options.format === 'managePage') {
        ...
        var viewTemplate = 'manage' + self.geop.utils.capitalizeFirst(view) + 'View';
        results = {
  ...
  self.routes.permissions = function(req, res) {
    ...
    const permissions = {};
    return self.geop.schemas.convert(req, schema, 'form', req.body, permissions, function(err) {
      if (err) {
        self.geop.utils.error(err);
  ...
  self.routes.trash = function(req, res) {
    ...
    if (req.body.job) {
      // New way
      return self.geop.modules['geoportal-jobs'].run(req, function(req, id, callback) {
    ...
    if (Array.isArray(req.body.ids)) {
      ids = self.geop.launder.ids(req.body.ids);
    } else {
      ids = [ self.geop.launder.id(req.body._id) ];
    }
  ...
  self.routes.insertViaUpload = function(req, res) {
    ...
    if (!field) {
      self.geop.utils.error('The pieces module ' + self.options.__meta.name + ' has insertViaUpload turned on but the schema does not contain a field of type attachment.');
      res.statusCode = 404;
      return res.send('notfound');
    }
    ...
    function acceptFile(callback) {
      return self.geop.attachments.insert(req, file, function(err, _attachment) {
        if (err) {
          return callback(err);
        }
        attachment = _attachment;
        if (!self.geop.attachments.acceptableExtension(field, attachment)) {
