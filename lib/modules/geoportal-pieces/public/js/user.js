...
geop.define('geoportal-pieces', {
  ...
  construct: function(self, options) {
    ...
    self.clickHandlers = function() {
      ...
      geop.ui.link('apos-manage', self.name, function($button, _id) {
        self.manage();
      });
      geop.ui.link('apos-edit', self.name, function($button, _id) {
        var hint = $button.attr('data-apos-edit-hint');
        self.edit(_id, { hint: hint });
      });
      geop.ui.link('apos-rescue', self.name, function($button, _id) {
        self.rescue(_id);
      });
      geop.ui.link('apos-create', self.name, function($button) {
        self.create();
      });
      geop.ui.link('apos-publish', self.name, function($button, id) {
        var piece = { _id: id };
      ...
      geop.ui.link('apos-versions', self.name, function($button, id) {
        geop.versions.edit(id, function() {
          geop.change(self.name);
        });
      });
      geop.ui.link('apos-trash', self.name, function($button, id) {
