...
geop.define('geoportal-pieces-manager-modal', {
  ...
  construct: function(self, options) {
    ...
    self.beforeShow = function(callback) {
      self.$manageView = self.$el.find('[data-geop-manage-view]');
    ...
    self.enableFilters = function() {
      ...
      _.each(self.filters, function(filter) {
        ...
        self.link('geop-' + filter.name, function($el, value) {
    ...
    self.enableInsertViaUpload = function() {
      ...
      self.$controls.on('click', '[data-geop-upload-' + self.options.name + ']', function() {
      ...
      function feedback(data) {
        if (getUploader().fileupload('active') <= 1) {
          if (self.insertViaUploadStatus.errors) {
            geop.notify('Some files were damaged, of an unsuitable type or too large to be uploaded.', { type: 'error' });
          } else if (self.insertViaUploadStatus.unsuitable) {
            geop.notify('Some uploaded files were unsuitable for the current placement.', { type: 'error' });
          } else {
      ...
      function busy(state) {
        geop.ui.globalBusy(state);
      }

      function getUploader() {
        return self.$controls.find('[data-geop-uploader-' + self.options.name + ']');
      }
    };
    ...
    self.reflectSort = function() {
      ...
      $columns.removeClass('geop-manage-column--forward')
        .removeClass('geop-manage-column--backward')
        .removeClass('geop-manage-column--next-forward')
        .removeClass('geop-manage-column--next-backward')
        .removeClass('geop-manage-column--next-none');
      ...
      if (self.sort) {
        ...
        if (self.sort.direction === '1') {
          $current.addClass('geop-manage-column--forward');
        } else {
          $current.addClass('geop-manage-column--backward');
        }
      ...
      function indicateNext($column, direction) {
        if (direction === '1') {
          $column.addClass('geop-manage-column--next-forward');
        } else if (direction === '-1') {
          $column.addClass('geop-manage-column--next-backward');
        } else {
          $column.addClass('geop-manage-column--next-none');
        }
      }
    };

    self.enableChooseViews = function() {
      self.link('geop-choose-manage-view', function($el, value) {
        self.viewName = value;
        self.refresh();
      });
    };
    ...
    self.enableCheckboxEvents = function() {
      ...
      self.$el.on('click', '.geop-field-input-checkbox-indicator', function (e) {
        var box = $(this).siblings('.geop-field-input-checkbox')[0];
    ...
    self.shrinkGrid = function() {
      self.$el.find('.geop-modal-footer').css('bottom', '72px');
      self.$el.find('.geop-modal-body, .geop-chooser').css('height', 'calc(100% - 144px)');
    };

    self.growGrid = function() {
      self.$el.find('.geop-modal-footer').css('bottom', '0px');
      self.$el.find('.geop-modal-body, .geop-chooser').css('height', 'calc(100% - 72px)');
    };
    ...
    self.reflectSelectEverything = function () {
      ...
      if ($pieces.length === $checked.length) {
        ...
        self.shrinkGrid();
        if (!$selectEverything.hasClass('geop-active')) {
        ...
        self.growGrid();
        $selectEverything.removeClass('geop-active');
    ...
    self.clearChoices = function() {
      self.choices = [];
      self.$el.removeClass('geop-manager-modal--has-choices');
      self.$el.addClass('geop-manager-modal--has-no-choices');
      self.reflectChoiceCount();
    };
    ...
    self.refreshSelectEverything = function() {
      ...
      return self.api('list', listOptions, function(response) {
        ...
        self.getSelectEverything().addClass('geop-active');
        self.reflectSelectEverythingCheckbox();
      });
    };
    ...
    self.reflectHasChoices = function() {
      if (!self.getIds().length) {
        ...
        self.$el.addClass('geop-manager-modal--has-choices');
        self.$el.removeClass('geop-manager-modal--has-no-choices');
      }
    };
    ...
    self.refresh = function(callback) {
      ...
      return self.api('list', listOptions, function(response) {
        ...
        geop.emit('enhance', self.$filters);
        geop.emit('enhance', self.$manageView);
        geop.emit('enhance', self.$pager);
    ...
    // of `geoportal-images` are honored.
