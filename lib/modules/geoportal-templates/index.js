...
// which exist courtesy of [geoportal-module](https://docs.geoportalcms.org/geoportal/modules/geoportal-module)
// and invoke methods of this module more conveniently for you.
//
// You may have occasion to call `self.geop.templates.safe` when
// implementing a helper that returns a value that should not be
// escaped by Nunjucks. You also might call `self.geop.templates.filter` to
...
// You may find it easier and more maintainable to call `geop.templates.addFilter(name, fn)`.
//
// ### `language`: your own alternative to the object
// returned by require('nunjucks'). Replacing Nunjucks
// entirely in Geoportal would be a vast undertaking, but perhaps
...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.templateApos = {
      modules: {},
      log: function(msg) {
        self.geop.utils.log.apply(self.geop, arguments);
      },
      prefix: self.geop.prefix
    };

    self.helperShortcuts = {};
    self.filters = {};

    self.nunjucks = options.language || require('@geoportalcms/nunjucks');

    // Add helpers in the namespace for a particular module.
    // They will be visible in nunjucks at
    // geop.modules[module-name].helperName. If the alias
    // option for the module is set to "shortname", then
    // they are also visible as geop.shortname.helperName.
    ...
    // and should only be used for geop.area, geop.singleton,
    ...
    self.modulesReady = function() {
      ...
      _.each(self.templateApos.modules, function(helpers, moduleName) {
        var alias = self.geop.modules[moduleName].options.alias;
        if (alias) {
      ...
      _.each(self.templateApos.modules, function(helpers, moduleName) {
        helpers.options = self.geop.modules[moduleName].options;
      });
    ...
    // Geoportal Nunjucks helpers such as `geop.area` are
    ...
    self.renderBody = function(req, type, s, data, module) {
      ...
      try {
        ...
        // "Why are you using addGlobal here?" Because Geoportal
        ...
        self.geop.utils.error('e.stack: ', e.stack);
        throw e;
      };
      self.renderDepth--;
      if (!self.renderDepth) {
    ...
    self.addStandardFilters = function(env) {
      ...
      env.addFilter('date', function(date, format, locale) {
        ...
        var dateLocale = locale || self.geop.templates.contextReq.locale;
        var m = moment(date);
        if (dateLocale) {
          m.locale(dateLocale);
        }
      ...
      env.addFilter('build', self.geop.urls.build);
      ...
      env.addFilter('nlbr', function(data) {
        data = self.geop.utils.globalReplace(data, "\n", "<br />\n");
        return self.geop.templates.safe(data);
      });
      ...
      env.addFilter('css', function(s) {
        return self.geop.utils.cssName(s);
      });

      env.addFilter('clonePermanent', function(o, keepScalars) {
        return self.geop.utils.clonePermanent(o, keepScalars);
      });
      ...
      env.addFilter('jsonAttribute', function(data, options) {
        if (typeof (data) === 'object') {
          return self.safe(self.geop.utils.escapeHtml(JSON.stringify(data), options));
        } else {
          // Make it a string for sure
          data += '';
          return self.safe(self.geop.utils.escapeHtml(data, options));
        }
      });
      ...
      // geop.utils.isBlessed and the same additional arguments. See
      // self.geop.utils.bless.
      ...
      env.addFilter('bless', function(options /* , 'widget', widget.type... */) {
        ...
        self.geop.utils.bless.apply(self.geop.utils, args);
        return options;
      });
    };

    // Typically you will call the `sendPage` method of
    // your own module, provided by the `geoportal-module`
    ...
    self.renderPageForModule = function(req, template, data, module) {
      ...
      var globalCalls = self.geop.push.getBrowserCalls('always');
      if (scene === 'user') {
        globalCalls += self.geop.push.getBrowserCalls('user');
      }
      ...
      var decorate = !(req.query.geop_refresh || req.query.xhr || req.xhr || (req.decorate === false));

      if (req.query.geop_refresh) {
        // Trigger the geop.ready and geop.enhance events after the
        // DOM settles and pushed javascript has had a chance to run;
        // do it just in the context of the refreshed main content div
        req.browserCall('geop.pageReadyWhenCalm($("[data-geop-refreshable]"))');
      } else if (decorate) {
        // Trigger the geop.ready and geop.enhance events after the
        // DOM settles and pushed javascript has had a chance to run
        req.browserCall('geop.pageReadyWhenCalm($("body"));');
      } else {
        // If we're ajaxing something other than data-geop-refreshable,
      ...
      if (req.user) {
        req.browserCall('geop.user = ?;', _.pick(req.user, 'title', '_id', 'username'));
      }

      req.browserCall('geop.scene = ?', scene);
      ...
      if (parsed.query && parsed.query.geop_refresh) {
        delete parsed.query.geop_refresh;
        delete parsed.search;
        dataUrl = urls.format(parsed);
      }

      var args = {
        outerLayout: decorate ? 'geoportal-templates:outerLayout.html' : 'geoportal-templates:refreshLayout.html',
        permissions: (req.user && req.user._permissions) || {},
        when: scene,
        js: {
        ...
        refreshing: req.query && (!!req.query.geop_refresh),
      ...
      if (req.geopError) {
        ...
        return error(req.geopError, 'template');
      }
      ...
      function error(e, type) {
        ...
        self.geop.utils.error(':: ' + now + ': ' + type + ' error at ' + req.url);
        self.geop.utils.error('Current user: ' + (req.user ? req.user.username : 'none'));
    ...
    // is attached to `req.data`, where the string `req.data.geopBodyClasses` is built up.
    ...
    self.addBodyClass = function(req, bodyClass) {
      req.data.geopBodyClasses = (req.data.geopBodyClasses ? (req.data.geopBodyClasses + ' ') : '') +
        bodyClass;
    };

    // Add a body attribute to be emitted when the page is rendered. This information
    // is attached to `req.data`, where `req.data.geopBodyDataAttributes` is built up
    ...
    self.addBodyDataAttribute = function(req, name, value = '') {
      ...
      _.each(values, (value, key) => {
        ...
        req.data.geopBodyDataAttributes = (req.data.geopBodyDataAttributes ? (req.data.geopBodyDataAttributes + ' ') : ' ') +
        ('data-' + (!_.isUndefined(value) && value.toString().length > 0 ? (self.geop.utils.escapeHtml(key) + ('="' + self.geop.utils.escapeHtml(value) + '"')) : self.geop.utils.escapeHtml(key)));
      });
    };
    ...
    // `geop.templates.prepended('location')` call, with the same location, to
    ...
    // to Geoportal and allows you to add that markup without forcing
    ...
    self.prepend = function(location, helperFn) {
      if ((typeof helperFn) === 'string') {
        throw new Error('Do not pass a string to geop.templates.prepend. Pass a function which returns a string and optionally takes a req argument.');
      }
    ...
    self.append = function(location, helperFn) {
      if ((typeof helperFn) === 'string') {
        throw new Error('Do not pass a string to geop.templates.append. Pass a function which returns a string and optionally takes a req argument.');
      }
      return self.insert('append', location, helperFn);
    };

    // Implementation detail of `geop.templates.prepend` and `geop.templates.append`.
    ...
    // Implementation detail of `geop.templates.prepended` and `geop.templates.appended`.

    self.inserted = function(end, location) {
      var key = end + '-' + location;
      return self.geop.templates.safe(_.map(self.insertions[key] || [], function(helperFn) {
        return helperFn(self.geop.templates.contextReq);
      }).join('\n'));
    };
    ...
    // (HTML tags are allowed). Available as the `geop.templates.prepended`
    ...
    // location and returns the resulting markup as a Nunjucks "safe" string
    // (HTML tags are allowed). Available as the `geop.templates.appended`
    // helper, and invoked as such by `outerLayoutBase.html`.
