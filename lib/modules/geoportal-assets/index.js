...
// the [geoportal-module](../geoportal-module/README.md) base class.
//
// Geoportal implements two "asset scenes," `anon` and `user`. When
...
// to use geoportal schema-based forms on a page for anonymous site visitors.
//
// This module also pushes most of Geoportal's standard front-end assets,
...
// ### `lean`
//
// If this option is set to `true`, Geoportal will *not* push any assets to an anonymous, logged-out site visitor, except for those pushed with `{ when: 'lean' }`. By default this includes only a tiny subset of the `geop.utils` library with necessary services to make widget players possible, with no library dependencies.
//
// Note that this means assets pushed with `{ when: 'always' }` will *not* be received, except by logged-in users.
//
// There are also no widget players, except for modules that allow you to opt in to a lean widget player by passing the `player: true` option when configuring those modules. This is currently supported by `geoportal-video-widgets`.
//
// ### `static`
//
...
// See [Deploying Geoportal in the Cloud with Heroku](https://docs.geoportalcms.org/geoportal/tutorials/howtos/deploying-geoportal-in-the-cloud-with-heroku)
...
//
// `APOS_BUNDLE=1 node app geoportal:generation`
//
// Then make sure the variable is also set when running actual production instances of the site:
//
// `APOS_BUNDLE=1 node app`
//
// Alternatively, if you specified an explicit bundle name to `--create-bundle` when using `geoportal:generation`,
...
// generated to reference those files via uploadfs. But this is the hard way; just run `geoportal:generation` with
// APOS_BUNDLE=1, and also set `APOS_BUNDLE=1` in the environment when launching Geoportal. That's really all you
// have to do.
// See [Deploying Geoportal in the Cloud with Heroku](https://docs.geoportalcms.org/geoportal/tutorials/howtos/deploying-geoportal-in-the-cloud-with-heroku) for more information.
//
// ### `APOS_BUNDLE_CLEANUP_DELAY`
//
// If set to a number of milliseconds, Geoportal delays that long before
...
module.exports = {
  ...
  singletonWarningIfNot: 'geoportal-assets',

  afterConstruct: function(self) {
    self.geop.tasks.add('geoportal', 'generation',
    ...
    self.pushDefaults();
    self.isGenerationTask = (self.geop.argv._[0] === 'geoportal:generation');
    self.pushCreateSingleton();
  },

  beforeConstruct: function(self, options) {
    if (options.minify === undefined) {
      options.minify = options.geop.launder.boolean(process.env.APOS_MINIFY);
    }
  ...
  construct: function(self, options) {
    ...
    if ((process.env.APOS_BUNDLE === '1') || (options.geop.argv['create-bundle'] === true)) {
    ...
    self.determineGenerationFromDb = function() {
      ...
      return Promise.try(function() {
        ...
        if (!c) {
          throw new Error('You must first run the geoportal:generation task, with APOS_BUNDLE=1 set in the environment');
        }
    ...
    self.determineGeneration = function() {
      ...
      if (self.isGenerationTask) {
        ...
        generation = self.geop.utils.generateId();

        var bundle = self.geop.argv['create-bundle'] || self.simpleBundle;
        if (bundle) {
          if (bundle === true) {
            self.toBundleName = 'assets-' + generation;
            self.toBundle = self.geop.rootDir + '/data/temp/' + self.toBundleName;
          } else {
            self.toBundleName = bundle;
            self.toBundle = self.geop.rootDir + '/' + self.toBundleName;
          }
        ...
        if (self.geop.argv['create-bundle'] !== true) {
    ...
    self.enableBundles = function() {
      ...
      self.generations = self.geop.db.collection('geoportalGenerations');
      // Used to identify the current generation, based on a fixed _id, separate in purpose
      // from self.generations
      self.generationCollection = self.geop.db.collection('aposGeneration');
    ...
    self.extractBundleFromGenerationCollection = function() {
      ...
      return Promise.try(function() {
        ...
        // tar.extract did the job (most of it perhaps?) and then just
        // hung for no reason for us in production, so using actual tar, which
        // is probably faster anyway
        return Promise.promisify(spawn)('tar', [ '-zxf', self.getUploadfsBundleTempName(), '-C', self.geop.rootDir ]);
        function spawn(cmd, args, callback) {
          require('child_process').spawn(cmd, args).on('close', function(code) {
            if (!code) {
    ...
    self.on('geoportal:modulesReady', 'determineGenerationAndExtract', function() {
      return Promise.try(function() {
    ...
    self.uploadfsBundleCleanup = function() {
      ...
      function cleanup() {
          ...
          if (locked) {
            return self.geop.locks.unlock(self.__meta.name, function(_err) {
              if (err || _err) {
                ...
                self.geop.utils.error(err || _err);
              }
            });
          } else {
            if (err) {
        ...
        function insert(callback) {
          ...
          var current = {
            _id: self.generation,
            copies: self.enumerateCopies(self.extractedTarFiles ? { files: self.extractedTarFiles, prefix: './public' } : (self.geop.rootDir + '/' + self.toBundleName + '/public'), '/assets/' + self.generation),
            when: new Date()
          };
    ...
    // geoportal:generation task with the --create-bundle=NAME
    ...
    self.extractBundle = function(name) {
      self.recursiveCopy(self.geop.rootDir + '/' + name, self.geop.rootDir);
    };
    ...
    // pushAsset method of geoportal-module, the base class
    // of all modules.
    //
    ...
    self.afterInit = function(callback) {
      if (self.geop.isTask() && !self.isGenerationTask) {
        // Tasks other than geoportal:generation should not waste time and create conflicts by
      ...
      return self.geop.locks.withLock(self.__meta.name + ':devAssetBuild', function(callback) {
        return async.series([
          self.symlinkModules,
    ...
    // (where symbolic links to the `public` subdirectories of Geoportal modules are automatically
    // created by `symlinkModules`). If `root` is not set, the root
    // of the project is assumed.

    self.ensureFolder = function(root) {
      if (!root) {
        root = self.geop.rootDir;
      }
    ...
    // performance problem for Geoportal's assets, which are modest
      ...
      if (fs.existsSync(to)) {
        ...
        if (stat.isSymbolicLink()) {
          try {
            ...
            self.geop.utils.error('WARNING: old style symbolic link exists at ' + to + ' and I do not have the privileges to remove it. You probably ran this site as Administrator before. Remove that symbolic link and the others in that folder as Administrator and try again as this non-Administrator user.');
            process.exit(1);
          }
        } else {
          rimraf.sync(to);
    ...
    self.buildLessMasters = function(callback) {
      ...
      // are retroactive to the very first use of the mixin. So geoportal-ui-2
      // can alter decisions made in the geoportal module, for instance.
      return self.forAllAssetScenes(function(scene, callback) {
        ...
        // Avoid race conditions, if geoportal:generation created
              ...
              self.lessCssWatches.push(fs.watch(absolutePath, {
                ...
                if (fs.existsSync(masterFile)) {
                  ...
                  try {
                    ...
                    self.geop.utils.warn('Unable to touch master CSS file, you might have to restart yourself to see new changes:', e);
                  }
                }
              }));
            }
    ...
    self.minify = function(callback) {
      ...
      return self.forAllAssetScenes(function(scene, callback) {
        return async.eachSeries([ 'stylesheets', 'scripts' ], function(type, callback) {
          self.purgeExcept('/geop-minified/' + scene + '-*.' + self.typeMap[type], '-' + self.generation);
          var file = self.getAssetRoot() + '/public/geop-minified/' + scene + '-' + self.generation + '.' + self.typeMap[type];
          ...
          if (!needed) {
            needed = true;
            self.geop.utils.log('MINIFYING, this may take a minute...');
          }
        ...
        if (needed) {
          self.geop.utils.log('Minification complete.');
        }
        return callback(null);
      });
    };
    ...
    self.minifySceneAssetType = function(scene, type, minifier, callback) {
      ...
      var cache = self.geop.caches.get('minify');
      return async.series({
        checkCache: function (callback) {
          ...
          return async.map(assets, function(asset, callback) {
            ...
            return self.geop.utils.md5File(asset.file, callback);
          }, function(err, md5s) {
            if (err) {
              return callback(err);
            }
    ...
    self.minifyScript = function(script, callback) {
      ...
      if (!exists) {
        self.geop.utils.warn("Warning: " + script.file + " does not exist");
        return callback(null);
      }
      if (script.preshrunk) {
    ...
    self.compileStylesheet = function(stylesheet, callback) {
      ...
      if (!exists) {
        self.geop.utils.warn('WARNING: stylesheet ' + stylesheet.file + ' does not exist');
        return callback(null, '');
      }
      ...
      return less.render(fs.readFileSync(src, 'utf8'), lessOptions, function(err, css) {
        if (err) {
          self.geop.utils.error('LESS CSS ERROR:');
          self.geop.utils.error(err);
        }
        css = css.css;
        if ((self.isGenerationTask && self.simpleBundle) || self.geop.argv['sync-to-uploadfs']) {
          css = self.prefixCssUrlsWith(css, self.uploadfs().getUrl() + '/assets/' + self.generation);
        } else if (self.geop.prefix) {
    ...
    self.modulesReady = function() {
      ...
      // override Geoportal-level styles
      self.pushConfigured();
    };
    ...
    self.enableCsrf = function() {
      ...
      self.geop.push.browserCall('always', 'geop.csrf()');
    };
    ...
    self.enableHtmlPageId = function() {
      ...
      self.geop.push.browserCall('user', 'geop.enableHtmlPageId()');
    };

    self.enablePrefix = function() {
      // ensure that all $.ajax and related jquery calls
      // respect Geoportal's prefix automatically
      self.geop.push.browserCall('always', 'geop.prefixAjax()');
    };
    ...
    self.enableLessMiddleware = function() {
      var lessMiddlewareOptions = {
        ...
        render: {
          sourceMap: {
            ...
            sourceMapBasepath: self.geop.rootDir + '/public',
      ...
      self.lessMiddleware = lessMiddleware(self.geop.rootDir + '/public', lessMiddlewareOptions,
        {
          // parser options
        },
        {
    ...
    self.prefixCssUrls = function(css) {
      return self.prefixCssUrlsWith(css, self.geop.prefix);
    };
    ...
    self.servePublicAssets = function() {
      ...
      middleware.push(self.geop.express.static(
        self.geop.rootDir + '/public',
        self.options.static || {}
      ));
      self.expressMiddleware = {
        // Run really early, before all of the stuff geoportal-express normally
        // puts in, for performance reasons. Preempts expensive
        // queries related to `geoportal-global` on every static file
        when: 'beforeRequired',
    ...
    self.generationTask = function(callback) {
      ...
      function copyFilesToUploadfs(callback) {
        if (!(self.geop.argv['sync-to-uploadfs'] || (self.isGenerationTask && self.simpleBundle))) {
          return callback(null);
        }
    ...
    self.getUploadfsBundleTempName = function() {
      const temp = self.geop.rootDir + '/data/temp';
      if (!self.__tempMkdir) {
        self.mkdirp(temp);
    ...
    self.stylesheetsHelper = function(when) {
      if (self.options.minify) {
        return self.geop.templates.safe('<link href="' + self.assetUrl('/geop-minified/' + when + '-' + self.generation + '.css') + '" rel="stylesheet" />');
      } else {
        if (self.lessMasters && self.lessMasters[when]) {
          return self.geop.templates.safe('<link href="' + self.assetUrl(self.lessMasters[when].web) + '" rel="stylesheet" />');
        }
        return self.geop.templates.safe(_.map(self.filterAssets(self.pushed['stylesheets'], when), function(stylesheet) {
    ...
    self.scriptsHelper = function(when) {
      ...
      if (self.options.minify) {
        ...
        result += '<script src="' + self.assetUrl('/geop-minified/' + when + '-' + self.generation + '.js') + '"></script>\n';
        return self.geop.templates.safe(result);
      } else {
    ...
    self.uploadfs = function() {
      return self.geop.attachments.uploadfs;
    };

    self.addHelpers({
      // geop.assets.stylesheets renders markup to load CSS that
      // is needed on any page that will use Geoportal.
      //
      ...
      // geop.assets.scripts renders markup to load JS that
      // is needed on any page that will use Geoportal.
      //
      // `when` can be set to either `user` or `anon` and signifies
      // whether a user is logged in or not; when users are
      ...
      // See `outerLayout.html` in the templates module.
      //
      // geop.assets.scripts also creates the apos object,
    ...
    self.on('geoportal:destroy', 'closeWatches', function() {
    ...
    self.pushCreateSingleton = function() {
      ...
      self.geop.push.browserCall('always',
        'geop.create("geoportal-assets", ?)',
        {
