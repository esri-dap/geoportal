...
// ### `lean`
//
// If this option is set to `true`, Apostrophe will *not* push any assets to an anonymous, logged-out site visitor, except for those pushed with `{ when: 'lean' }`. By default this includes only a tiny subset of the `geop.utils` library with necessary services to make widget players possible, with no library dependencies.
//
...
module.exports = {
  ...
  afterConstruct: function(self) {
    ...
    self.pushDefaults();
    self.isGenerationTask = (self.geop.argv._[0] === 'apostrophe:generation');
  ...
  construct: function(self, options) {
    ...
    self.enableBundles = function() {
      ...
      self.generations = self.geop.db.collection('apostropheGenerations');
    ...
    self.stylesheetsHelper = function(when) {
      if (self.options.minify) {
        return self.geop.templates.safe('<link href="' + self.assetUrl('/apos-minified/' + when + '-' + self.generation + '.css') + '" rel="stylesheet" />');
      } else {
