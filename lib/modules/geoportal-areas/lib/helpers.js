...
module.exports = function(self, options) {
  self.addHelpers({
    ...
    // `geop.singleton` renders a single widget of a fixed type, standing alone
    ...
    // mind later and call `geop.area` with the same `name`.
    //
    ...
    //
    // If `_options` is not specified, Geoportal falls back to the options
    ...
    // very convenient when working with `geoportal-pieces`.
    //
    ...
    singleton: function(doc, name, type, _options) {
      ...
      if (arguments.length === 1) {
        ...
        if (!name.match(/^\w+$/)) {
          self.geop.utils.warn(name + ' in ' + doc.slug + ': area names should be made up only of letters, underscores and digits. Otherwise they may not save properly.');
        }
        options = { type: type };
        if (_.has(doc, name)) {
          area = doc[name];
      ...
      if (area._edit) {
        _.each(options.widgets || {}, function(options, type) {
          self.geop.utils.bless(self.geop.templates.contextReq, options, 'widget', type);
        });
      }
    ...
    // `geop.area` renders an area: a column of widgets of one or more types.
    //
    ...
    // in `geop.singleton`. See the documentation above for `addLabel`,
    ...
    area: function(doc, name, _options) {
      ...
      options.hint = options.hint || self.geop.templates.contextReq.__('Use the Add Content button to get started.');
    ...
    // geop.areas.widget renders one widget. Invoked by both `geop.area` and
    // `geop.singleton`. Not
    ...
    // Returns the rich text markup of all `geoportal-rich-text` widgets
    ...
    // providers of rich text, such as subclasses of `geoportal-rich-text`,
    ...
    richText: function(within, options) {
      ...
      return self.geop.templates.safe(self.richText(within, options));
    },
    ...
    widgetControlGroups: function(widget, options) {
      return self.widgetControlGroups(self.geop.templates.contextReq, widget, options);
    },
