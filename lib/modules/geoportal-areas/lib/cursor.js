      ...
      after: function(results, callback) {
          ...
          if (areasInfo.length) {
            ...
            if (req.areasLoadedFor[doc._id] >= 5) {
              if (!req.suppressAreaLoaderRecursionWarnings) {
                self.geop.utils.warn('WARNING: reached maximum area loader recursion level. Doc _id is ' + doc._id + '. Are you using projections for all joins?');
              }
        ...
        if (req.deferWidgetLoading && (!req.loadingDeferredWidgets)) {
          ...
          _.each(types, function(type) {
            var manager = self.geop.areas.getWidgetManager(type);
            if (!manager) {
              self.geop.areas.warnMissingWidgetType(type);
    ...
    self.addFilter('originalWidgets', {
      ...
      after: function(results) {
        _.each(results, function(result) {
          ...
          self.geop.areas.walk(result, function(area, dotPath) {
            _.each(area.items || [], function(item) {
