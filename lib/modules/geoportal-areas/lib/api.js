...
module.exports = function(self, options) {
  ...
  // These properties have special meaning in Geoportal docs and are not
  ...
  // this method is called for you when you extend the `geoportal-widgets`
  ...
  self.warnMissingWidgetType = function(name) {
    ...
    if (!self.missingWidgetTypes[name]) {
      self.geop.utils.error('WARNING: widget type ' + name + ' exists in content but is not configured');
  ...
  // `geop.area` and `geop.singleton` template helpers.
  ...
  self.renderArea = function(area, options) {
    ...
    if (self.geop.templates.contextReq.disableEditing) {
  ...
  self.saveArea = function(req, docId, dotPath, items, callback) {
    ...
    return async.series({
      find: function(callback) {
        return self.geop.docs.find(req, { _id: docId })
          .permission('edit-doc')
          .published(null)
      ...
      update: function(callback) {
        ...
        if (_.isEqual(self.geop.utils.clonePermanent(items), self.geop.utils.clonePermanent(existingItems))) {
        ...
        return self.geop.docs.update(req, doc, callback);
      }
    }, function(err) {
      return callback(err);
    });
  ...
  self.lockSanitizeAndSaveArea = function(req, areaInfo, callback) {
    ...
    var docId = self.geop.launder.id(areaInfo.docId);
    var dotPath = self.geop.launder.string(areaInfo.dotPath);
    ...
    return async.series([
      ...
      if (hardLocked) {
        return self.geop.locks.unlock('save-area-' + docId, function(_err) {
    ...
    function hardLock(callback) {
      return self.geop.locks.lock('save-area-' + docId, { waitForSelf: true }, function(err) {
        if (err) {
          return callback(err);
        }
        hardLocked = true;
    ...
    function advisoryLock(callback) {
      ...
      return self.geop.docs.lock(req, docId, req.htmlPageId, callback);
    }
  ...
  self.walk = function(doc, iterator) {
    return self.geop.docs.walk(doc, function(o, k, v, dotPath) {
      if (v && (v.type === 'area')) {
  ...
  // `geoportal-pieces` in a page template.
  ...
  self.getSchemaOptions = function(doc, name) {
    ...
    var manager = self.geop.docs.getManager(doc.type);
    if (!manager) {
  ...
  //
  // Also available as a helper via `geop.areas.richText(area, options)` in templates.
  //
  ...
  // if you wish.
  //
  // Also available as a helper via `geop.areas.plaintext(area, options)` in templates.
  //
  // Content will be retrieved from any widget type that supplies a
  // `getRichText` method.
  ...
  self.plaintext = function(within, options) {
    ...
    var plaintext = self.geop.utils.htmlToPlaintext(richText).trim();
    ...
    return self.geop.utils.truncatePlaintext(plaintext, options.limit, ellipsis);
  };

  // Very handy for imports of all kinds: convert plaintext to an area with
  // one `geoportal-rich-text` widget if it is not blank, otherwise an empty area. null and
  ...
  self.fromPlaintext = function(plaintext, options) {
    ...
    if (plaintext && ((typeof plaintext) === 'string') && plaintext.length) {
      ...
      if (options && options.el) {
        content = `<${options.el}>${self.geop.utils.escapeHtml(plaintext, true)}</${options.el}>`;
      } else {
        content = self.geop.utils.escapeHtml(plaintext, true);
      }
      area.items.push({
  ...
  // Convert HTML to an area with one 'geoportal-rich-text' widget, otherwise
  ...
  // geop.areas.richText() method which returns just the rich text of the area
  ...
  // also at the end of the `geoportal-global` middleware.
  ...
  self.widgetControlGroups = function(req, widget, options) {
    ...
    var controlGroups = ([
      ...
      {
        classes: 'geop-button-group--data',
        controls: [
          {
            label: '$editLabel',
            action: 'edit-item'
  ...
  self.addSchemaWidgetControls = function(req, widget, controlGroups) {
    ...
    return controlGroups.concat([{
      classes: 'geop-widget-schema-controls',
      controls: schema
    }]);
  };
  ...
  // merge new methods with all geoportal-cursors
  self.geop.define('geoportal-cursor', require('./cursor.js'));
};
