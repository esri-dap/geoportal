geop.define('geoportal-areas', {
  ...
  construct: function(self, options) {
    ...
    // Called by subclasses of `geoportal-widgets`. The `manager`
    // object must provide, at a minimum, `setData`, `getData` and
    // `play` methods. See `always.js` and `user.js` in the
    // `geoportal-widgets` module for examples. Normally you
    // will extend `geoportal-widgets` and this will be invoked
    // for you.
    ...
    // Geoportal event, which is triggered both on page load and
    ...
    self.enablePlayers = function() {
      ...
      geop.on('enhance', function($el) {
        ...
        $el.find('[data-geop-widget]').each(enhance);
        // Also works if $el itself is a widget
        $el.filter('[data-geop-widget]').each(enhance);
        ...
        function enhance() {
          ...
          if (geop.utils.widgetPlayers[type]) {
            // If a lean player is present, it always wins, so we
            // can progressively migrate to lean mode before
            // setting `lean: true` for `geoportal-assets` to completely
            // disable the classic frontend js players and libraries
            return;
          }
          var manager = self.getWidgetManager($widget.attr('data-geop-widget'));
    ...
    // widget. The `data-geop-widget` attribute is used to identify
    ...
    self.getWidgetData = function($widget) {
      var manager = self.getWidgetManager($widget.attr('data-geop-widget'));
      if (!manager) {
        geop.utils.warn('WARNING: no manager for widget type ' + $widget.attr('data-geop-widget'));
        return {};
      }
