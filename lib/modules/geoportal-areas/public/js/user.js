geop.define('geoportal-areas', {
  ...
  construct: function(self, options) {
    ...
    // When a `[data-geop-add-content]` button is clicked, toggle the `geop-active` class
    // on the closest ancestor with `[data-geop-dropdown]`, and also the closest
    // ancestor with `.geop-area-controls`.
    self.enhanceAddContent = function() {
      var $body = $('body');
      $body.on('click', '[data-geop-add-content]', function(e) {
        var $area = $(this).closest('.geop-area');
        var $dropdown = $(this).closest('[data-geop-dropdown]');
        if ($dropdown.hasClass('geop-active')) {
          // Call this consistently in all places we dismiss the menu
          $area.data('editor').dismissContextContentMenu();
        } else {
          $(this).closest('[data-geop-dropdown]').addClass('geop-active');
          $(this).closest('.geop-area-controls').addClass('geop-active');
          $(this).closest('.geop-area').addClass('geop-content-menu-active');
          ...
          $('.geop-context-menu-container').addClass('geop-content-menu-active');
    ...
    self.enhanceControlsHover = function(sel) {
      $(sel).find('[data-geop-widget]').addBack('[data-geop-widget]').each(function() {
        var $widget = $(this);
        $widget.on('mouseenter', '[data-geop-tooltip]', geop.ui.createTooltip);
        $widget.on('mouseleave click', '[data-geop-tooltip]', geop.ui.removeTooltip);
      ...
      function mouseoverEnhance($el) {
        $el.on('mouseover', function(e) {
          var $self = $(this);
          e.stopPropagation();
          $('.geop-area-widget-controls').removeClass('geop-peek');
          $self.find('[data-geop-widget-controls]:first .geop-area-widget-controls').addClass('geop-peek');
          $self.next().children().addClass('geop-peek');
        });
      };
      ...
      function mouseleaveEnhance($el) {
        $el.on('mouseout', function(e) {
          var $self = $(this);
          $self.find('[data-geop-widget-controls]:first .geop-area-widget-controls').removeClass('geop-peek');
          $self.next().children().removeClass('geop-peek');
        });
      }
    ...
    self.fromTemplate = function(sel) {
      return $('[data-geop-area-editor-templates]').find(sel).clone();
    };
    ...
    self.enableCkeditor = function() {
      ...
      CKEDITOR.plugins.addExternal('split', geop.prefix + self.options.action + '/js/ckeditorPlugins/split/', 'plugin.js');
    ...
    self.enableOnEnhance = function() {
      geop.on('enhance', function($context) {
    ...
    self.enableAll = function(sel) {
      ...
      $(sel).find('[data-geop-area-edit]').each(function() {
        var $el = $(this);
        ...
        if (_.has(self.editors, areaId)) {
          ...
          geop.create('geoportal-areas-editor', { $el: $el, action: self.options.action });
        }
      });
    ...
    self.remapDotPaths = function() {
      $('body').find('[data-geop-area]').each(function() {
        var $area = $(this);
    ...
    self.recalculateDotPathsInArea = function($area) {
      var docId = $area.attr('data-doc-id');
      var dotPath = $area.attr('data-dot-path');
      var $widgets = $area.findSafe('[data-geop-widget]', '[data-geop-area]');
      var ordinal = 0;
    ...
    self.recalculateDotPathsOfAreasInWidget = function($widget, docId, dotPath) {
      var $areas = $widget.findSafe('[data-geop-area]', '[data-geop-widget]');
      $areas.each(function() {
        var $area = $(this);
    ...
    self.enableShift = function() {
      ...
      geop.on('shiftDown', function() {
        $('[data-geop-move-item]').each(function() {
          var $self = $(this);
          if ($self.attr('data-geop-move-item') === 'up') {
            $self.children('i').toggleClass('icon-double-angle-up');
            $self.attr('data-geop-move-item', 'top');
          } else if ($self.attr('data-geop-move-item') === 'down') {
            $self.children('i').toggleClass('icon-double-angle-down');
            $self.attr('data-geop-move-item', 'bottom');
          }
        });
      });

      geop.on('shiftUp', function() {
        $('[data-geop-move-item]').each(function() {
          var $self = $(this);
          $self.children('i').removeClass('icon-double-angle-up');
          $self.children('i').removeClass('icon-double-angle-down');
          if ($self.attr('data-geop-move-item') === 'top') {
            $self.attr('data-geop-move-item', 'up');
          } else if ($self.attr('data-geop-move-item') === 'bottom') {
            $self.attr('data-geop-move-item', 'down');
          }
        });
      });
    };
    ...
    self.getWidgetOptions = function($widget) {
      var $area = $widget.closest('[data-geop-area]');
      var options = {};
    ...
    self.getWidgetData = function($widget) {
      ...
      if (!manager) {
        geop.utils.warn('WARNING: no manager for widget type ' + type);
        return;
      }
    ...
    self.setWidgetData = function($widget, data) {
      ...
      var manager = self.getWidgetManager(type);
      if (!manager) {
        geop.utils.log('WARNING: no manager for widget type ' + type);
        return;
      }
    ...
    // (subclasses of geoportal-areas-editor) which
    // are currently visible.

    self.getEditors = function() {
      var $areas = $('[data-geop-area]:visible');
      var editors = [];
      $areas.each(function() {
        var $area = $(this);
