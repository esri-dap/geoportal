...
geop.define('geoportal-areas-editor', {
  ...
  construct: function(self, options) {
    ...
    self.$body = $('body');
    self.action = geop.areas.options.action;

    self.resetEl = function($el) {
      self.$el = $el;
      if (self.$selected && self.$selected.length) {
        self.$selected = self.$el.find('[data-geop-widget-id="' + self.$selected.data('geop-widget-id') + '"]', '[data-geop-area]');
      }
      self.init();
    };
    ...
    self.init = function() {
      ...
      if (self.options.virtual) {
        ...
        self.$el.attr('data-geop-area-virtual', '1');
      }
      geop.areas.register(self.$el.attr('data-doc-id'), self.$el.attr('data-dot-path'), self);
      var canceled = false;
      self.$el.parents('[data-geop-area]').each(function() {
        if (canceled) {
          return;
        }
        var $parent = $(this);
      ...
      self.$controls = self.$el.findSafe('[data-geop-area-controls]', '[data-geop-widget]');
    ...
    self.addEmptyClass = function() {
      if (self.$el.find('[data-geop-widgets]').html() === "") {
        self.$el.addClass('geop-empty');
      }
    };
    ...
    self.registerClickHandlers = function() {
      self.link('geop-add-item', self.startAutosavingHandler(self.addItem));
      self.link('geop-edit-item', self.startAutosavingHandler(self.editItem));
      self.link('geop-move-item', self.startAutosavingHandler(self.moveItem));
      self.link('geop-trash-item', self.startAutosavingHandler(self.trashItem));
      self.link('geop-clone-item', self.startAutosavingHandler(self.cloneItem));
    ...
    self.registerEventHandlers = function() {
      ...
      self.$el.mouseover(function(e) {
        self.$el.addClass('geop-hover');
        e.stopPropagation();
      });
      self.$el.mouseout(function(e) {
        self.$el.removeClass('geop-hover');
        e.stopPropagation();
      });
    };
    ...
    // property, such as `geoportal-rich-text` (note no suffix).
    ...
    self.addItem = function($el, type, data, callback) {
      self.$el.removeClass('geop-empty');
      ...
      self.$selected = $el.parentsUntil('[data-geop-area]').filter('[data-geop-widget-wrapper]', self.$el).find('[data-geop-widget]').first();
    ...
    self.editItem = function($el) {
      var $widget = $el.closest('[data-geop-widget]');
      if ($widget) {
    ...
    self.moveItem = function($el) {
      ...
      var $wrapper = $el.closest('[data-geop-widget-wrapper]');
      var direction = $el.data('geop-move-item');
      ...
      geop.areas.remapDotPaths();
      geop.emit('widgetMoved', $wrapper.find('[data-geop-widget]'));
    };
    ...
    self.trashItem = function($el) {
      ...
      var $wrapper = $widget.parent('[data-geop-widget-wrapper]');
      var $undoer = self.fromTemplate('[data-geop-undo-remove-widget]');
      $undoer.find('[data-geop-widget-type-label]').text(geop.areas.getWidgetManager($widget.attr('data-geop-widget')).label);
      ...
      geop.areas.remapDotPaths();
      geop.emit('widgetTrashed', $widget);
    };

    self.getWidgets = function() {
      return self.$el.findSafe('[data-geop-widget]', '[data-geop-area]');
    };

    // Disable area controls interactions while certain menus are open
    self.disableAreaControls = function() {
      $('body').find('[data-geop-widget-controls]').addClass('geop-area-widget-controls--disabled');
    };

    self.enableAreaControls = function() {
      $('body').find('[data-geop-widget-controls]').removeClass('geop-area-widget-controls--disabled');
    };
    ...
    self.dismissContextContentMenu = function() {
      ...
      self.$el.find('[data-geop-dropdown]').removeClass('geop-active');
      self.$el.find('.geop-area-controls').removeClass('geop-active');
      self.$el.removeClass('geop-context-content-menu-active');
      ...
      $('.geop-context-menu-container').removeClass('geop-content-menu-active');
    ...
    self.removeInitialContent = function($el, entireItem) {
      if (entireItem) {
        ...
        $el.find('[data-geop-widget="geoportal-rich-text"]:has([data-initial-content])').remove();
      } else {
    ...
    self.stopEditingRichText = function() {
      ...
      geop.areas.getWidgetManager('geoportal-rich-text').stopEditing(self.$activeRichText);
    ...
    self.insertWidget = function($wrapper) {
      var $widget = $wrapper.children('[data-geop-widget]').first();
      self.insertItem($wrapper);
      self.enhanceWidgetControls($widget);
      self.respectLimit();
      geop.areas.remapDotPaths();
      geop.emit('enhance', $widget);
    };
    ...
    self.enhanceWidgetControls = function($widget) {
      var $controls = $widget.findSafe('[data-geop-widget-controls]', '[data-geop-area]');
      if (self.options.limit === 1) {
        $controls.addClass('geop-limit-one');
      }
      self.checkEmptyWidget($widget);
      $widget.parent('[data-geop-widget-wrapper]').draggable(self.draggableSettings);
      self.updateAllSchemaWidgetControlChoices($widget);
    };

    self.addAreaControls = function($widget) {
      $widget.parent('[data-geop-widget-wrapper]').append(self.$controls.clone().removeAttr('data-geop-area-controls-original'));
    };

    self.removeAreaControls = function($widget) {
      $widget.parent('[data-geop-widget-wrapper]').find('[data-geop-area-controls]').remove();
    };

    self.checkEmptyWidget = function($widget) {
      var type = $widget.attr('data-geop-widget');
      if (geop.areas.getWidgetManager(type).isEmpty($widget)) {
        $widget.addClass('geop-empty');
      }
    };
    ...
    self.replaceWidget = function($old, $wrapper) {
      var $widget = $wrapper.findSafe('[data-geop-widget]', '[data-geop-area]');
      var data = geop.areas.getWidgetData($widget);
      geop.areas.setWidgetData($widget, data);
      $old.replaceWith($widget);
      self.enhanceWidgetControls($widget);
      $widget.parent('[data-geop-widget-wrapper]').attr('class', $wrapper.attr('class'));
      geop.areas.remapDotPaths();
      $widget.data('areaEditor', self);
      geop.emit('enhance', $widget);
    };
    ...
    self.insertItem = function($item) {
      $item.find('[data-geop-widget]:first').data('areaEditor', self);
      if (self.$selected && self.$selected.length) {
        self.$selected.parent('[data-geop-widget-wrapper]').after($item);
      } else {
        self.$el.find('[data-geop-widgets]:first').prepend($item);
      }
      self.addAreaControls($item.findSafe('[data-geop-widget]', '[data-geop-area]'));
    };
    ...
    self.addSeparators = function($draggable) {
      ...
      $areas.each(function() {
        var $area = $(this);
        var $ancestor = $draggable.closest('[data-geop-area]');
        var i;
        ...
        function atTop() {
          ...
          $area.find('[data-geop-widgets]:first').prepend(self.newSeparator());
        }

        function after() {
          var $widgets = $area.findSafe('[data-geop-widget-wrapper]', '[data-geop-area]');
      ...
      $('[data-geop-drag-item-separator]:not(.geop-template)').droppable(self.separatorDropSettings);
      setImmediate(function() {
        $areas.addClass('geop-dragging');
      });
    };

    self.removeSeparators = function() {
      $(window).trigger('aposStopDragging');
      $('[data-geop-area]').removeClass('geop-dragging');
      $('[data-geop-refreshable] [data-geop-drag-item-separator]')
    ...
    self.getDroppableAreas = function($draggable) {
      var $widget = $draggable.find('[data-geop-widget]').first();
      // Only the current area, and areas that are not full; also
      // rule out areas that do not allow the widget type in question
      return $('[data-geop-area]').filter(function() {
        var editor = $(this).data('editor');
        if (editor && ((!editor.limitReached()) || ($widget.data('areaEditor') === editor))) {
          var movableOptionKey = [$widget.attr('data-geop-widget'), 'controls', 'movable'];
          var hasWidget = _.has(editor.options.widgets, $widget.attr('data-geop-widget'));
    ...
    self.newSeparator = function() {
      var $separator = self.fromTemplate('[data-geop-drag-item-separator]');
      return $separator;
    };
    ...
    self.editWidget = function($widget) {
      ...
      var type = self.$selected.attr('data-geop-widget');
      var data = geop.areas.getWidgetData($widget);
      ...
      geop.areas.getWidgetManager(type).edit(
        data,
        options,
        function(data, callback) {
          $.jsonCall(self.action + '/render-widget',
            ...
            // picky about HTML
              ...
              return callback(null, $newWidget.findSafe('[data-geop-widget]', '[data-geop-area]'));
            });
        }
      );
    };
    ...
    self.cloneWidget = function($widget) {
      ...
      function newIds($widget) {
        var id = geop.utils.generateId();
        $widget.attr('data-geop-widget-id', id);
        var data = geop.areas.getWidgetData($widget);
        data._id = id;
        var docId = data.__docId;
        geop.areas.setWidgetData($widget, data);
        var $descendants = $widget.find('[data-geop-widget]');
    ...
    self.checkEmptyAreas = function() {
      $('[data-geop-area]').each(function() {
        var $el = $(this);
        if ($el.find('[data-geop-widget]').length === 0) {
          $el.addClass('geop-empty');
        } else {
          $el.removeClass('geop-empty');
        }
      });
      return false;
    };
        ...
        function(data, saved) {
          $.jsonCall(self.action + '/render-widget',
            ...
            },
            {
              ...
              self.insertWidget($wrapper);
              self.checkEmptyAreas();
              saved(null, $wrapper.findSafe('[data-geop-widget]', '[data-geop-area]'));
    ...
    self.separatorDropSettings = {
      accept: '[data-geop-widget-wrapper]',
      activeClass: 'geop-active',
      hoverClass: 'geop-hover',
      tolerance: 'pointer',
      ...
      drop: function(event, ui) {
        ...
        var $oldWidget = $item.find('[data-geop-widget]');
        var oldEditor = self;
        var $newArea = $(event.target).closest('[data-geop-area]');
        ...
        oldEditor.startAutosavingThen(function() {
          ...
          newEditor.startAutosavingThen(function() {
            newEditor.reRenderWidget($item, function(err) {
              if (err) {
                geop.notify('An error occurred.', { type: 'error' });
                return;
              }
    ...
    self.reRenderWidget = function($wrapper, callback) {
      self.stopEditingRichText();
      var $widget = $wrapper.find('[data-geop-widget]');
      var type = $widget.attr('data-geop-widget');
      var data = geop.areas.getWidgetData($widget);
      var originalData = _.clone(data);
      var options = self.options.widgets[type] || {};
    ...
    self.serialize = function() {
      ...
      self.getWidgets().each(function() {
        var $item = $(this);
        var item = geop.areas.getWidgetData($item);
        items.push(item);
      });
      ...
      return geop.utils.clonePermanent(items);
    };
    ...
    self.prepareAutosaveRequest = function() {
      ...
      return {
        ...
        options: geop.areas.getAreaOptions(self.$el),
        items: items
      };
    };
        ...
        function(result) {
          if (result.status !== 'ok') {
            if ((result.status === 'locked') || (result.status === 'locked-by-me')) {
              // geop.notify unsuitable because this is quite modal.
              // TODO: implement an geop.alert that displays
              ...
              if (result.status !== 'error') {
                ...
                _.each(path, function(c) {
                  if (failing) {
                    ...
                    if (c.match(/^\d+$/)) {
                      $el = $el.findSafe('[data-geop-widget]', '[data-geop-widget]').eq(parseInt(c));
                      if ($el.length) {
                        $lastWidget = $el;
                      } else {
                        unconsumed.push(c);
                      ...
                      $el = $el.findSafe('[data-geop-area][data-dot-path$=".' + c + '"]', '[data-geop-widget]');
                      if (!$el[0]) {
                        unconsumed.push(c);
                        failing = true;
                      }
                ...
                if ($lastWidget.length) {
                  var $area = $lastWidget.closest('[data-geop-area]');
                  $lastWidget.data('errorPath', unconsumed);
                  $lastWidget.data('error', errorName);
                  $area.data('editor').editWidget($lastWidget);
                  geop.notify('Incomplete or incorrect information was provided. Please review.');
                } else {
                  geop.notify('An error occurred saving the document.', { type: 'error' });
                }
              } else {
                geop.notify('An error occurred saving the document.', { type: 'error' });
              }
    ...
    self.respectLimit = function() {
      ...
      if (self.limitReached()) {
        self.$el.addClass('geop-limit-reached');
        $toggles.addClass('geop-disabled');
      } else {
        self.$el.removeClass('geop-limit-reached');
        $toggles.removeClass('geop-disabled');
      }
    };
    ...
    self.fromTemplate = function(sel) {
      return geop.areas.fromTemplate(sel);
    };
    ...
    self.enableHideControlsOnRichTextStart = function() {
      ...
      self.on('aposRichTextStarted', function(e) {
        ...
        self.dismissContextContentMenu();
        $(e.target).find('[data-geop-area-item-buttons]:first').hide();
      });
      ...
      self.on('aposRichTextActive', function(state) {
        self.$el.addClass('geop-rich-text-active');
      });
      self.on('aposRichTextInactive', function() {
        self.$el.removeClass('geop-rich-text-active');
      });
    };

    // Override default geoportal-context functionality because we
    // need to use $.onSafe, for the sake of nested areas.

    self.link = function(action, callback) {
      var attribute;
      attribute = 'data-' + geop.utils.cssName(action);
    ...
    self.on = function(eventType, selector, fn) {
      if (arguments.length === 2) {
        fn = selector;
        selector = undefined;
        self.$el.onSafe(eventType, '[data-geop-area]', fn);
      } else {
        self.$el.onSafe(eventType, selector, '[data-geop-area]', fn);
      }
    };
    ...
    self.startAutosaving = function(callback) {
      ...
      if (!self.$el.is('[data-autosave]')) {
        var $closest = self.$el.closest('[data-geop-area][data-autosave]');
        if ($closest.length) {
      ...
      if (self.$el.closest('[data-geop-area-virtual]').length) {
    ...
    self.changedSchemaWidgetControl = function(event) {
      ...
      geop.areas.setWidgetData($widget, data);
      self.reRenderWidget($widget.closest('[data-geop-widget-wrapper]'));
      return false;
    };

    self.updateAllSchemaWidgetControlChoices = function($widget) {
      var data = geop.areas.getWidgetData($widget);
      $widget.find('.geop-area-widget-controls:first [data-schema-widget-control-label]').each(function() {
        var $control = $(this);
    ...
    self.updateSchemaWidgetControlChoices = function($widget, name, selected) {
      ...
      var widgetType = $widget.attr('data-geop-widget');
      var manager = geop.areas.getWidgetManager(widgetType);
      ...
      function getChoices(callback) {
        ...
        geop.ui.globalBusy(true);
        return $.jsonCall(geop.schemas.action + '/choices', {
          field: field
        }, function(result) {
          geop.ui.globalBusy(false);
          if (result.status !== 'ok') {
