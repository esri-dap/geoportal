// Provides geop.ui, a singleton that provides user interface features
// used throughout Geoportal on the browser side.
//
...
geop.define('geoportal-ui', {
  ...
  construct: function(self, options) {
    ...
    // the data-geop-tooltip="YR TOOLTIP MSG" pattern
    ...
    self.prepTooltip = function() {
      var $tooltip = $('[data-geop-tooltip-template]');
      var $clone = $tooltip.clone();
      return $clone
        .removeAttr('data-geop-tooltip-template')
        .attr('data-geop-tooltip', 'true')
        .removeClass('geop-hidden');
    };
    ...
    self.createTooltip = function($el) {
      ...
      $newTooltip.text($this.attr('data-geop-tooltip'));
      ...
      if (top < 0) {
        $newTooltip.addClass('geop-tooltip--bottom');
      ...
      $newTooltip.addClass('geop-tooltip--visible');
    ...
    // Utility that lets you add the data attribute 'data-geop-actionable' to any element and have
    // its `geop-active` class be toggled on click.
    //
    ...
    self.enableActionable = function() {
      var $body = $('body');
      $body.on('click', '[data-geop-actionable]', function(event) {
        var $el = $(this);
        if ($el.attr('data-geop-actionable')) {
          $el.closest('[' + $el.attr('data-geop-actionable') + ']').toggleClass('geop-active');
        } else {
          $el.toggleClass('geop-active');
        }
      });
    };

    // Implement automatic width adjustment for `[data-geop-dropdown]` menus that have a
    // `[data-geop-dropdown-button-label]`.

    self.enhanceDropdowns = function() {
      $('body').on('click change', '[data-geop-dropdown]', function(event) {
        event.preventDefault();
        var $this = $(this);
        if ($this.find('[data-geop-dropdown-button-label]').length) {
          // only adjust width for button'y dropdowns
          // not global menu dropdowns
          var $label = $(this).find('[data-geop-dropdown-button-label]');
          if ($label.attr('data-geop-dropdown-button-label') === 'active') {
            $label.css('padding-right', $label.attr('data-geop-original-right-padding'));
            $label.attr('data-geop-dropdown-button-label', '');
          } else {
            $label.attr('data-geop-original-right-padding', $label.css('padding-right'));
            var val = parseInt($label.attr('data-geop-original-right-padding').substring(0, $label.attr('data-geop-original-right-padding').length - 2));
            var extra = ($(this).find('[data-geop-dropdown-items]')[0].getBoundingClientRect().width - $label[0].getBoundingClientRect().width + val);
            $label.css('padding-right', extra);
            $label.attr('data-geop-dropdown-button-label', 'active');
          }
        }
      });
    };
    ...
    // See also [geoportal-express](https://docs.geoportalcms.org/geoportal/modules/geoportal-express)
    ...
    // indicate Geoportal is busy loading a modal
    ...
    // locks are released.
    //
    // See also geop.busy for interactions that
    ...
    self.globalLock = function() {
      ...
      var $freezer = $('<div class="geop-global-busy"></div>');
      $('body').append($freezer);
    ...
    // Geoportal site, even if the prefix option is in use
    ...
    self.enhanceColorpicker = function($el, options) {
      ...
      function emptyColorpicker() {
        $label.text($el.attr('data-geop-color-empty-label'));
        $preview.addClass('geop-field-input-color-preview--empty');
      ...
      function populateColorpicker() {
        if ($el.val()) {
          ...
          $preview.removeClass('geop-field-input-color-preview--empty');
          $preview.css('background-color', $el.spectrum('get').toHexString());
        }
      }
    };

    // Converts geoportal-schemas 24 hour time field strings
    ...
    // of the attribute.
    //
    // self.link('geop-manage', 'blogPost', fn)
    //
    ...
    // flow that way, for example 'geop-workflow-export' does not
    ...
    self.link = function(sel, verb, object, callback) {
      ...
      var attribute;
      if (object === null) {
        attribute = 'data-' + geop.utils.cssName(verb);
      } else {
        attribute = 'data-' + geop.utils.cssName(verb) + '-' + geop.utils.cssName(object);
      }
    ...
    self.enableAjax = function() {
      $('body').on('submit', '[data-geop-ajax-context] form', self.ajaxSubmitHandler);
      $('body').on('click', '[data-geop-ajax-context] a', self.ajaxClickHandler);
    ...
    self.ajaxInfiniteScrollHandler = function() {
      var $infiniteScroll = $('[data-geop-ajax-infinite-scroll]');
    ...
    self.ajaxSubmitHandler = function(event) {
      ...
      // (window.location.pathname.replace(new RegExp('^' + geop.utils.regExpQuote(geop.prefix)), ''));
    ...
    self.ajaxClickHandler = function(event) {
      ...
      if ($anchor.attr('data-geop-no-ajax')) {
        return true;
      }
    ...
    // (the `data-geop-ajax-context` attribute of its closest ancestor that has one).

    self.ajaxContextOf = function($el) {
      return $el.closest('[data-geop-ajax-context]').attr('data-geop-ajax-context');
    };

    // Refresh the named `data-geop-ajax-context` with the content returned by the specified
    ...
    // is explicitly `false`.
    //
    // Any text input or textarea fields, and any other elements with a distinct data-geop-ajax-preserve
    ...
    //
    // After the replacement takes place, the `enhance` and `ajax` Geoportal events
    ...
    // `data-geop-ajax-preserve` and `data-geop-ajax-preserved` attributes.
    //
    // If `url` or `data` contains an `append` parameter set to `1`, then the portion of
    // the new content that falls within an element with the `data-geop-ajax-append` attribute
    // is appended to the corresponding element already nested within `data-geop-ajax-content`.
    // However everything *not* in side the `data-geop-ajax-append` element is replaced in the
    ...
    self.ajaxGo = function(name, url, data, options) {
      data = data || {};
      var $old = $('[data-geop-ajax-context="' + name + '"]');
      var $appendTo = $old.find('[data-geop-ajax-append]');
      var append = false;
      ...
      // add an geop-ajax=1 parameter if not present

      if (url.match(/[?&]geop-ajax=1/)) {
        hasAposAjax = true;
      }
      ...
      $.ajax({
        ...
        success: function(html) {
          ...
          if (append) {
            ...
            $appending = $new.find('[data-geop-ajax-append]');
          ...
          // a concern you can avoid it by checking for `data-geop-ajax-preserved`
          geop.emit('enhance', $old);
          geop.emit('ajax', $old);
          if (append) {
    ...
    self.ajaxPopStateHandler = function(event) {
      ...
      if (event.state && event.state.geopAjax) {
        self.ajaxGo(event.state.geopAjax.name, document.location, null, { push: false });
      }
    };

    // Push the specified URL to the browser history, associating it with the named data-geop-ajax-context
    // element in the page. Any geop-ajax query parameter is stripped off so that the history shows
    ...
    // within `data-geop-ajax-context`. To avoid unexpected results the default behavior
    ...
    self.ajaxError = function(err) {
      ...
      geop.notify('Unfortunately a server error occurred. Please try again later.', { type: 'error', dismiss: true });
    };
    ...
    // of $old with a data-geop-ajax-preserve attribute are preserved, and of
    ...
    //
    // Each preserved element must have a **unique value** for data-geop-ajax-preserve,
    ...
    self.replaceUnpreserved = function($old, $new, options) {
      ...
      function tagAncestors($context) {
        ...
        find($context, 'preserves').removeAttr('data-geop-ajax-preserves');
      ...
      function find($el, key) {
        return $el.find('[data-geop-ajax-' + key + ']');
      }

      // Find prefixed with specified value
      function findVal($el, key, val) {
        return $el.find('[data-geop-ajax-' + key + '="' + val + '"]');
      }

      // .is() prefixed
      function is($el, key) {
        return $el.is('[data-geop-ajax-' + key + ']');
      }

      // is() prefixed with specified value
      function isVal($el, key, val) {
        return $el.is('[data-geop-ajax-' + key + '="' + val + '"]');
      }

      // .attr() prefixed, getter and setter
      function attr($el, key, val) {
        if (arguments.length === 2) {
          return $el.attr('data-geop-ajax-' + key);
        }
        $el.attr('data-geop-ajax-' + key, val);
      }
