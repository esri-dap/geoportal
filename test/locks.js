...
describe('Locks', function() {
  ...
  it('should be a property of the apos object', function(done) {
    ...
    apos = require('../index.js')({
      ...
      afterInit: function(callback) {
        assert(geop.modules['geoportal-locks']);
        assert(geop.modules['geoportal-locks-1']);
        assert(geop.modules['geoportal-locks-2']);
        assert(geop.modules['geoportal-locks-3']);
  ...
  it('cleanup', function(done) {
    geop.locks.db.remove({}, function(err) {
      assert(!err);
      done();
    });
  });
  ...
  it('should allow a single lock without contention uneventfully', function(done) {
    var locks = geop.modules['geoportal-locks'];
  ...
  it('four parallel lock calls via the different modules should all succeed but not simultaneously', function(done) {
    var one = geop.modules['geoportal-locks'];
    var two = geop.modules['geoportal-locks-1'];
    var three = geop.modules['geoportal-locks-2'];
    var four = geop.modules['geoportal-locks-3'];
    var active = 0;
    var successful = 0;
    attempt(one);
    attempt(two);
