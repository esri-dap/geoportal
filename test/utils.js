...
describe('Utils', function() {
  ...
  describe('methods', function() {
    ...
    it('truncatePlaintext: should tuncate a message without cutting off a word', function(done) {
      var s = geop.utils.truncatePlaintext('I want to be cut off here. This is an extra sentance.', 25);
    ...
    it('escapeHtml: should replace html tags with html string entites', function(done) {
      var s = geop.utils.escapeHtml('<div>hello</div>');
    ...
    it('htmlToPlaintext: should strip all html notation', function(done) {
      var s = geop.utils.htmlToPlaintext('<div>hello</div>');
    ...
    it('capitalizeFirst: should capitalize the first letter', function(done) {
      var s = geop.utils.capitalizeFirst('hello');
    ...
    it('cssName: should covert camelCase or underscore name formats to hyphenated css-style', function(done) {
      var s = geop.utils.cssName('camelCase and under_score');
    ...
    it('cssName: should preserve double dash', function() {
      var s = geop.utils.cssName('this-is--doubled');
      assert(s === 'this-is--doubled');
    });

    it('cssName: should not preserve triple dash', function() {
      var s = geop.utils.cssName('this-is---tripled');
    ...
    it('addSlashIfNeeded: should add a slash "/" to the end of a path if necessary', function(done) {
      var s = geop.utils.addSlashIfNeeded('/my/path');
    ...
    it('clonePermanent should not behave bizarrely with a test case from the punkave site', function() {
      ...
      var clone = geop.utils.clonePermanent(input);
    ...
    it('gives sensible results for insensitiveSort', function() {
      ...
      geop.utils.insensitiveSort(input);
    ...
    it('does not crash when geop.utils.profile is called with two arguments', function() {
      geop.utils.profile(geop.tasks.getReq(), 'this.is.a.path')();
      assert(true);
    });

    it('does not crash when geop.utils.profile is called with three arguments', function() {
      geop.utils.profile(geop.tasks.getReq(), 'this.is.a.path', 100);
      assert(true);
    });

    it('does not crash when geop.utils.profile is called with one argument (no req arg)', function() {
      geop.utils.profile('this.is.a.path')();
      assert(true);
    });

    it('does not crash when geop.utils.profile is called with two arguments (no req arg)', function() {
      geop.utils.profile('this.is.a.path', 100);
      assert(true);
    });
