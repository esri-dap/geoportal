...
describe('Pieces', function() {
  ...
  it('should initialize with a schema', function(done) {
    apos = require('../index.js')({
      ...
      afterInit: function(callback) {
        assert(geop.modules['things']);
        assert(geop.modules['things'].schema);
        geop.argv._ = [];
  ...
  function findPiece(req, id, callback) {
    return geop.modules['things'].find(req, { _id: id })
      .permission('edit')
      .published(null)
      .trash(null)
  ...
  it('should make sure there is no test data hanging around from last time', function(done) {
    ...
    geop.docs.db.remove({}, function(err) {
      ...
      geop.docs.db.findWithProjection({ _id: 'testThing' }).toArray(function(err, docs) {
        assert(!err);
  ...
  it('should be able to create a new piece', function() {
    assert(geop.modules['things'].newInstance);
    var thing = geop.modules['things'].newInstance();
    assert(thing);
  ...
  it('should be able to insert a piece into the database', function(done) {
    assert(geop.modules['things'].insert);
    geop.modules['things'].insert(geop.tasks.getReq(), testThing, function(err, piece) {
      assert(!err);
  ...
  it('same thing with promises', function(done) {
    assert(geop.modules['things'].insert);
    geop.modules['things'].insert(geop.tasks.getReq(), testThing2)
  ...
  it('should be able to retrieve a piece by id from the database', function(done) {
    assert(geop.modules['things'].requirePiece);
    var req = geop.tasks.getReq();
    req.body = {};
    req.body._id = "testThing";
    geop.modules['things'].requirePiece(req, req.res, function() {
      assert(req.piece);
  ...
  it('should be able to update a piece in the database', function(done) {
    assert(geop.modules['things'].update);
    testThing.foo = 'moo';
    geop.modules['things'].update(geop.tasks.getReq(), testThing, function(err, piece) {
      assert(!err);
      assert(testThing === piece);
  ...
  it('same thing with promises', function(done) {
    assert(geop.modules['things'].update);
    testThing.foo = 'goo';
    geop.modules['things'].update(geop.tasks.getReq(), testThing)
      .then(function(piece) {
        assert(testThing === piece);
        // Now let's get the piece and check if it was updated
        var req = geop.tasks.getReq();
  ...
  it('should only execute filters that are safe and have a launder method', function() {
    ...
    var mockCursor = geop.docs.find(geop.tasks.getAnonReq());
    _.merge(mockCursor, {
      filters: {
        publicTest: {
          launder: function(s) {
  ...
  it('should add some more things for testing', function(done) {
    ...
    async.each(additionalThings, function(thing, callback) {
      geop.modules['things'].insert(geop.tasks.getReq(), thing, function(err) {
        callback(err);
      });
    }, function(err) {
      assert(!err);
  ...
  it('should list all the pieces if skip and limit are set to large enough values', function(done) {
    assert(geop.modules['things'].list);
    var req = geop.tasks.getReq();
    var filters = {
      limit: 10,
      skip: 0
    };
    geop.modules['things'].list(req, filters, function(err, results) {
      assert(!err);
  ...
  it('should be able to trash a piece', function(done) {
    assert(geop.modules['things'].trash);
    assert(geop.modules['things'].requirePiece);
    var req = geop.tasks.getReq();
    var id = 'testThing';
    req.body = {_id: id};
    ...
    findPiece(req, id, function(err, piece) {
      assert(!err);
      assert(!piece.trash);
      geop.modules['things'].trash(req, id, function(err) {
        assert(!err);
  ...
  it('should be able to rescue a trashed piece', function(done) {
    assert(geop.modules['things'].rescue);
    var req = geop.tasks.getReq();
    var id = 'testThing';
    req.body = {_id: id};
    ...
    findPiece(req, id, function(err, piece) {
      assert(!err);
      assert(piece.trash === true);
      geop.modules['things'].rescue(req, id, function(err) {
        assert(!err);
  ...
  it('should pass through an error message if the error is passed as a string', function(done) {
    assert(geop.modules['things'].apiResponse);
    var res = geop.tasks.getAnonReq().res;
    ...
    geop.modules['things'].apiResponse(res, errMsg, { foo: 'bar' });
  });
  ...
  it('should properly pass a result as a json if there is no error', function(done) {
    ...
    geop.modules['things'].apiResponse(res, null, { foo: 'bar' });
  });
  ...
  it('should insert an item from the routes.insert method', function(done) {
    assert(geop.modules['things'].routes.insert);
    ...
    return geop.modules['things'].routes.insert(req, res);
  });

  // routes.retrieve
  it('should get an item from the routes.retrieve method', function(done) {
    assert(geop.modules['things'].routes.retrieve);
    ...
    return geop.modules['things'].routes.retrieve(req, res);
  });

  // routes.list
  it('should get a list of all the items from routes.list', function(done) {
    assert(geop.modules['things'].routes.list);
    ...
    return geop.modules['things'].routes.list(req, res);
  });

  // routes.update
  it('should update an item in the database from route.update', function(done) {
    assert(geop.modules['things'].routes.update);
    ...
    geop.modules['things'].requirePiece(req, res, function() {
      return geop.modules['things'].routes.update(req, res);
    });
  });

  // routes.trash
  it('should trash an item in the database from route.trash', function(done) {
    assert(geop.modules['things'].routes.trash);
    assert(geop.modules['things'].requirePiece);
    ...
    findPiece(req, id, function(err, piece) {
      assert(!err);
      assert(!piece.trash);
      geop.modules['things'].routes.trash(req, res);
    });
  ...
  it('should rescue an item in the database from route.rescue', function(done) {
    assert(geop.modules['things'].routes.rescue);
    assert(geop.modules['things'].requirePiece);
    ...
    findPiece(req, id, function(err, piece) {
      assert(!err);
      assert(piece.trash === true);
      geop.modules['things'].routes.rescue(req, res);
    });
  ...
  it('people can find things via a join', function() {
    var req = geop.tasks.getReq();
    return geop.docs.db.insert(testPeople)
      .then(function() {
        return geop.docs.getManager('person').find(req, {}).toObject();
      })
  ...
  it('people cannot find things via a join with an inadequate projection', function() {
    var req = geop.tasks.getReq();
    return geop.docs.getManager('person').find(req, {}, {title: 1}).toObject()
      .then(function(person) {
        assert(person);
        assert(person.title === 'Bob');
  ...
  it('people can find things via a join with a "projection" of the join name', function() {
    var req = geop.tasks.getReq();
    return geop.docs.getManager('person').find(req, {}, {title: 1, _things: 1}).toObject()
      .then(function(person) {
        assert(person);
        assert(person.title === 'Bob');
        assert(person._things);
