...
describe('Versions', function() {
  ...
  it('inserting a doc should result in a version', function(done) {
    ...
    geop.docs.insert(geop.tasks.getReq(), object, function(err, object) {
      ...
      geop.versions.db.findWithProjection({ docId: docId }).toArray(function(err, versions) {
        assert(!err);
  ...
  it('should be able to update', function(done) {
    geop.docs.find(geop.tasks.getReq(), { slug: 'one' }).toArray(function(err, docs) {
      ...
      geop.docs.update(geop.tasks.getReq(), object, function(err, object) {
        ...
        // did the versions module kick-in?
        geop.versions.db.findWithProjection({ docId: object._id }).sort({createdAt: -1}).toArray(function(err, versions) {
          assert(!err);
          // we should have a document
          assert(versions);
  ...
  it('should be able to revert to a previous version', function(done) {
    geop.docs.find(geop.tasks.getReq(), { slug: 'one' }).toObject(function(err, doc) {
      assert(!err);
      geop.versions.find(geop.tasks.getReq(), { docId: doc._id }, {}, function(err, versions) {
        assert(!err);
        assert(versions.length === 2);
        geop.versions.revert(geop.tasks.getReq(), versions[1], function(err) {
          assert(!err);
  ...
  it('should be able to fetch all versions in proper order', function(done) {
    ...
    geop.docs.find(req, { slug: 'one' }).toObject(function(err, doc) {
      assert(!err);
      geop.versions.find(geop.tasks.getReq(), { docId: doc._id }, {}, function(err, versions) {
        assert(!err);
  ...
  it('should be able to compare versions and spot a simple field change', function(done) {
    ...
    geop.docs.find(req, { slug: 'one' }).toObject(function(err, doc) {
      assert(!err);
      geop.versions.find(req, { docId: doc._id }, {}, function(err, versions) {
        assert(!err);
        assert(versions.length === 3);
        return geop.versions.compare(req, doc, versions[1], versions[0], function(err, changes) {
          assert(!err);
