...
describe('Templates', function() {
  ...
  it('should render pages successfully with outerLayout', function() {
    var req = geop.tasks.getAnonReq();
    var result = geop.modules['templates-test'].renderPage(req, 'page');
  ...
  it('cross-module-included files should be able to include/extend other files relative to their own module', function() {
    var req = geop.tasks.getAnonReq();
    var result = geop.modules['templates-test'].renderPage(req, 'pageWithLayout');
    assert(result.indexOf('<title>I am the title</title>') !== -1);
  ...
  it('should render pages successfully with prepend and append to locations', function() {
    ...
    geop.templates.prepend('head', function(req) {
      assert(req.res);
      return '<meta name="before-test" />';
    });
    geop.templates.append('head', function(req) {
      assert(req.res);
      return '<meta name="after-test" />';
    });
    geop.pages.addAfterContextMenu(function(req) {
      assert(req.res);
      return '<h4>After the Context Menu</h4>';
    });
    var result = geop.pages.renderPage(req, 'pages/withLayout');
  ...
  it('should not escape <br />', function() {
    var req = geop.tasks.getAnonReq();
    var result = geop.modules['templates-test'].render(req, 'testWithNlbrFilter');
  ...
  it('should format dates', function() {
    var req = geop.tasks.getAnonReq();
    // default locale
    var result = geop.modules['templates-test'].renderString(req, '{{ data.now | date(\'LLLL\') }}', {
      now: new Date(2018, 11, 1)
    });
    assert.equal(result, 'Saturday, December 1, 2018 12:00 AM');
    // locale parameter is respected
    result = geop.modules['templates-test'].renderString(req, '{{ data.now | date(\'LLLL\', \'en-gb\') }}', {
      now: new Date(2018, 11, 1)
    });
    ...
    result = geop.modules['templates-test'].renderString(req, '{{ data.now | date(\'LLLL\') }}', {
      now: new Date(2018, 11, 1)
    });
    assert.equal(result, 'Saturday, December 1, 2018 12:00 AM');
  });
