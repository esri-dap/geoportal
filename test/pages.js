...
describe('Pages', function() {
  ...
  it('parked trash can exists', function(done) {
    return geop.pages.find(geop.tasks.getReq(), { slug: '/trash' }).published(null).trash(null).toObject(function(err, trash) {
      assert(!err);
      assert(trash);
  ...
  it('should have a find method on pages that returns a cursor', function() {
    var cursor = geop.pages.find(geop.tasks.getAnonReq());
    assert(cursor);
  });

  it('should be able to find the parked homepage', function(done) {
    var cursor = geop.pages.find(geop.tasks.getAnonReq(), { slug: '/' });
  ...
  it('should be able to find just a single page', function(done) {
    var cursor = geop.pages.find(geop.tasks.getAnonReq(), { slug: '/child' });
  ...
  it('is able to insert a new page', function(done) {
    ...
    geop.pages.insert(geop.tasks.getReq(), parentId, newPage, function(err, page) {
  ...
  it('is able to insert a new page in the correct order', function(done) {
    var cursor = geop.pages.find(geop.tasks.getAnonReq(), { slug: '/new-page' });
  ...
  it('is able to insert a new page with promises', function(done) {
    ...
    geop.pages.insert(geop.tasks.getReq(), parentId, newPage).then(function(page) {
  ...
  it('is able to insert a new page in the correct order with promises', function(done) {
    var cursor = geop.pages.find(geop.tasks.getAnonReq(), { slug: '/new-page-2' });
  ...
  it('is able to move root/parent/sibling/cousin after root/parent', function(done) {
    // 'Cousin' _id === 4312
    // 'Parent' _id === 1234
    geop.pages.move(geop.tasks.getReq(), '4312', '1234', 'after', function(err) {
      if (err) {
        console.log(err);
      }
      assert(!err);
      var cursor = geop.pages.find(geop.tasks.getAnonReq(), {_id: '4312'});
      cursor.toObject(function(err, page) {
        if (err) {
          console.log(err);
        }
  ...
  it('is able to move root/cousin before root/parent/child', function(done) {
    // 'Cousin' _id === 4312
    // 'Child' _id === 2341
    geop.pages.move(geop.tasks.getReq(), '4312', '2341', 'before', function(err) {
      if (err) {
        console.log(err);
      }
      assert(!err);
  ...
  it('is able to move root/parent/cousin inside root/parent/sibling', function(done) {
    // 'Cousin' _id === 4312
    // 'Sibling' _id === 4321
    geop.pages.move(geop.tasks.getReq(), '4312', '4321', 'inside', function(err) {
      if (err) {
        console.log(err);
      }
      assert(!err);
  ...
  it('moving /parent into /another-parent should also move /parent/sibling', function(done) {
    geop.pages.move(geop.tasks.getReq(), '1234', '4333', 'inside', function(err) {
      if (err) {
        console.log(err);
      }
      assert(!err);
      var cursor = geop.pages.find(geop.tasks.getAnonReq(), {_id: '4321'});
      cursor.toObject(function(err, page) {
        if (err) {
          console.log(err);
        }
  ...
  it('should detect that the home page is an ancestor of any page except itself', function() {
    ...
    assert(!geop.pages.isAncestorOf({
      path: '/'
    }, {
      path: '/'
    }));
  ...
  it('is able to move parent to the trash', function(done) {
    geop.pages.moveToTrash(geop.tasks.getReq(), '1234', function(err) {
      if (err) {
        console.error(err);
      }
      assert(!err);
      var cursor = geop.pages.find(geop.tasks.getAnonReq(), {_id: '1234'});
      cursor.toObject(function(err, page) {
        if (err) {
          console.log(err);
        }
        assert(!err);
        assert(!page);
        geop.pages.find(geop.tasks.getAnonReq(), { _id: '1234' })
...
describe('Pages with trashInSchema', function() {
  ...
  it('is able to move p2 inside p1', function(done) {
    // 'Cousin' _id === 4312
    // 'Parent' _id === 1234
    geop.pages.move(geop.tasks.getReq(), 'p2', 'p1', 'inside', function(err) {
      if (err) {
        console.log(err);
      }
      assert(!err);
      var cursor = geop.pages.find(geop.tasks.getAnonReq(), {_id: 'p2'});
      cursor.toObject(function(err, page) {
        if (err) {
          console.log(err);
        }
  ...
  it('p2c2 is now grandchild of p1, but still in trash', function() {
    return Promise.try(function() {
      return geop.docs.db.findOne({ _id: 'p2c2' });
    }).then(function(p2c2) {
      assert(p2c2.level === 3);
  ...
  it('add permissions for a new group to the home page', function() {
    const req = geop.tasks.getReq();
    let group;
    return Promise.try(function() {
      return geop.groups.insert(req, {
        title: 'test',
        permissions: [ 'edit-page' ]
      });
    }).then(function(_group) {
      group = _group;
      return geop.docs.db.findOne({ slug: '/' });
    }).then(function(home) {
      home.editGroupsIds = [ group._id ];
      const update = Promise.promisify(geop.pages.update);
  ...
  it('"apply to subpages": propagate group id to child pages', function() {
    ...
    return Promise.try(function() {
      ...
      return geop.docs.db.find({ slug: /^\//, trash: { $ne: true } }).toArray();
    }).then(function(pages) {
